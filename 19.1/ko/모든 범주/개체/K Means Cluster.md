...
**구문:** obj = K Means Cluster(...Window View( "Visible"|"Invisible"|"Private" )...) &lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = K Means Cluster(...&lt;By( column(s) )&gt;...)
...
**구문:** obj = K Means Cluster(...Columns( column(s) )...)
...
**구문:** obj = K Means Cluster(...&lt;Freq( column )&gt;...)
...
**구문:** obj = K Means Cluster(...&lt;Weight( column )&gt;...)
...
**구문:** obj = K Means Cluster(...Y( column(s) )...)
...
**구문:** Columns Scaled Individually( state=0|1 )

**설명:** 각 열을 다른 열과 독립적으로 척도화합니다. 기본적으로 설정되어 있습니다.
...
**구문:** obj &lt;&lt; Initial Clusters( "Default" | "Randomize" | column )

**설명:** 초기 군집 생성 방법을 결정합니다. 랜덤화하여 초기 군집을 무작위로 지정하거나, 범주형 열을 지정한 후 각 범주의 평균으로 시드를 설정하여 초기 군집을 정의할 수 있습니다.
...
**설명:** SOM(자기 조직화 지도)을 사용하여 군집을 생성합니다. 이 방법에 사용되는 격자 구조는 군집을 2차원으로 해석하는 데 사용할 수 있습니다.
...
**구문:** obj &lt;&lt; SOM Bandwidth( number )

**설명:** SOM(자기 조직화 지도)의 대역폭을 지정합니다.
...
**구문:** obj &lt;&lt; SOM N Rows( number )

**설명:** SOM(자기 조직화 지도)의 행 수를 설정합니다.
...
**구문:** obj &lt;&lt; Shift distances by rates( state=0|1 )

**설명:** 더 큰 군집에 점들이 할당되도록 높은 선호를 줍니다.
...
**구문:** obj &lt;&lt; Single Step( state=0|1 )

**설명:** 각각의 반복을 단계별로 진행할 수 있습니다. K 평균 개요에서 각 반복에 대해 단계 버튼을 클릭하십시오.
...
**구문:** obj &lt;&lt; Use within cluster std dev( state=0|1 )

**설명:** 각 군집에 대해 추정된 표준편차로 척도화된 거리를 계산합니다.
...
**구문:** obj &lt;&lt; SOM Heat Map( state=0|1 )

**설명:** 군집화에 사용된 Y 변수 중 하나로 색상이 지정된 SOM 군집 평균의 히트맵을 표시하거나 숨깁니다.

**JMP추가된 버전:** 17

```jsl

dt = Open( "$SAMPLE_DATA/Iris.jmp" );obj = K Means Cluster(	Y( :Sepal length, :Sepal width, :Petal length, :Petal width ),	{SOM N Rows( 2 ), SOM Bandwidth( 0.5 ), Single Step( 0 ), Number of Clusters( 4 ), SOM, Go});obj << SOM Heat Map;

```

#### Save Cluster Distance

**구문:** obj &lt;&lt; Save Cluster Distance

**설명:** 할당된 군집 번호에 대한 거리가 포함된 열을 데이터 테이블에 저장합니다.
...
**구문:** obj &lt;&lt; Save Distance Formula

**설명:** 할당된 군집 번호에 대한 거리 계산식이 포함된 열을 데이터 테이블에 저장합니다.
...
**구문:** obj &lt;&lt; Save K Cluster Distances

**설명:** 각 군집 중심에 대한 거리를 데이터 테이블에 별도의 열로 저장합니다.
...
**구문:** obj &lt;&lt; Save K Distance Formulas

**설명:** 각 군집 중심에 대한 거리 계산식을 데이터 테이블에 별도의 열로 저장합니다.
...
**구문:** obj &lt;&lt; Save SOM Grid

**설명:** 최대 확률 분류 군집에 대한 SOM 격자 행과 열이 포함된 새 열을 데이터 테이블에 저장합니다.
