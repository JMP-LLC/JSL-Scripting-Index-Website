...
dt = Open( "$SAMPLE_DATA/2D Gaussian Process Example.jmp" );dt << New Column( "_bycol",	Character,	Nominal,	Set Values( Repeat( {"A", "B"}, N Rows( dt ) )[1 :: N Rows( dt )] ));obj = dt << Gaussian Process( Y( :Y ), X( :X1, :X2 ), By( :_bycol ), Group Options( Return Group( 1 ) ) );obj[1] << Copy ByGroup Script;
...
**구문:** obj = Gaussian Process(...Window View( "Visible"|"Invisible"|"Private" )...) &lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj &lt;&lt; Block Size( number )

**설명:** 빠른 GASP에 대해 각 블록에 배치할 관측값 수를 설정합니다.
...
**구문:** obj &lt;&lt; Estimate Nugget( state=0|1 )

**설명:** 추정 절차에 능형 회귀 모수를 도입합니다. 잡음에 대해 평활함으로써 과대적합을 방지합니다.
...
**구문:** obj &lt;&lt; Fast GASP( state=0|1 )

**설명:** 빠른 GASP 옵션을 사용하여 수천 개의 행이 있는 모형을 적합시킬 수 있습니다.
...
**설명:** 예측 계산식을 생성하고 이를 계산식 저장소에 계산식 열 스크립트로 게시합니다.
...
**구문:** obj &lt;&lt; Publish Variance Formula

**설명:** 분산 계산식을 생성하고 이를 계산식 저장소에 계산식 열 스크립트로 게시합니다.
...
**구문:** obj &lt;&lt; Save Jackknife Predicted Values

**설명:** 잭나이프 예측값을 데이터 테이블의 새 열에 저장합니다.
...
**구문:** obj &lt;&lt; Save Variance Formula

**설명:** 분산 계산식을 데이터 테이블의 새 열에 저장합니다.
...
**구문:** obj &lt;&lt; Set Correlation Function( structure=가우시안 )

**설명:** 모형에 사용되는 상관 구조를 설정합니다.

&apos;가우시안&apos;은 두 반응 간의 상관계수를 0이 아닌 값으로 제한합니다.

&apos;3차&apos;는 충분히 떨어져 있는 점에 대해 상관계수 0을 허용합니다. 기본값은 "가우시안"입니다.

```jsl

dt = Open( "$SAMPLE_DATA/2D Gaussian Process Example.jmp" );obj = dt << Gaussian Process( Y( :Y ), X( :X1, :X2 ), Set Correlation Function( "Gaussian" ) );

```

### Set Minimum Theta

**구문:** obj &lt;&lt; Set Minimum Theta( number )

**설명:** 모형 적합에 사용되는 최소 세타 값을 설정합니다.
...
**구문:** obj &lt;&lt; Set Nugget( number )

**설명:** 추정 절차에서 능형 회귀 모수로 사용되는 너겟의 크기를 상수로 설정합니다.
