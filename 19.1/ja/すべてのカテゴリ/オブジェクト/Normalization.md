...
**構文:** obj = Normalization(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj &lt;&lt; X (Control) Columns( column(s) )
...
**説明:** 後続する分析のための前処理として、測定上のバイアスを調整する。
...
**構文:** Mean Expression Threshold ( number = 0.05 )

**説明:**  MAプロットのX軸(A値)に対する閾値。TMM法およびKDMM法に対するオプション。 デフォルトの値は"0.05"。
...
**構文:** Create New Tables for Results ( state =0|1 )
...
**構文:** Interpolated Quantiles( state =0|1 )

**説明:** 基準(reference)を決めるために分位点の内挿を行う。TMM法およびKDMM法に対するオプション。
...
**構文:** Log Transformation( state =0|1 )

**説明:** 対数変換を行う。TMM法およびKDMM法に対するオプション。
...
**構文:** Logratio Threshold ( number = 0.3 )

**説明:** MAプロットのY軸(M値)に対する閾値。TMM法およびKDMM法に対するオプション。 デフォルトの値は"0.3"。
...
**構文:** Method( "Row Standardize | Relative Abundance Percentage (RAP) | Relative Abundance Ratio (RAR) | Centered Log Ratio (CLR) | Count/Read Per Million (CPM/RPM) | Read/Fragment Per Kilobase of Transcript Per Million Mapped Reads/Fragments (RPKM/FPKM) | Transcript Per Million (TPM) | Trimmed Mean of M-Values (TMM) | Kernel Density Mean of M-Component (KDMM)" )
...
**構文:** Total Prior Count ( number = 0.25 )

**説明:** ライブラリサイズの調整のために全標本に事前度数を適用する 。TMM法およびKDMM法に対するオプション。 デフォルトの値は"0.25"。
...
**構文:** Replace Y Data( state =0|1 )

**説明:** データテーブルにおけるY変数のデータを変換したものに置換する。
