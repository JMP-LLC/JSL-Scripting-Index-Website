...
**構文:** obj = Predictor Screening(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj = Predictor Screening(...&lt;By( column(s) )&gt;...)
...
**構文:** obj = Predictor Screening(...Response( column(s) )...)
...
**構文:** obj = Predictor Screening(...X( column(s) )...)
...
**構文:** obj = Predictor Screening(...Y( column(s) )...)
...
**構文:** obj = Predictor Screening(...Number Trees( number=100 )...)

**説明:** ブートストラップ森モデルに含まれるデシジョンツリーの本数を指定する。 デフォルトの値は"100"。
...
**説明:** レポートの説明変数の順位表を、新しいデータテーブルに保存する。
...
**構文:** obj = Predictor Screening(...Set Random Seed( number )...)

**説明:** モデルの学習で用いる開始値のシード値を設定する。
