...
#### 工程のスクリーニング Xbar-R管理図によるスクリーニング
...
#### 工程のスクリーニング XBar-S管理図によるスクリーニング
...
#### 工程のスクリーニング 別データに保存された仕様限界の読み込み
...
#### 工程のスクリーニング 連続尺度の非負値データに対する環境モニタリング
...
**説明:** Holt法(2乗指数平滑化モデル)でドリフトの検出に使用される重みを指定する。 デフォルトの値は".05"。
...
**説明:** 後で[シフトテーブルの保存](Save Shift Table)オプションを使ってデータテーブルに保存できるように、すべてのシフト情報を保管する。このオプションは、起動スクリプトで指定しておく必要がある。
...
**説明:** 度数や非負連続値に対する管理図の場合に、表における「最大値」列の表示/非表示を切り替える。 デフォルトではオン。
...
**説明:** [工程のスクリーニング]プラットフォームを再起動して、元のレポートで選択された工程のみを使って新しいレポートを作成する。
...
**説明:** シフト情報を新しいデータテーブルに保存する。このコマンドを実行するには、起動スクリプトで[詳細なシフトデータの作成](Make Detailed Shift Data)オプションを指定しておく必要がある。
...
**説明:** 要約表において、処置基準値に関する列の表示/非表示を切り替える。この処置基準値に基づく警告は、少なくとも1つの点が上側処置基準値より大きい場合、または下側処置基準値より小さい場合に発せられる。 デフォルトではオン。
...
**説明:** 要約表において警報基準値に関する列の表示/非表示を切り替える。警報基準値に基づく警告は、少なくとも1つの点が上側警報基準値より大きい場合、または下側警報基準値より小さい場合に発せられる。
...
**説明:** 要約表における「増加警報」列の表示/非表示を切り替える。この列には、工程が増加していて直前の点が上側警報基準値を上回っているケース、または工程が減少していて直前の点が下側警報基準を下回っているケースの度数が表示される。
...
**説明:** 割合の管理図(p管理図)において、Kシグマの下側管理限界を使用するかどうかを指定する。このオプションは、割合の管理図でのみ使用できる。
...
**説明:** 割合の管理図(p管理図)において、Kシグマの上側管理限界を使用するかどうかを指定する。このオプションは、割合の管理図でのみ使用できる。 デフォルトではオン。
