...
**構文:** obj &lt;&lt; Error StdDev( Set Error StdDev(colume name(value),...) )

**説明:** 応答変数の誤差標準偏差を設定する。ここで設定した値は、シミュレーションにて応答変数の誤差を生成するのに使われる。
...
**説明:** 連続尺度の因子を指定された値でロックする。このロックは一時的なものである。
...
**説明:** 影響比の表示/非表示を切り替える。影響比は、因子の1つずつにおいて、その因子値を中間値からその因子の仕様限界値へと変更した場合の予測値の変化量を分子に、中間点での予測値と応答変数の仕様限界との差を分子にして計算された指標である。
