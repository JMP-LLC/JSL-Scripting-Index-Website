...
**構文:** obj &lt;&lt; Add Text( "string" )

**説明:** ピン留めされたホバーラベルにテキストのカスタムブロックを追加する。
...
**構文:** obj &lt;&lt; Click Link( "&lt;index, integer&gt;" )

**説明:** 特定のテキストまたはインデックスに関連付けられているリンクに対して、クリックアクションをシミュレートする。何も指定しない場合は、最後のリンク(通常はグラフレット)が想定される。
...
**説明:** ホバーラベルの内容をクリップボードにコピーする。

左辺に変数を指定している場合、ホバーラベルのテキストがその変数に保存される。
...
**構文:** obj &lt;&lt; Filled( state=0|1 )

### Font

**構文:** obj &lt;&lt; Font

**説明:** ピン留めされたホバーラベルのフォントを設定する。
...
**説明:** ホバーラベルのクリック可能なエントリに関連付けられているテキストのリストを戻す。

イメージとグラフレットは空白の文字列に関連付けられる。
...
**構文:** Associative Array = obj &lt;&lt; Get Local Context

**説明:** ホバーラベル実行コンテキストの一部として指定されているローカル変数の名前と値の連想配列を戻す。

使用例については、グラフレットのプリセット定義を参照。
...
**構文:** obj &lt;&lt; Launch graphlet

**説明:** グラフレットのグラフまたはプラットフォームを新しいウィンドウで起動する。

[元に戻す]コマンドで起動したウィンドウ/前の内容を復元することができる。

グラフレットのサムネールをクリックしても同じ結果になる。
...
**構文:** obj &lt;&lt; Launch in last

**説明:** グラフレットのグラフまたはプラットフォームを新しいウィンドウで起動するか、最後に起動したウィンドウの内容に代えて起動する。

[元に戻す]コマンドで起動したウィンドウ/前の内容を復元することができる。

Altキーを押しながらグラフレットのサムネールをクリックしても同じ結果になる。
...
**構文:** obj &lt;&lt; Launch in place

**説明:** グラフレットのグラフまたはプラットフォームを現在のウィンドウで起動する。

[元に戻す]コマンドで起動したウィンドウ/前の内容を復元することができる。

Ctrlキーを押しながらグラフレットのサムネールをクリックしても同じ結果になる。
...
**構文:** obj &lt;&lt; Replace Text( "string" )

**説明:** ピン留めされたホバーラベルのテキストを、指定したテキストで置換する。
...
**説明:** ピン留めされたホバーラベルのテキストを元のテキストに戻す。
...
**説明:** ローカルデータフィルタを適用してデータテーブルを起動し、ホバーラベルにあるグラフ要素に関連付けられているサブセットのデータのみを表示する。
