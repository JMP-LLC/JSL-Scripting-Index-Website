...
**説明:** 他のディスプレイボックスをグループ化するディスプレイボックスを戻す。
...
**構文:** obj &lt;&lt; Add All Open Files( &lt;Menu(0,1)&gt; )

**説明:** アウトラインボックスに開いて入るファイルすべてを追加する。
...
**説明:** OutlineBoxのキーと値のペアで、TableBoxの[連結したデータテーブルの作成]コマンドのための列を追加する。

**JMP追加されたバージョン:** 16

### Add Directory of Files

**構文:** obj &lt;&lt; Add Directory of Files

**説明:** アウトラインボックスにディレクトリを追加するためのウィンドウを開く。
...
**構文:** obj &lt;&lt; Add File Reference

**説明:** アウトラインボックスにファイル参照を追加するためのウィンドウを開く。
...
**構文:** obj &lt;&lt; Add Open Files( &lt;Menu(0,1)&gt; )
...
**構文:** obj &lt;&lt; Add Outline Item

**説明:** アウトラインボックスにアウトライン項目を追加するためのウィンドウを開く。
...
**構文:** obj &lt;&lt; Add Script Button

**説明:** アウトラインボックスにスクリプトボタンを追加するためのウィンドウを開く。
...
**構文:** obj &lt;&lt; Add Text Item

**説明:** アウトラインボックスにテキストアイテムを追加するためのウィンドウを開く。
...
**構文:** obj &lt;&lt; Add URL Reference

**説明:** アウトラインボックスにURLを追加するためのウィンドウを開く。
...
**構文:** obj &lt;&lt; Add Window Reference

**説明:** アウトラインボックスにウィンドウ参照を追加するためのウィンドウを開く。
...
**構文:** obj &lt;&lt; Close All Like This

**説明:** このアウトラインボックスと、ディスプレイツリー内で同じタイトルまたはヘルプキーを持つすべてのアウトラインボックスを閉じる。
...
**構文:** obj &lt;&lt; Close Where No Outlines
...
**説明:** スクリプトボタンで定義されているファイルのリンクを編集する。

**JMP追加されたバージョン:** 16

### Get Allow Title Wrapping

**構文:** 0|1 = obj &lt;&lt; Get Allow Title Wrapping( state=0|1 )
...
**構文:** obj &lt;&lt; Get Close Orientation

**説明:** アウトラインボックスを折りたたむ向きの設定を取得する。
...
**構文:** obj &lt;&lt; Get Image Export Display

**説明:** イメージの保存時にアウトラインテキストの書き出しを抑制するオプションの設定を取得する。
...
**構文:** 0|1|-1 = obj &lt;&lt; Get Menu Item State( index )

**説明:** indexポップアップメニューの項目の状態を取得する。状態は、通常(0)、選択されている(1)、選択不可(-1)のいずれか。
...
**構文:** list = obj &lt;&lt; Get Menu Script

**説明:** オブジェクトに指定されているメニュースクリプトを戻す。
...
**構文:** scriptable object = obj &lt;&lt; Get Scriptable Object

**説明:** オブジェクトのスクリプト可能オブジェクトへの参照を戻す。
...
**構文:** "string" = obj &lt;&lt; Get Title

**説明:** アウトラインボックスのタイトルを取得する。
...
**構文:** obj &lt;&lt; Title Background Color( color ); color = obj &lt;&lt; Get Title Background Color

**説明:** 背景色が指定されている場合、アウトラインタイトルは中身が描画される前に背景色で塗りつぶされる。背景色が「None」に指定されている場合、中に含まれているボックスの背景と中身が透けて表示される。
...
**構文:** obj &lt;&lt; Title Border( sides ); sides = obj &lt;&lt; Get Title Border

**説明:** タイトルの境界線はアウトラインのタイトルの外側を囲む実線。値を1つだけ指定した場合は、それがすべての辺に適用される。値を2つ指定した場合、それぞれ横と縦の境界線に適用される。タイトルの境界線は、タイトルの向きに応じて描画される。縦に閉じたアウトラインでは、境界線と中身が回転する。
...
**構文:** obj &lt;&lt; Title Border Color( color ); color = obj &lt;&lt; Get Title Border Color

**説明:** アウトラインタイトルの境界線の色をデフォルトのものから変更する。
...
**構文:** obj &lt;&lt; Title Padding( sides ); sides = obj &lt;&lt; Get Title Padding

**説明:** タイトルのパディングは、タイトルの中身と境界線の間にスペースを追加する。値を1つだけ指定した場合は、それがすべての辺に適用される。値を2つ指定した場合、それぞれ横と縦のパディングに適用される。タイトルのパディングは、タイトルの向きに応じて行われる。縦に閉じたアウトラインでは、パディングと中身が回転する。
...
**構文:** obj &lt;&lt; Title Text Color( color ); color = obj &lt;&lt; Get Title Text Color

**説明:** アウトラインタイトルのテキストの色が指定された場合、テキストはその色で描画される。指定されていない場合、タイトルは中に含まれているボックスのテキストの色を継承する。
...
**構文:** obj &lt;&lt; GetHorizontal

**説明:** アウトラインボックスの子が横方向に並んでいる場合は1、そうでない場合は0を戻す。
...
**説明:** アウトラインボックスが開いている場合は1、閉じている場合は0を戻す。
...
**構文:** obj &lt;&lt; Horizontal( bool )

**説明:** アウトラインボックスの子を横に並べる。
...
**構文:** obj &lt;&lt; Image Export Display( "通常"|"テキストのみ"|"なし" )

**説明:** イメージの保存時にアウトラインテキストの書き出しを抑制するオプション。
...
**説明:** アウトラインボックスがジャーナルの一部である場合、ジャーナルのロックを解除して、編集を可能にする。ジャーナルは余白部分を右クリックするとロックされる。

**JMP追加されたバージョン:** 16

### Lock Title

**構文:** obj &lt;&lt; Lock Title( state=0|1 )

**説明:** アウトラインのタイトルをロックして、インタラクティブな変更を行えないようにする。
...
**構文:** obj &lt;&lt; Open All Like This

**説明:** このアウトラインボックスと、ディスプレイツリー内の同じタイトルまたはヘルプキーを持つすべてのアウトラインボックスを開く。
...
**構文:** obj &lt;&lt; Set Allow Title Wrapping( 0|1 )
...
**構文:** obj &lt;&lt; Set Menu Item State( index, 0|1|-1 )

**説明:** indexで指定されたポップアップメニュー項目の状態を、通常(0)、選択されている(1)または選択不可(-1)に設定する。
...
**構文:** obj &lt;&lt; Set Menu Script( {"string",script,"string",script, ...} )

**説明:** 赤い三角ボタンをクリックしたときに開くメニューに項目を追加する。
...
**構文:** obj &lt;&lt; Set Scriptable Object( &lt;scriptable object&gt; )

**説明:** Outline Box(アウトラインボックス)にスクリプト可能オブジェクトを関連付ける。実行すると、スクリプト可能オブジェクトに対するメッセージを含む赤い三角ボタンメニューが、Outline Boxに表示される。なお、引数が指定されない場合は、スクリプト可能オブジェクトがクリアされる。
...
**構文:** obj &lt;&lt; Set Submenu( index, submenu count )
...
**構文:** obj &lt;&lt; Set Title( "string", &lt;Run Script(0 | 1)&gt; )

**説明:** アウトラインボックスのタイトルを変更する。
...
**構文:** obj &lt;&lt; Set Title Changed

**説明:** アウトラインのタイトルが変更されたときに実行されるスクリプトを設定する。
...
**構文:** obj &lt;&lt; SetHorizontal( state=0|1 )
