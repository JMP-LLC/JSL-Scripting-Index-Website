...
**説明:** 楕円の弧を描画する。角度は、時計の12時の方向を0度、3時の方向を90度とし、度を単位として指定する。sin()やcos()で使用されるラジアンの値に合わせるには、回転を逆にして90度の位相シフトを加える。たとえば、radians = 2 \* pi() \* (90 - degrees) / 360。円弧は、時計回りに始点から終点まで連続的に描画される。
