...
**语法:** Learning Rate( fraction )

**说明:** 设置估计中使用的学习率。默认值为 0.1。 默认为“.1”。
...
**语法:** Number of Layers( number )

**说明:** 设置估计中使用的层数。默认值为 50。 默认为“100”。
...
**语法:** Overfit Penalty( fraction )

**说明:** 设置过拟合惩罚，它引入偏倚，对带有分类响应的模型将概率移离零。默认值为 0.0001。
...
**说明:** Guarda el R cuadrado de validación junto con el número de árbol en una nueva tabla de datos. Solo disponible al utilizar un conjunto de validación.
...
**语法:** Splits Per Tree( number )

**说明:** 设置估计中使用的每个树的拆分数。默认值为 3。 默认为“3”。
...
**说明:** 运行时使用的调节参数表，支持: 每树拆分数、学习率、行抽样率、列抽样率、层数、最小拆分大小

### Use Excluded Rows for Validation

**语法:** obj = Boosted Tree(...Use Excluded Rows for Validation( state=0|1 )...)
