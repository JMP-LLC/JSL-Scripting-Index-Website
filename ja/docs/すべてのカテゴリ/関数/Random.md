...
**構文:** points = Random Linearly Constrained Uniform( numSamples, A, b, L, U, neq, nle, nge, &lt;nwarm=200&gt;, &lt;nstride=25&gt;, &lt;tol=1e-8&gt;, &lt;G&gt;, &lt;LC&gt;, &lt;UC&gt; )

**説明:** 線形制約や各変数に対する範囲制約を課した一様乱数を生成する。また、変数の部分集合ごとに要素数制約を課すこともできる。引数numSamplesは生成される乱数の個数。引数Aは線形制約式の係数を表す行列。引数bは線形制約式の右辺値のベクトル。引数LおよびUは、それぞれ変数の下限および上限のベクトル。引数neqは等号制約式の数。引数nleは「以下」を示す不等号制約式の数。引数ngeは「以上」を示す不等号制約式の数。引数nwarmは点を結果に出力する前のウォームアップの反復回数。引数nstrideは結果に出力する際の間隔(出力する点と点の間の反復回数)。tol引数は許容範囲。G引数は要素数制約を行う場合に変数を部分集合に分割する通し番号のベクトルであり、要素数制約を課したくない変数には欠測値またはゼロを指定する。引数LCおよびUCは、要素数制約における各部分集合での要素数の上限および下限。なお、線形制約は、等号制約、「以下」を示す不等号制約、「以上」を示す不等号制約の順に指定すること。
