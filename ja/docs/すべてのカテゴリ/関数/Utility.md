...
**説明:** アドインを登録する。AutoLoadの値を1に設定すると、登録時に必ずアドインがロードされる。0に設定すると、アドインはロードされない。AutoLoadを指定しなかった場合は、addin.defの設定が使われるか、設定が見つからない場合はデフォルトでアドインがロードされる。
...
**説明:** Sobol列を用いてSpace Fillingの一連の疑似乱数(最大21.201次元)を生成する。
