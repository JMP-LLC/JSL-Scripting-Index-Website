...
**構文:** result = jmp.from_dataframe(&lt;library.Dataframe&gt;, allow_copy=&lt;boolean&gt;, allow_csv_fallback=&lt;boolean&gt;, visibility=&lt;string&gt;)
...
**構文:** result = jmp.from_dataframe_using_csv(&lt;library.Dataframe&gt;, visibility=&lt;string&gt;)
...
**説明:** 主にオブジェクトへの参照を消去することで、共有されたPython環境をリセットする。読み込まれたモジュールの読み込みキャッシュには変化がない。これはPython環境自体の限界である。共有ライブラリをロードするモジュールを、実行中のプロセスによってアンロードすることはできない。純粋なPythonコードを再ロードする方法については、Python.orgのマニュアルでimportlib reload()を参照してください。
...
**説明:** サブクラスがこの関数をオーバーライドして接続を作成する。jmp.DataSourceのサブクラス形式で戻される接続は、クエリビルダーとのインテグレーションを可能にする。



戻されたデータソース内で自身のコネクタインスタンスの参照を使用しないでください。他のコードも参照を保持していて、データソースの設定値が置き換えられてしまう可能性があります。copy.copyを使用して"self_copy = copy.copy(self)"のように独立したコピーを作成することを検討してください。
...
**説明:** サブクラスは、この関数をオーバーライドしてテーブルを直接開くことができる。この関数は、JSLの Openメッセージ(New Data Connector(...) << Open())を実装する。



この関数と_do_as_data_sourceの両方が実装されていれば、どちらを使っても同じテーブルを開くことができる。
...
**説明:** DataConnectorオブジェクトは、標準のcopy.copy関数を使ったシャローコピーをサポートする。



サブクラスでコピー動作を変更することはできず、サブクラスがステートを追加してもコピーされないため、追加すべきではありません。
...
**説明:** jmp.DataConnectorTypeサブクラスとjmp.DataConnectorサブクラスを関連付けて結び付け（Tie）、両クラスの定義を完成させる。これにより、jmp.DataConnectorサブクラスがjmp.DataConnectorTypeサブクラスとそのフィールド定義にアクセスできるようになる。また、jmp.DataConnectorTypeサブクラスが、指定されたサブクラスを使用してjmp.DataConnectorのインスタンスを作成できるようになる。



この関数は、サブクラスで属性を設定することで関連付けを行う。
...
**説明:** DataConnectorFieldは、フィールドを定義する。
...
type: intなど、フィールドの種類 (詳細については、jmp.DataConnectorType.fieldsを参照)。



default: 42など、フィールドのデフォルト値。指定しなかった場合、デフォルト値はtypeによって決まる。typeが"int | None"の場合はNoneで、それ以外の場合は、その種類の"empty"値、つまり、strなら""、intなら0、boolならFalse。



tooltip: None以外の値を指定した場合、それがデータコネクタエディタ内でフィールドのツールヒントとして使用される。



ui_name: None以外の値を指定した場合、それがデータコネクタエディタ内で実際のフィールド名に代わってフィールド名として使用される。



タイプが"str"または"str | None"の場合の追加の引数:



credential: None以外の値を指定した場合は、このフィールドによって保管されるログイン情報。有効な値は"username"と"password"。JMPはこの情報を使ってプレースホルダーのサポートを可能にする。



sensitive: trueの場合、このフィールドは機密情報を含むとみなされるため、値がプレーンテキストで書き出されることはない。デフォルトでは、"credential"が設定されている場合にtrue、それ以外の場合はfalseとなる。



mask_input: trueの場合、値は隠され、データコネクタエディタ内で黒丸で表示される。デフォルトでは、"credential"が"password"の場合、または"sensitive"がtrueで"credential"が設定されていない場合にtrueとなる。
...
**説明:** jmp.DataConnectorGroupedFieldsは、指定された名前のグループでフィールドを定義する。グループは、ペアのリストとして渡される。各ペアの1つ目の要素はグループの名前で、2つ目はフィールドを含むディクショナリ。このディクショナリは、jmp.DataConnectorType.fieldsの値として直接使用されるものと同じ形式を持つ。



グループの使用は、データコネクタエディタ内でのフィールドの表示に影響するが、プログラムによるアクセスなどには影響しない。また、グループ名は、UIのみで使用されるため、JSL名で使用が許可されていない文字を含んでいても問題ない。
...
**説明:** サブクラスは、そのタイプの設定オプションを指定する、"fields”という名前のクラスレベルの変数を定義しなければならない。dictまたはjmp.DataConnectorGroupedFieldsオブジェクトを指定する。



dictのキーは、設定オプションの名前であり、エディタに表示され、JSLでも使用できる。JSL識別子の形式で指定しなければならない。各キーに対応する値は、フィールドの種類(str、int、jmp.DataConnectorFieldオブジェクトなど)。



サポートされる種類: bool、int、str、およびこれらの任意のバージョン(typing.Optional[bool]、bool | None for boolなど)。欠測値はNoneで表される。
...
**説明:** サブクラスは、この関数をオーバーライドしてデータソース内のスキーマのリストを指定することができる。この関数がオーバーライドされなかった場合、またはNoneを戻した場合、データソースは、スキーマをサポートしていないとみなされる。
...
**説明:** サブクラスは、この関数をオーバーライドしてデータソース内のテーブルのリストを指定する。スキーマがサポートされている場合は、渡されたスキーマのテーブルのみを含む。スキーマがサポートされていない場合は、schemaパラメータを無視する。
...
**説明:** サブクラスは、この関数をオーバーライドして名前が指定されたテーブルのテーブルデータを取得する。スキーマがサポートされていない場合は、schema引数の値は無視される。この関数は、jmp.DataTable、またはJMPでサポートされているデータ形式のファイルへのパスを含んだ文字列を戻す。



データテーブルを直接戻す場合は、テーブルをプライベートに作成する。パスの文字列を戻す場合は、JMPがテーブルを開いて使用された設定をキャプチャし、クエリービルダースクリプトの中に保持する。
...
**構文:** col_obj.display_width col_obj.display_width = &lt;int&gt;
...
**構文:** col_obj.format col_obj.format = tuple&lt; &lt;string&gt;|&lt;int&gt;, ... &gt;
...
**構文:** col_obj.formula col_obj.formula = &lt;Expression | string&gt;
...
**構文:** col_obj.name col_obj.name = &lt;string&gt;
...
**構文:** dt.begin_update() # added JMP 19.1

**説明:** Small tables update rapidly, but for large tables, having to update the user interface while making mass changes to a column is very time consuming. The begin_update() method stops the GUI updates to the data table until a corresponding end_update().
...
**構文:** dt.cell_height dt.cell_height = &lt;int&gt;
...
**構文:** dt.end_update() # added JMP 19.1

**説明:** Restores the data table GUI update processing.
...
**構文:** dt.row_states dt.row_states = [state1, state2, ..., stateN]

**説明:** データテーブルのすべての行に行属性を設定する。

**JMP追加されたバージョン:** 19

**タプルのリスト(<index>, <RowState | int>)**
...
**構文:** dt.scripts dt.scripts = &lt;dict&lt;str : &lt;str | Expression&gt;&gt;&gt;
...
**構文:** expr_obj.jsl expr_obj.jsl = &lt;string&gt;
...
**構文:** rs_obj.color rs_obj.color = &lt;int&gt;
...
**構文:** rs_obj.excluded rs_obj.excluded = &lt;boolean&gt;
...
**構文:** rs_obj.hidden rs_obj.hidden = &lt;boolean&gt;
...
**構文:** rs_obj.labeled rs_obj.labeled = &lt;boolean&gt;
...
**構文:** rs_obj.marker rs_obj.marker = &lt;int&gt;
...
**構文:** rs_obj.selected rs_obj.selected = &lt;boolean&gt;
...
**説明:** PythonのスコーピングとPython Get()の設計により、Pythonグローバルスコープにある変数のみアクセス可能。jmp.globalsディクショナリを使うと、Pythonコードから直接JSL変数を設定または作成することができる。Python Get()で現在サポートされているすべてのPythonタイプの値をサポートしている。
...
**説明:** 現在の&apos;here&apos;名前空間からJSL変数をPythonオブジェクトとして取得する。オブジェクトが見つからない場合や、データタイプが不透明で処理できない場合は、None(なし)を戻す。JSL変数をローカルのPythonスコープに直接取り込むことができる。Python Send()と同じタイプのオブジェクトを転送できる。JSLの名前空間スコーピングでは、&apos;here&apos;名前空間はスクリプトの実行ごとに固有で、異なるスクリプトウィンドウから送信されたコードは別の&apos;here&apos;名前空間を持つ。別のスクリプトをincludeするコードは、単一の&apos;here&apos;名前空間を持つ。この共有名前空間は、Submit()またはExecute()を介してPythonを実行するJSLスクリプトや、run_jsl()を呼び出すPythonスクリプトに対しても存在する。
...
**説明:** JSLスクリプトの&apos;here&apos;名前空間に値を設定する手段を提供する。これにより、Python Get()ではできなかった値を送り返すことが可能になる。Python Get()は、Pythonグローバルスコープにある変数しか見ることができないため。Python Get()と同じオブジェクトタイプをサポートする。JSL名前空間スコーピングでは、&apos;here&apos;名前空間はスクリプトの実行ごとに固有で、異なるスクリプトウィンドウから送信されたコードは別の&apos;here&apos;名前空間を持つ。別のスクリプトをincludeするコードは、単一の&apos;here&apos;名前空間を持つ。これは、Submit()またはExecute()を介してPythonを実行するJSLスクリプトや、run_jsl()を呼び出すPythonスクリプトでも同じ。
...
**説明:** jmpexパッケージにはJMPでサポートされる拡張インターフェースが含まれる。現在は、Rサポートモジュールのみが含まれる。
...
**構文:** success = is_installed(package_name)

**説明:** package_nameの名前を持つパッケージがインストールされている場合はTrue、インストールされていない場合はFalseを戻す。
...
**構文:** result = jpip( &apos;pip_cmd&apos;, packages=&apos;&apos;, echo=True )

**説明:** JMP内から呼び出し可能な、Pythonのpipコマンドのラッパーとなっている関数。subprocess.CompletedProcessオブジェクトを戻す。この関数にpipコマンド引数を文字列として渡す。オプションで、インストールする複数のパッケージをスペースで区切った文字列で指定できる。より確実な方法として、各引数を含んだリストを使用することもできる。packages引数のデフォルト値は空の文字列。&apos;jmputils&apos;パッケージは、JMPに組み込まれたPython標準ライブラリに含まれる。jmputils内の関数は、Pythonの標準ライブラリの呼び出しまたはJMPのビルトイン機能のみを使用する。
...
**構文:** package_version(package_name)

**説明:** 名前を指定したパッケージ(がインストールされている場合は、その)バージョン番号。
...
**構文:** pkgs_dict = packages()

**説明:** インストールされたパッケージのリストを含むPythonディクショナリを戻す。
