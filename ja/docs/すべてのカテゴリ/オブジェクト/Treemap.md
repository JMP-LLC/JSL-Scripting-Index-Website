...
**構文:** obj = Treemap(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** Treemap( Categories( column ), <Sizes( column )>, <Coloring( column )> )

**説明:** 名義尺度または順序尺度の変数を長方形のタイルで表したプロット。カテゴリを示すタイルの大きさは、選択された変数におけるカテゴリの度数に比例する。グラフビルダーでも作成可能。
...
**構文:** obj << Change Color Column( column )

**説明:** カテゴリの色分けに使用する列を変更する。
...
**構文:** obj << Color Range( low, high )

**説明:** グラデーションの範囲を設定する。デフォルトでは、色分け列の最小値がlow、最大値がhighとなる。
...
**構文:** obj = Treemap(...Layout( "分割"|"長方形分割"|"混合" )...)

### Legend

**構文:** obj << Legend( state=0|1 )

**説明:** 色と列の値との関係を示す凡例の表示/非表示を切り替える。
