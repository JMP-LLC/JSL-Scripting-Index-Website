...
**構文:** obj = Latent Class Analysis(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj = Latent Class Analysis(...&lt;By( column(s) )&gt;...)
...
**構文:** obj = Latent Class Analysis(...&lt;Freq( column )&gt;...)
...
**構文:** obj = Latent Class Analysis(...&lt;ID( column )&gt;...)
...
**構文:** obj = Latent Class Analysis(...&lt;Weight( column )&gt;...)
...
**構文:** obj = Latent Class Analysis(...Y( column(s) )...)
...
**構文:** obj &lt;&lt; New Number of Clusters( number )

**説明:** クラスターの数を変えて、もう一度分析を実行する。
...
**構文:** obj = Latent Class Analysis(...Number of Clusters( number=3 )...)

**説明:** 潜在クラスターの個数を指定する。 デフォルトの値は"3"。
...
**構文:** obj = Latent Class Analysis(...Up to( number )...)

**説明:** クラスターの最大個数を指定する。クラスター数が「クラスターの数」以上で「最大個数」以下までの、複数のモデルがあてはめられる。
...
**構文:** obj&lt;&lt;(Fit[number] &lt;&lt; Color By Cluster)

**説明:** 最も確率の高いクラスターにデータテーブルの行を色付けする。
...
**構文:** obj &lt;&lt; ( fit[number] &lt;&lt; Effect Sizes( state=0|1 ) )

**説明:** 「効果の大きさ」表の表示/非表示を切り替える。 デフォルトではオン。
...
**構文:** obj &lt;&lt; (Fit[number] &lt;&lt; Get Probability Formulas)
...
**構文:** obj &lt;&lt; ( fit[number] &lt;&lt; MDS Plot( state=0|1 ) )

**説明:** 「多次元尺度構成プロット」（MDSプロット）の表示/非表示を切り替える。「多次元尺度構成プロット」は、クラスターの類似度を2次元で表現したグラフ。 デフォルトではオン。
...
**構文:** obj &lt;&lt; ( fit[number] &lt;&lt; Mixture Probabilities( state=0|1 ) )

**説明:** 「混合確率」表の表示/非表示を切り替える。
...
**説明:** 「モデルの要約」表の表示/非表示を切り替える。この表には、(-1)\*対数尤度、パラメータ数、BIC、AICが含まれている。 デフォルトではオン。
...
**構文:** obj &lt;&lt; ( fit[number] &lt;&lt; Parameter Estimates( state=0|1 ) )

**説明:** 「パラメータ推定値」レポートの表示/非表示を切り替える。レポートには、パラメータ推定値の表とグラフが含まれている。 デフォルトではオン。
...
**構文:** obj &lt;&lt; (Fit[number] &lt;&lt; Publish Probability Formulas)
...
**構文:** obj &lt;&lt; (Fit[number]&lt;&lt;Rename Clusters( {argument list} )

**説明:** レポート内のクラスター名を変更する。分析者が理解しやすいクラスター名に変更できる。引数を指定しなかった場合、クラスター名を指定するためのダイアログが呼び出される。
...
**構文:** obj &lt;&lt; (Fit[number] &lt;&lt; Save Cluster Formula Only)
...
**構文:** obj &lt;&lt; (Fit[number] &lt;&lt; Save Cluster Only)

**説明:** データテーブルに、所属する確率が最も大きいクラスターを含んだ列を保存する。
...
**構文:** obj &lt;&lt; (Fit[number] &lt;&lt; Save Mixture Probabilities)
...
**構文:** obj &lt;&lt; (Fit[number] &lt;&lt; Save Mixture and Cluster Formulas)
...
**構文:** obj &lt;&lt; ( fit[number] &lt;&lt; Transposed Parameter Estimates( state=0|1 ) )

**説明:** 「転置したパラメータ推定値」表の表示/非表示を切り替える。
