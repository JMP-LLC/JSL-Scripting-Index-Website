...
**説明:** MAプロットにおけるX軸(A値)の閾値 デフォルトの値は"0.05"。
...
**説明:** Create New Tables for Results
...
**説明:** TMM正規化において基準値を求める際、分位点を内挿する。
...
**説明:** Log Transformation of TMM Method
...
**説明:** MAプロットにおけるY軸(M値)の閾値 デフォルトの値は"0.3"。
...
**構文:** Method( "Row Standardize" | "Wrench" )

**説明:** Specifies the normalization method.
...
**説明:** ライブラリのサイズを調整するための合計事前度数。 デフォルトの値は"0.25"。
