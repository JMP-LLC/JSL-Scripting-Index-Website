...
**構文:** obj = EWMA Control Chart(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj = EWMA Control Chart(...Y( column(s) )...)
...
**構文:** obj = EWMA Control Chart(...Center Data( state=0|1 )...)

**説明:** 各観測値から目標値を引いてデータを中心化する。
...
in Columnを指定すると、平均が管理限界列プロパティのEWMAの項目に保存される。



in New Tableを指定すると、各管理図のLambda（または重み）、標準偏差、平均が新しいデータテーブルに保存される。
...
**構文:** obj = EWMA Control Chart(...Show Excluded Region( state=0|1 )...)

**説明:** サブグループ軸に除外されている領域を表示する。サブグループ変数のみを対象とする。
