...
**構文:** obj = Multidimensional Scaling(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**説明:** 三次元プロットの表示/非表示を切り替える。このプロットは、最低3つの次元が必要。
...
**構文:** Data Format( Distance Matrix | Attribute List )

**説明:** データテーブルの形式が、距離行列なのか、属性の元データなのかを指定する。
...
**説明:** 多次元尺度構成プロットの表示/非表示を切り替える。 デフォルトではオン。
...
**構文:** obj &lt;&lt; Save Proximity

**説明:** 類似度行列をデータテーブルに保存する。データの形式が「属性の元データ」の場合にのみ使用可能。
...
**構文:** obj = Multidimensional Scaling(...Set Dimensions( number=2 )...)

**説明:** 多次元尺度構成法の次元を指定する。 デフォルトの値は"2"。
...
**構文:** obj &lt;&lt; Shepard Diagram( state=0|1 )

**説明:** Shepard図の表示/非表示を切り替える。この図には、類似度の予測値と実測値がプロットされる。 デフォルトではオン。
...
**構文:** obj &lt;&lt; Show Coordinates( state=0|1 )
...
**構文:** obj &lt;&lt; Show Proximity( state=0|1 )

**説明:** 類似度の予測値と実測値を示す表の表示/非表示を切り替える。
...
**構文:** Transformation( None | Ratio | Interval | Ordinal )

**説明:** 多次元尺度構成法で使用する変換方法を指定する。
...
**構文:** obj &lt;&lt; Waern Links( Smallest Portion | Largest Portion, portion, visible )

**説明:** 多次元尺度構成プロットにおけるWaernリンクの描き方を指定する。第1引数は、最短距離と最長距離のどちらのリンクを描くかを指定する。第2引数は割合を指定する。第3引数はブール値で、Waernリンクの表示/非表示を指定する。
