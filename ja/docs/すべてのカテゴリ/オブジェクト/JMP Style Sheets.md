...
**説明:** JMP Style Sheets (JSS) helps us separate DisplayBox content from DisplayBox presentation. The system is modeled on CSS. CSS is to HTML as JSS is to DisplayBox trees in JMP.
...
**構文:** And( selector, ..., selector )

**説明:** Combine two or more selectors, making a new selector that matches only when all of those match. Use the functional form And(...) or wrap in parentheses before sending messages.
...
**構文:** Child( parentSelector, childSelector )

**説明:** Combines two selectors into a direct parent/child path. A box matches when it and its direct parent both match their individual selectors.
...
**説明:** Matches boxes that have the given JSS class (loose comparison).
...
**構文:** Constants( var1 = expr1; var2 = expr2; ... )

**説明:** Defines constants that can be used within rule declarations. Basic operations are allowed in the expressions and the names can reference each other. Names are global within a Theme and the last declaration of a name is used.
...
**構文:** Descend( selector, ..., selector )

**説明:** Combines two or more selectors into a path. A box matches when the path to the box matches. Note that the path can skip levels in the display tree.
...
**説明:** Matches when a box is the first child of its parent.
...
**説明:** Matches boxes that have the given JSS id (loose comparison).
...
**構文:** If( condA, rulesA, ..., rulesElse )

**説明:** The allowable condition expressions are limited and resolved when the JSS is parsed. No branch testing is done during the JSS application passes. In other words, these are for testing the state of the system and JMP, not individual boxes or box trees.
...
**説明:** Matches when a box is the last child of its parent.
...
**構文:** Next Sibling( precedingSelector, selector )

**説明:** Box must match the second selector and be immediately preceded by a box matching the first selector (sharing the same parent).
...
**構文:** Or( selector, ..., selector )

**説明:** Combines two or more selectors, making a new selector that matches when any of the individual selectors match. Use the functional form Or(...) or wrap in parentheses before sending messages.
...
**説明:** Matches boxes of the given type (loose comparison).
...
**説明:** Matches all boxes. Useful mainly for clearly expressing intent.
