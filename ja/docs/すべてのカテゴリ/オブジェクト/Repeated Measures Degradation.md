...
**構文:** obj = Repeated Measures Degradation(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj << Reference Temperature

**説明:** 該当するモデルで使用される基準温度を設定する。
...
**構文:** obj << Save Posterior To Script( state=0|1 )

**説明:** 現在のレポートのスクリプトを保存する際に、スクリプトに事後分布から生成された標本データを含めることを指定する。保存されたスクリプトを実行すると、その事後分布標本データがスクリプトから［反復測定劣化］分析に直接読み込まれる。 デフォルトではオン。
