...
**構文:** obj = Destructive Degradation(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj << Control( yTrans, timeTrans, distribution, path definition )

**説明:** レポートの「経路の定義」パネルでの設定を指定する。選択したモデルは、先頭のグラフにも表示される。
...
**構文:** obj << Model( yTrans, timeTrans, distribution, path definition )

**説明:** あてはめるモデルに対する設定を指定し、あてはめた結果のレポートを表示する。
...
**構文:** obj << Shade( state=0|1 )

**説明:** 分位点区間の上限と下限の表示／非表示を切り替える。このオプションは、区間内の塗りつぶしの表示／非表示も切り替える。 デフォルトではオン。
...
**構文:** obj << Shade Coverage( proportion )
...
**構文:** obj << Use Condition Temperature( "Celsius"|"Fahrenheit"|"Kelvin", temperature )

**説明:** 1次反応速度モデルにおける使用条件の温度（X0）を指定する。
