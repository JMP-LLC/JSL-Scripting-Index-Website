...
**構文:** obj = Recurrence Analysis(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj &lt;&lt; Timestamp at End( column )
...
**構文:** obj &lt;&lt; Timestamp at Start( column )
...
**構文:** obj &lt;&lt; Calendar Event Plot( state=0|1 )

**説明:** 経過時間ではなく時刻に対してイベントをプロットしたグラフを表示または非表示にする。
...
**構文:** obj &lt;&lt; Event Plot( state=0|1 )

**説明:** イベントプロットの表示/非表示を切り替える。
...
**説明:** パラメトリックな再生性モデルをあてはめるためのアウトラインがウィンドウ内に呼び出される。そこで、強度関数もしくは累計関数に対するパラメトリックなモデルを指定できる。
...
**構文:** obj &lt;&lt; MCF Confid Limits( state=0|1 )

**説明:** MCFプロットにおいて、平均累計関数(MCF)の近似95%信頼区間を示す線の表示/非表示を切り替える。
...
**構文:** obj &lt;&lt; MCF Plot( state=0|1 )

**説明:** MCF(平均累計関数)プロットの表示/非表示を切り替える。 デフォルトではオン。
...
**構文:** obj &lt;&lt; MCF Plot Each Group( state=0|1 )

**説明:** グループ変数の各水準の平均累計関数(MCF)プロットを含むレポートの表示/非表示を切り替える。このオプションは、グループ変数を指定した場合のみ使用可能。
...
**構文:** obj &lt;&lt; Plot Interarrival by Age( state=0|1 )

**説明:** 最初からの時間(=経過時間)に対して、1つ前に発生した故障からの時間(=故障間隔時間)をプロットしたグラフを、表示または非表示にする。
...
**構文:** obj &lt;&lt; Plot MCF Differences( state=0|1 )

**説明:** 平均累計関数(MCF)の各差のプロットと95%信頼区間の表示/非表示を切り替える。このオプションは、グループ変数を指定した場合のみ使用可能。
...
**構文:** obj &lt;&lt; Save MCF Differences( &lt;Last&gt; )

**説明:** 平均累計関数(MCF)の差をグループ変数の水準ごとにまとめた新しいデータテーブルを作成する。オプションの引数によって、各差の最後の行だけを新しいデータテーブルに保存することもできる。このオプションは、グループ変数を指定した場合のみ使用可能。
...
**構文:** obj &lt;&lt; Show MCF Legend( state=0|1 )

**説明:** 平均累計関数(MCF）プロットの凡例の表示/非表示を切り替える。このオプションは、グループ変数を指定した場合のみ使用可能。 デフォルトではオン。
