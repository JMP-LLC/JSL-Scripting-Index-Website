...
**構文:** obj = Parallel Plot(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj << Grouping( columns )

**説明:** この列の値が同じである行をグループとしてグラフを作成する。
...
**構文:** obj << Response( columns )

**説明:** 各行の、すべての列の値を1本の線でつなぐ。
...
**構文:** Parallel Plot( Y( columns ), Center at Zero( state=0|1 ) )
...
**構文:** Parallel Plot( Y( columns ), Scale Uniformly( state=0|1 ) )

**説明:** すべてのプロットでスケールを統一する。
...
**構文:** obj << Show Reversing Checkboxes( state=0|1 )

**説明:** 各Y変数に対する、軸を反転するためのチェックボックスの表示/非表示を切り替える。
