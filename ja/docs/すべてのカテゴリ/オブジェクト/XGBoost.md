...
**説明:** 勾配ブースティングの手法として、XGBoostまたはLigthGBMを指定する。 デフォルトの値は"xgboost"。
...
**説明:** カテゴリ変数の扱いとして、ワンホットエンコーディングにするか、「1カテゴリ vs その他のカテゴリ」の分割にするかを切り替えるカテゴリ数を指定する。ここで指定されたカテゴリ数以下のカテゴリカル変数には、ワンホットエンコーディングが使われる。 デフォルトの値は"4"。
...
**説明:** 目的関数において乱数が使われる際に、その使われる乱数のシード値を指定する。 デフォルトの値は"5"。
...
**説明:** 予測早期打ち切りにおけるマージンの閾値を指定する。この閾値を指定すると、予測値を求める際に、もしマージンがこの閾値よりも大きい場合に予測早期打ち切りを行う。 デフォルトの値は"10"。
...
**構文:** obj &lt;&lt; top_k( number=0 )

**説明:** 貪欲な方法による特徴量選択において選択する上位の特徴量の個数。このオプションは、GBLinearブースターにのみ適用される。 デフォルトの値は"0"。
...
**説明:** DART アルゴリズムの除外において、均等な確率で木を除外するかどうかを指定する。 デフォルトの値は"0"。
