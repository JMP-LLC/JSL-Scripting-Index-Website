...
**構文:** Fit Group( model1, model2, ... );Fit Group( model1; model2; ... )

**説明:** 最小2乗、非線形、ニューラル、Gauss過程、および混合モデルといったモデルのあてはめを1つのウィンドウにまとめ、結果を1つにまとめたプロファイルを表示する。
...
**説明:** プラットフォームに作られた複数のレポートを、指定の行数で並べ替える。
...
**説明:** すべての応答変数に対する等高線プロファイルの表示/非表示を切り替える。すべての応答変数に対するモデル項の効果を同時に確認できる。
...
**構文:** obj &lt;&lt; Order by Goodness of Fit

**説明:** 複数のレポートを適合度の順に並べ替える。適合度の指標は、各モデルのR2乗統計量。このオプションは、プラットフォームレベルでR2乗統計量が計算されるプラットフォームのみに適用される。
...
**説明:** すべての応答変数に対する予測プロファイルの表示/非表示を切り替える。すべての応答変数に対するモデル項の効果を同時に確認できる。また、複数の応答変数において、多目的最適化を実行することもできる。
...
**説明:** 1つ1つの応答変数ごとに作成される個別の曲面プロファイルの表示/非表示を切り替える。
