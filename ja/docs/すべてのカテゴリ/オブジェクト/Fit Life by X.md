...
**構文:** obj = Fit Life by X(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj &lt;&lt; Add Density Curve to Scatterplot( number )

**説明:** 散布図の指定されたX変数の値の位置に密度曲線を追加する。凡例で選択されている確率分布ごとに密度曲線が描かれる。凡例は散布図の右側に表示されている。
...
**構文:** obj &lt;&lt; Add Quantile Line to Scatterplot( quantile )

**説明:** 散布図の指定された分位点の位置に曲線を追加する。凡例で選択されている確率分布ごとに分位点曲線が描かれる。凡例は散布図の右側に表示されている。
...
**構文:** obj = Fit Life by X(...Censor Code( value=1 )...)
...
**構文:** obj = Fit Life by X(...Confidence Interval Method( method="Wald" )...)

**説明:** パラメータの信頼区間を計算する方法を指定する。選択肢はWaldまたは尤度。Wald法は近似法で、計算時間が短い。尤度法は、より精確なパラメータを求めるため、計算に時間がかかる。 デフォルトの値は"Wald"。
...
**構文:** obj &lt;&lt; Density( Weibull|Lognormal|Loglogistic|Frechet| SEV|Normal|Logistic|LEV, t, x )

**説明:** 寿命t、共変量xにおける指定の分布の密度を戻す。
...
**構文:** obj = Fit Life by X(...Distribution( Weibull|Lognormal|Loglogistic|Frechet |SEV|Log|Normal|Logistic|LEV )...)

**説明:** X変数とY変数の関係をモデル化する際に仮定する確率分布を指定する。
...
**構文:** obj &lt;&lt; Hazard( Weibull|Lognormal|Loglogistic|Frechet| SEV|Normal|Logistic|LEV, t, x )

**説明:** 寿命t、共変量xにおける指定の分布のハザードを戻す。
...
**構文:** obj &lt;&lt; Nested Model Tests( Saturated Location|Location|Location and Scale|Saturated Location and Scale|Regression|No Effect )

**説明:** ノンパラメトリックな確率プロット、包含モデルの検定、複数群の確率プロットをレポートに追加する。
...
**構文:** obj &lt;&lt; Probability( Weibull|Lognormal|Loglogistic|Frechet| SEV|Normal|Logistic|LEV, t, x )

**説明:** 寿命t、共変量xにおける指定の分布の確率を戻す。
...
**構文:** obj &lt;&lt; Quantile( Weibull|Lognormal|Loglogistic|Frechet| SEV|Normal|Logistic|LEV, p, x )

**説明:** 確率p、共変量xにおける指定の分布の分位点を戻す。
...
**構文:** obj = Fit Life by X(...Relationship( Arrhenius Celsius|Arrhenius Fahrenheit|Arrhenius Kelvin|Inverse Power|Linear|Log|Logit|Reciprocal|Square Root|Box-Cox|Custom|No Effect|Location|Location and Scale )...)

**説明:** 寿命と因子の関係を示す式を指定する。
...
**構文:** obj &lt;&lt; Set Level of Quantile Line CI Bands( alpha=0.95 )

**説明:** 分位点曲線の信頼区間に対する信頼水準を指定する。
...
**構文:** obj &lt;&lt; Set Scale( Weibull|Lognormal|Loglogistic|Frechet|SEV |Normal|Logistic|LEV|Linear )

**説明:** ノンパラメトリック重ね合わせプロットに使うスケールを指定する。
...
**構文:** obj &lt;&lt; Set Scriptables( {&lt;Distribution Comparisons( options )&gt;, &lt;Quantile Comparisons( options )&gt;, &lt;Hazard Comparisons( options )&gt;, &lt;Density Comparisons( options )&gt;} )

**説明:** プロファイルに対するスクリプト可能なオプションを設定する。
...
**構文:** obj &lt;&lt; Show Density Curves( state=0|1 )
...
**構文:** obj &lt;&lt; Show Overlay by Levels( state=0|1 )

**説明:** 重ね合わせグラフの表示/非表示を切り替える。
...
**説明:** ノンパラメトリック重ね合わせプロットと複数群の確率プロットにおいて、データ点の表示/非表示を切り替える。点を非表示にした場合は、ステップ関数が表示される。 デフォルトではオン。
...
**構文:** obj &lt;&lt; Show Quantile Line CI Bands( state=0|1 )

**説明:** 分位点曲線の信頼区間の表示/非表示を切り替える。
...
**構文:** obj &lt;&lt; Show Surface Plot( state=0|1 )

**説明:** レポートの「個々の分布」結果セクションに曲面プロットを表示する。各分布の「分布」・「分位点」・「ハザード」・「密度」セクションに曲面プロットが表示される。
...
**構文:** obj &lt;&lt; TAF( Weibull|Lognormal|Loglogistic|Frechet, value, x )

**説明:** 指定の分布、加速条件x、使用条件()に対する時間加速係数を戻す。
...
**構文:** obj &lt;&lt; Tabbed Individual Report( state=0|1 )

**説明:** 個々のレポートをタブ形式で表示する。 デフォルトではオン。
...
**構文:** obj &lt;&lt; Tabbed Overall Report( state=0|1 )

**説明:** レポートのプロット・比較・結果の各セクションをタブ形式で表示する。
...
**構文:** obj &lt;&lt; Time Acceleration Baseline( number )
...
**構文:** obj &lt;&lt; Transposed Axes( state=0|1 )

**説明:** 加速因子を横軸ではなく縦軸に表示する
...
**構文:** obj &lt;&lt; Use Transformation Scale( state=0|1 )

**説明:** 散布図において、加速因子の軸に変換スケールを使うよう指定する。このオプションは、加速因子の軸のスケールを線形と非線形の間で切り替える。 デフォルトではオン。
