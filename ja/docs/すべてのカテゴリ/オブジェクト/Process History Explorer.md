...
**構文:** obj = Process History Explorer(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**構文:** obj = Process History Explorer(...Goal is to Minimize Y( state=0|1 )...)

**説明:** 目標が応答変数をの最小化であることを指定する。
...
**構文:** obj &lt;&lt; Levels with Lowest Yield( state=0|1 )

**説明:** 工程変数の水準ごとの収率に関する要約統計量表のテーブルの表示／非表示を切り替える。
...
**構文:** obj &lt;&lt; Levels with Lowest Yield with Time Filter( state=0|1 )

**説明:** 収率が低い方から50区間の時間間隔を示す要約統計表の表示／非表示を切り替える。
...
**構文:** obj &lt;&lt; Order Interaction

**説明:** 収率を応答として順序列を要因とした一元配置分散分析の表示／非表示を切り替える。

**JMP追加されたバージョン:** 14

### Save Count Table

**構文:** obj &lt;&lt; Save Count Table

**説明:** [X, 工程]変数の各水準を通過したユニットの数を含む新しいデータテーブルを保存する。
...
**構文:** obj &lt;&lt; Save Log Count Table

**説明:** Log(Count + 1)の項目を除き、度数表と同じ形式を持つ新しいデータテーブルを保存する。
...
**構文:** obj &lt;&lt; Save Waiting Time

**説明:** 待機時間分析レポートの各表を個別のデータテーブルに保存する。
...
**構文:** obj &lt;&lt; Stepwise Regression( Goal( "Biggest Individual Difference" | "Biggest Total Difference" | "Most Predictive"), Time Filtering( "None" | "Starting Time"), X Transform( "None" | "Log(Count+1)"), N Steps( number ) )

**説明:** 指定されたオプションに基づいて、ステップワイズ回帰分析を実行する。各ステップでモデルに追加された項の順序を示す表を表示する。
...
**構文:** obj &lt;&lt; Transition Analysis( columns, {"level1", "level2", ... } )

**説明:** ある工程から別の工程への遷移において、結果の収率に問題があるものを特定するのに役立つレポートを表示する。
...
**構文:** obj &lt;&lt; Waiting Time Analysis

**説明:** 各IDの待機時間の要約統計量表を表示する。この表では、特定のIDがあるステップを終えてから、別のステップを開始するまでの時間を待機時間としている。
...
**構文:** Yield Columns( columns )

**説明:** 収率の値を格納している列を指定する。
...
**構文:** Yield Table( data table )

**説明:** 各IDの収率を格納しているデータテーブルを指定する。
