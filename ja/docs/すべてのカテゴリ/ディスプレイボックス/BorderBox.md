...
**説明:** 枠線で囲まれたディスプレイボックスを中に含むディスプレイボックスを戻す。
...
**構文:** obj &lt;&lt; Bottom( pixels )

**説明:** 境界ボックスの最下部と中身の間にスペースを追加する。
...
**説明:** Border Box(境界ボックス)の背景を、背景色で塗りつぶすかを否かを設定する。
...
**構文:** obj &lt;&lt; Set Style( pen style )

**説明:** 枠線の線種を設定する。Pen styleは、次の番号か引用符付き文字列のどちらか: 0 (Solid)、1 (Dotted)、2 (Dashed)、3 (DashDot)、4 (DashDotDot)
