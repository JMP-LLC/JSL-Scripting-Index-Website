...
**構文:** obj << Editable( state=0|1 )

**説明:** 有効(1)の場合、テキストをマウスでクリックして編集することができる。
...
**構文:** obj << Fixed Size( state=0|1 )

**説明:** この指定を有効にすると、ユーザー定義の注釈を指定したサイズで表示する。有効でない場合は、テキストの内容によりサイズが計算される。
...
**構文:** 0|1 = obj << GetFilled

**説明:** テキスト背景が塗りつぶされているかどうかを戻す。
...
**構文:** N|NE|E|SE|S|SW|W|NW = obj << GetOctant
...
**構文:** 0|1 = obj << GetTagLine

**説明:** タグラインが有効なら1、無効なら0を戻す。
...
**構文:** obj << Octant( <N|NE|E|SE|S|SW|W|NW> )

**説明:** ディスプレイボックス内でテキストを描く位置を設定する。
...
**構文:** obj << Text Box( x1,y1,x2,y2 )

**説明:** テキストをテキストボックスに挿入する。
