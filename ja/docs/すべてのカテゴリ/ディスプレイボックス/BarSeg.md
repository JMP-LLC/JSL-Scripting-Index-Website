...
**構文:** Bar Seg( x, y, < Row States( dt | dt,[rows] | dt,{{rows}, ...} | {states} ) >, < Sizes( s ) > )

**説明:** 要約データの棒グラフを描くディスプレイセグメントを戻す。
...
**構文:** proportion = obj << Get Side by Side Overlap

**説明:** [横に並べて表示]形式の棒グラフの、項目の重なり幅を戻す。
...
**構文:** proportion = obj << Get Width Proportion

**説明:** 描画する棒の幅の割合を戻す。0は自動サイズ、1は隙間なし。
...
**構文:** Set Label Offset {Bar Index, X Scale Coordinate, Y Scale Coordinate}

**説明:** グラフの値ラベルから棒の座標までのオフセットを設定する。
...
**構文:** obj << Set Side by Side Overlap( proportion or missing )

**説明:** [横に並べて表示]形式の棒グラフの、項目の重なり幅を設定する。値は0 (重ねない)～1 (完全に重ねる)。
...
**構文:** obj << Set Width Proportion( proportion )

**説明:** 描画する棒の幅の割合を設定する。0は自動サイズ、1は隙間なし。
