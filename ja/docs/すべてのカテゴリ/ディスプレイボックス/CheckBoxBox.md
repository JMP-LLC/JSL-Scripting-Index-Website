...
**説明:** 入力されたlistの各項目についてチェックボックスを表示するディスプレイボックスを戻す。
...
**構文:** obj &lt;&lt; Enable Item( index, 0|1 )

**説明:** indexの項目を有効(1)または無効(0)に設定する。
...
**構文:** 0|1 = obj &lt;&lt; Get( &lt;index&gt; )

**説明:** 3状態チェックボックスの場合に、チェックボックスがオン（1）、オフ（0）、または中間状態（empty()）のどれであるかを戻す。
...
**構文:** indices = obj &lt;&lt; Get Selected Indices As Matrix

**説明:** Returns a list of all of the selected indices as a matrix
...
**構文:** obj &lt;&lt; Get Three State( &lt;Boolean&gt; )

**説明:** 3状態チェックボックスを作成する。すべての項目は、0、1、またはempty()のいずれかとなる。
...
**説明:** indexの項目が有効(選択可能)な場合は1、無効(選択不可)な場合は0を戻す。
...
**構文:** obj &lt;&lt; Set( &lt;index&gt;,&lt;Boolean|empty()&gt;,&lt;Run Script(0|1)&gt; )

**説明:** チェックボックスをオン（1）またはオフ（0）に設定する。RunScriptオプションを使用する場合、Boolean引数を指定する必要がある（位置指定の引数であるため）。
...
**構文:** obj &lt;&lt; Set All( &lt;Boolean|empty()&gt;,&lt;Run Script(0|1)&gt; )

**説明:** すべてのチェックボックスをオンまたは（1）オフ（0）に設定する。RunScriptオプションを使用する場合、Boolean引数を指定する必要がある（位置指定の引数であるため）。
...
**構文:** obj &lt;&lt; Set Function( Function( {thisBox, index}, &lt;script&gt; ) )

**説明:** コントロールに関数を設定する。関数のスクリプトは、Check Boxがクリックされたときに実行される。
...
**構文:** obj &lt;&lt; Set Three State( &lt;Boolean&gt; )
