...
**説明:** 階層型ツリーを表すディスプレイボックスを戻す。デフォルトでは1つの項目を選択できる。multiselectが指定されている場合は、ツリー内の複数の項目を選択できる。
...
**構文:** obj &lt;&lt; Enable Item( node, 0|1 )

**説明:** ツリーのノードを有効または無効にする。ノードを有効にするには2つめの引数に1　、無効にするには0に設定する。
...
**構文:** script = obj &lt;&lt; Get Context Menu Script

**説明:** ツリーまたはツリーノードが右クリックされたときに実行するスクリプト、関数、またはメソッドを戻す。
...
**構文:** script = obj &lt;&lt; Get Node Double Click Script

**説明:** ノードがダブルクリックされたときに実行するスクリプト、関数、またはメソッドを戻す。
...
**構文:** obj &lt;&lt; Get Node Select Script( script|Function( {thistree, thisnode}, &lt;script&gt; ) )

**説明:** ツリー内の選択が変更されたときに実行するスクリプト、関数、またはメソッドを戻す。
...
**説明:** ルートツリーノードのリストを取得する。ルートが存在しない場合は、空のリストが戻される。
...
**説明:** 単一選択のツリーの場合、選択されたツリーノード、またはノードが選択されていなければ空白が戻される。複数選択のツリーの場合、選択されたノードのリスト、またはノードが選択されていなければ空白が戻される。
...
**構文:** collapsed = obj &lt;&lt; Is Collapsed( node )

**説明:** このノードがツリー内で折りたたまれている場合は、真(true)を戻す。
...
**構文:** expanded = obj &lt;&lt; Is Expanded( node )

**説明:** このノードがツリー内で展開されている場合は、真(true)を戻す。
...
**構文:** enabled = obj &lt;&lt; Is Item Enabled( node )

**説明:** このノードがツリー内で有効(使用可能)である場合は、真(true)を戻す。
...
**構文:** obj &lt;&lt; Is Multiselect

**説明:** 複数選択のツリーの場合は1を戻し、単一選択ツリーの場合は0を戻す。
...
**説明:** このノードがツリーのルートノードである場合は、真(true)を戻す。
...
**構文:** selected = obj &lt;&lt; Is Selected( node )

**説明:** このノードがツリー内で選択されている場合は、真(true)を戻す。
...
**構文:** obj &lt;&lt; Make Visible( node )

**説明:** ツリー内でノードが見えるようにする。
...
**構文:** obj &lt;&lt; Set Context Menu Script( Function( {thistree, thisnode}, &lt;script&gt; ) )

**説明:** ツリーまたはツリーノードが右クリックされたときに実行するスクリプト、関数、またはメソッドを設定する。
...
**構文:** obj &lt;&lt; Set Multiselect( state=0|1 )

**説明:** ツリーを単一選択(0)または複数選択(1)に設定する。
...
**構文:** obj &lt;&lt; Set Node Double Click Script( script )

**説明:** ノードがダブルクリックされたときに実行するスクリプト、関数、またはメソッドを設定する。
...
**構文:** obj &lt;&lt; Set Node Select Script( script|Function( {thistree, thisnode}, &lt;script&gt; ) )

**説明:** ツリー内の選択が変更されたときに実行するスクリプト、関数、またはメソッドを設定する。選択がクリアされた場合は、Emptyがスクリプトに渡される。
...
**構文:** obj &lt;&lt; Set Roots( roots )

**説明:** このツリーに表示されるルートノードのリストを設定する。
...
**構文:** obj &lt;&lt; Set Selected( node|{nodes}, &lt;0|1&gt; )

**説明:** Tree Box内にある指定したツリーノードを選択する。ツリーノードのリストが指定された場合、複数選択のツリーではリスト内のすべてのノードが選択される。単一選択のツリーでは、リスト内の最初のノードが選択される。
