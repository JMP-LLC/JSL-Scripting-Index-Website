...
**構文:** obj << Add Axis Label( label )

**説明:** 指定された文字列の軸ラベルを追加する。
...
**構文:** obj << Add Ref Line( value, <"Solid|Dashed|Double">, <Color>, <"label">, <line width>, <transparency> )

**説明:** グリッドのvalueの位置に参照線を追加する。
...
**構文:** obj << Axis Settings

**説明:** 軸ダイアログを開く。または指定の軸に対し、設定可能な軸オプションを設定する。
...
**構文:** obj << Copy Axis Settings

**説明:** 現在の軸の設定を含んだスクリプトをクリップボードにコピーする。
...
**構文:** obj << Edit Value Order

**説明:** [値の表示順序]列プロパティの編集ウィンドウを開く。
...
**説明:** Returns the format specification for axis values.
...
**構文:** obj << Interval( "数値"|"年"|"四半期"|"月"|"週"|"日"|"時間"|"分"|"秒" )

**説明:** 目盛りの数値ラベルの形式を設定する。
...
**構文:** obj << Label Row Nesting( number )

**説明:** 日付形式の軸に対し、ラベルの入れ子の行数を設定する。
...
**構文:** obj << Minor Ticks( number )

**説明:** 主目盛りの間に配置する目盛りの数を設定する。
...
**構文:** obj << Paste Axis Settings

**説明:** クリップボードにコピーされている軸の設定を適応する。
...
**構文:** obj << Remove Axis Label

**説明:** Add Axis Labelで追加されたラベルをすべて削除する。
...
**構文:** obj << Reversed Scale

**説明:** 通常のスケールを逆転させ、最も高い値を左または下に配置する。
...
**説明:** 他のカスタマイズはそのままにして、軸のスケールを元に戻す。
...
**構文:** obj << Save To Column Property

**説明:** 軸の設定を、この軸に関連したデータ列に軸プロパティとして保存する。
...
**構文:** obj << Scale( "線形"|"対数"|"指数 確率"|"Weibull 確率"|"ロジスティック 確率"|"Frechet 確率"|"正規 確率"|"立方根"|"Johnson Su"|"地図座標"|"地図座標 米国"|"べき乗"|"ガンマ 確率"|"ベータ 確率"|"二重正規混合 確率"|"三重正規混合 確率"|"カスタムスケール" )

**説明:** 軸に適用するスケールの種類を指定する。カスタムスケールの場合は、さらに節が2つ必要になる。Scale To Internal(変換スケール)とScale To External(逆変換スケール)を参照。
...
**構文:** obj << Scale ID( number )

**説明:** 軸が複数ある場合のために軸に固有のIDを追加する。
...
**構文:** obj << Scale To External

**説明:** 指定されたカスタムの計算式に従ってスケールを設定する時、その計算式の逆関数を設定する。
...
**構文:** obj << Scale To Internal

**説明:** 指定されたカスタムの計算式に従ってスケールを設定する時、その計算式を設定する。
...
**説明:** 主目盛りのラベルを表示(1)または非表示(0)にする。 デフォルトではオン。
...
**構文:** obj << Show Major Grid( state=0|1 )

**説明:** 数値ラベルがつく目盛り(主目盛り)のグリッドの表示(1)/非表示(0)を切り替える。
...
**構文:** obj << Show Major Labels( state=0|1 )
...
**構文:** obj << Show Major Ticks( state=0|1 )

**説明:** 数値ラベルがつく目盛り(主目盛り)の表示(1)/非表示(0)を切り替える。 デフォルトではオン。
...
**構文:** obj << Show Minor Grid( state=0|1 )

**説明:** 数値ラベルがつかない目盛り(補助目盛り)のグリッドの表示(1)/非表示(0)を切り替える。
...
**構文:** obj << Show Minor Labels( state=0|1 )

**説明:** 補助目盛りのラベルを表示(1)または非表示(0)にする。
...
**構文:** obj << Show Minor Ticks( state=0|1 )

**説明:** 数値ラベルがつかない目盛り(補助目盛り)の表示(1)/非表示(0)を切り替える。 デフォルトではオン。
...
**構文:** <X / Y> Axis( Side ( <1/2> ) )

**説明:** 軸をどちら側に表示するか(primary (1)またはsecondary (2))を変更する。
...
**構文:** obj << Tick Label List

**説明:** 軸目盛りラベルの値と位置をリストで設定する。
...
**構文:** obj << Update Ref Line( ID, index, value, <"Solid|Dashed|Double">, <Color>, <"label">, <line width>, <transparency> )
