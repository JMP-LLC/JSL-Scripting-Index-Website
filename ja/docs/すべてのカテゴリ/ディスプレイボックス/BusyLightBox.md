...
**説明:** ビジーライトボックスを無効にして非表示にする。Disableが機能するには、Automaticがオフでなければならない。Automaticをオンにするか、ボックスにAdvanceメッセージを送ると、ビジーライトボックスが再度有効になり、表示される。
...
**説明:** Automaticをオンにしたときに、ビジーライトボックスが動作する速度を設定する。
