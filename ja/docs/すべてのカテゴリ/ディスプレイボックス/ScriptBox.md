...
**説明:** 編集可能なスクリプトボックスを含んだディスプレイボックスを戻す。
...
**構文:** obj &lt;&lt; Append Text( text )

**説明:** 指定したテキストをスクリプトの末尾に追加する。
...
**説明:** スクリプトボックスの中で折り畳めるセクションをすべて折り畳む。
...
**説明:** スクリプトボックスの中で折り畳んだセクションをすべて展開する。
...
**構文:** obj &lt;&lt; Get Cursor Line

**説明:** 現在、カーソルが置かれている行を戻す。
...
**構文:** obj &lt;&lt; Get Line Text( line number )
...
**説明:** スクリプトウィンドウ内のすべてのテキストを、1行の内容を1つの文字列とした文字列リストで戻す。
...
**構文:** obj &lt;&lt; Get Show Empty Scrollbars( state=0|1 )

**説明:** 必要でなくとも横と縦のスクロールバーが表示されるかどうかを示すブール値を戻す。
...
**構文:** obj &lt;&lt; Get Show Scrollbars

**説明:** 横と縦のスクロールバーが表示されるかどうかを示す2つの項目のリストを戻す。
...
**構文:** obj &lt;&lt; Read Only( state=0|1 )

**説明:** 真(1)に設定するとスクリプトボックス内のテキストは変更できなくなる。
...
**構文:** obj &lt;&lt; Set Cursor Position( position )
...
**構文:** obj &lt;&lt; Set Line Text( line number, text )
...
**構文:** obj &lt;&lt; Set Selection Width( width )
...
**構文:** obj &lt;&lt; Set Show Empty Scrollbars( state=0|1 )

**説明:** true (1)の場合、必要でなくとも、横または縦のスクロールバーが表示される。
...
**構文:** obj &lt;&lt; Set Show Scrollbars( state=0|1 )

**説明:** 横と縦のスクロールバーが表示されるかどうかを指定する。
...
**説明:** スクリプトボックス内の現在のテキストを指定したテキストに置き換える。
...
**構文:** obj &lt;&lt; Show Embedded Log( state=0|1 )

**説明:** ウィンドウ内のログを有効/無効にする。
...
**構文:** obj &lt;&lt; Show Line Numbers( state=0|1 )

**説明:** Script Boxの左側に行番号を表示する。
