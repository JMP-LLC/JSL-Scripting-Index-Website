...
**説明:** ボタンのクリックをシミュレートし、コールバックを実行する。トグルボタンの場合、コールバックを呼び出す前に状態が変わる。メニューボタンの場合、どの項目をクリックするかを指定する引数が必要。
...
**説明:** 切替ボタンの状態が「押された」になっている場合は1、「押されていない」になっている場合は0を戻す。Style("Toggle")を持つボタンにのみ適用される。
...
**構文:** left|right = obj &lt;&lt; Get Icon Location

**説明:** ボタンボックス内のアイコンの位置を戻す。アイコンはボタンテキストの左または右に表示できる。
...
**構文:** obj &lt;&lt; Get Menu Choice

**説明:** ユーザが最後に選択した項目の通し番号を取得する。
...
**構文:** list of menu items = obj &lt;&lt; Get Menu Items

**説明:** ボタンがクリックされたときに呼び出されるポップアップメニューの項目を取得する。
...
**構文:** style = obj &lt;&lt; Get Underline Style
...
**構文:** obj &lt;&lt; Open Next Outline

**説明:** アウトラインボックスを閉じて、次のものを開く。
...
**構文:** obj &lt;&lt; Restore Focus After Click( state=0|1 )

**説明:** クリックされた後、クリック前の状態にフォーカスを戻すようにボタンを設定する。Windowsの場合、デフォルトではクリックされたボタンにフォーカスが移動する。Macの場合、デフォルトではフォーカスがクリック前の状態に戻る。
...
**構文:** obj &lt;&lt; Set( pressed, &lt;run script(0|1)&gt; )

**説明:** 切替ボタンの状態を「押された」(1)または「押されていない」(0)に設定する。Style("Toggle")を持つボタンにのみ適用される。
...
**構文:** obj &lt;&lt; Set Button Name( name )

**説明:** ボタン名を指定のテキストに設定する。
...
**構文:** obj &lt;&lt; Set Function( Function( {thisBox}, &lt;script&gt; ) | Method({thisBox}, &lt;script&gt;) | &lt;symbol&gt; )

**説明:** コントロールに関数を設定する。関数のスクリプトは、ボタンが押されたときに実行される。
...
**構文:** obj &lt;&lt; Set Icon( icon name|image filepath )

**説明:** JMPアイコンまたはボタンの画像を表示する。JMPアイコン名または画像のファイルパスを指定する。
...
**構文:** obj &lt;&lt; Set Icon Location( left,right )

**説明:** アイコンの位置をボタンテキストの左側または右側に設定する。
...
**構文:** obj &lt;&lt; Set Menu Choice( index )

**説明:** 選択状態にするメニュー項目の番号を設定する。
...
**構文:** obj &lt;&lt; Set Menu Items( list of menu items )

**説明:** ボタンがクリックされたときに呼び出されるポップアップメニューの項目を設定する。
...
**構文:** obj &lt;&lt; Underline Style( state=0|1 )

**説明:** 標準スタイルのボタンではなくて、下線の付いたスタイルのボタンを設定する。
...
**構文:** obj &lt;&lt; Unhide Next Item

**説明:** アウトラインボックス内の非表示の項目を表示する。
