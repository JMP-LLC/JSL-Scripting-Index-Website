...
**説明:** 上向き/下向きの矢印ボタンを持つディスプレイボックスを戻す。script引数は、クリックされた矢印の方向を示す引数と共に呼び出される(負は下向き、正は上向き)。絶対値1が1回のクリックを示し、2以上の値は、アクションの反復を示す。
...
**説明:** ボタンがクリックされたものとし、指定されているスクリプトを実行する。
...
**構文:** obj &lt;&lt; Decrement( &lt;value&gt; )

**説明:** 下向き矢印がクリックされたものとし、指定されているスクリプトを実行する。
...
**構文:** obj &lt;&lt; Increment( &lt;value&gt; )

**説明:** 上向き矢印がクリックされたものとし、指定されているスクリプトを実行する。
...
**構文:** obj &lt;&lt; Set Function( Function( {thisBox, increment}, &lt;script&gt; ) )

**説明:** コントロールに関数を設定する。関数のスクリプトは、Spin Boxのボタンが押されたときに実行される。[increment]は、数値のボックスに設定される値の間隔。
