...
**説明:** 他のディスプレイボックスを横方向または縦方向に配列できるディスプレイボックスと、そのサイズをインタラクティブに調整するためのコントロールを戻す。子のサイズはSplitter Boxの幅または高さの割合で指定する。オプションのSize引数は、最上位のSplitter Boxに対する指定(ボックス全体のサイズ)で、子ボックスはそれに対する割合での指定となる。
...
**構文:** obj &lt;&lt; Close Panel( index,&lt;previous&gt; )

**説明:** Indexで指定されたパネルを閉じる。previous引数が1の場合はスペースが前のボックスに、それ以外の場合は次のボックスに割り当てられる。
...
**構文:** obj &lt;&lt; Divider Color( color ); color = obj &lt;&lt; Get Divider Color

**説明:** 分割線の色を指定するか、または戻す。指定しなかった場合、背景色に基づいて色が決定される。
...
**構文:** obj &lt;&lt; Divider Length( number ); number = obj &lt;&lt; Get Divider Length

**説明:** 分割線の長さを割合で指定するか、または戻す。値を0.0にすると、分割線は見えなくなる。値を1.0 (デフォルト)にすると、最長の分割線になる。
...
**構文:** obj &lt;&lt; Divider Thickness( number ); number = obj &lt;&lt; Get Divider Thickness

**説明:** 分割線の太さを指定するか、または戻す。この値は、非ゼロでなければならない。
...
**構文:** {size1,...} = obj &lt;&lt; Get Sizes

**説明:** Splitter Boxの各パネルのサイズの比率を戻す。
...
**構文:** obj &lt;&lt; Open Panel( index,&lt;previous&gt; )

**説明:** 指定されたindexのパネルを開く。previous引数が1の場合はリスト内の前のボックスからスペースが取られ、そうでない場合は次のボックスからスペースが取られる。
...
**構文:** obj &lt;&lt; Set Sizes( {size1,...} )

**説明:** Splitter Boxの各パネルのサイズの比率を設定する。
...
**説明:** Splitter Boxの最後のパネルのサイズの比率を設定する。
...
**構文:** state=0|1 = obj &lt;&lt; get horizontal

**説明:** 横のList Boxの場合は1、縦のList Boxの場合は0を戻す。
...
**構文:** obj &lt;&lt; set horizontal( state=0|1 )

**説明:** List Boxを横(1)、または縦(0)に設定する。
