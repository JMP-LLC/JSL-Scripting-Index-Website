...
**構文:** obj &lt;&lt; Column Names( strings or list of strings )
...
**説明:** セマンティック表示形式を行列に適用するかどうかを制御する。セマンティック表示形式は環境設定で制御することも可能。
...
**構文:** obj &lt;&lt; Get Effective Format( col, &lt;row&gt; )
...
**説明:** 行列のEnable Semantic Formatsの設定を取得する。セマンティック表示形式は環境設定で制御することも可能。
...
**構文:** obj &lt;&lt; Get Format( width, decimalplaces )

**説明:** 現在、設定されている表示形式を戻す。
...
**構文:** obj &lt;&lt; Make Combined Data Table( &lt;Invisible(bool)&gt; )

**説明:** 同様のすべての行列からデータテーブルを作成する。
...
**説明:** MatrixBox内の値を含んだ新しいデータテーブルを作成する。
...
**構文:** obj &lt;&lt; Open( state=0|1 )

**説明:** MatrixBox内の値を表示または非表示にする。
...
**構文:** obj &lt;&lt; Row Names( strings or list of strings )
...
**構文:** obj &lt;&lt; Set Conditional Format( None|Correlation|PValue|Factor Pattern )
...
**構文:** obj &lt;&lt; Sort( column, ascending )

**説明:** 1つの列の値に基づいて行列の行を並べ替える。1つ目の引数に0が指定されている場合は、並べ替えを元に戻す。
