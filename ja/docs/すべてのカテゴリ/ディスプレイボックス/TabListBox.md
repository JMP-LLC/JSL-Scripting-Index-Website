...
**説明:** 他のディスプレイボックスを含んだページを持つディスプレイボックスを戻す。
...
**構文:** obj &lt;&lt; Add(&lt;index&gt;,"title",contents);obj &lt;&lt; Add(&lt;index&gt;,tabPageBox)

**説明:** 新しいタブを追加する。indexが指定されなかった場合、タブは既存のタブの後ろに追加される。追加される内容がTab Page Boxでない場合、内容を格納するための新しいTab Page Boxを追加する。
...
**説明:** indexのタブを削除する。indexが指定されていない場合は、選択されているタブを削除する。
...
**構文:** obj &lt;&lt; DockName( text )

### Dockable

**構文:** obj &lt;&lt; Dockable( bool )

**説明:** ドッキング可能なコンテナであれば、移動可能な子のコンテナをドラッグアンドドロップで配置できる。
...
**構文:** bool = obj &lt;&lt; Get Dockable

**説明:** コンテナボックスのDockableの設定を戻す。
...
**構文:** obj &lt;&lt; Get Min Title Width

**説明:** Overflow Enabled が指定された場合において、タブタイトルの表示幅に対する最小値を取得する。デフォルト設定の0は、デフォルトの最小幅が適用されることを意味する。
...
**構文:** obj &lt;&lt; Get Overflow Enabled

**説明:** Overflow Enabledが1の場合、タブリストはタブタイトルの幅に自動調整されない。タブリストの幅が一部のタイトルには不十分な場合、新しいタブを選択できるオーバーフローメニューが表示される。
...
**構文:** obj &lt;&lt; Get Show Tabs

**説明:** タブの現在の表示/非表示状態を取得する。
...
**構文:** style = obj &lt;&lt; Get Style

**説明:** タブコントロールの現在の表示スタイルを取得する。
...
**構文:** obj &lt;&lt; Get Tab Changed

**説明:** 異なるタブが選択されたときに実行するスクリプトを戻す。
...
**構文:** obj &lt;&lt; Get Tab Close

**説明:** スクリプトまたは閉じるアイコンでタブを閉じる際に実行されるスクリプトを戻す。このスクリプトが1を戻すとタブが閉じ、0を戻すと閉じる処理がキャンセルされます。
...
**構文:** "string" = obj &lt;&lt; Get Tab Close Tip

**説明:** タブを閉じるアイコンのツールヒントを戻す。
...
**構文:** all | horizontal,vertical | left,top,right,bottom = obj &lt;&lt; Get Tab Margin
...
**説明:** 新しいタブがクリックされたときに実行するスクリプトを戻す。このスクリプトによって新しいタブが作成される。
...
**構文:** "string" = obj &lt;&lt; Get Tab New Icon
...
**構文:** style = obj &lt;&lt; Get Title Location

**説明:** 現在のタイトルの位置の設定を取得する。
...
**構文:** index = obj &lt;&lt; Index of Tab( "string" )

**説明:** 指定のタイトルを持つタブの番号を戻す。
...
**構文:** obj &lt;&lt; Insert(&lt;index&gt;,"title",contents);obj &lt;&lt; Insert(&lt;index&gt;,tabPageBox)

**説明:** 新しいタブを挿入する。indexが指定されなかった場合、新しいタブは既存のタブの後ろに追加される。追加される内容がTab Page Boxでない場合、内容を格納するための新しいTab Page Boxを追加する。
...
**構文:** obj &lt;&lt; Move Tab( fromIndex, toIndex )

**説明:** fromIndexの位置にあるタブをtoIndexの位置に移動する。
...
**構文:** obj &lt;&lt; Replace(&lt;index&gt;,"title",contents);obj &lt;&lt; Replace(&lt;index&gt;,tabPageBox)

**説明:** タブを置換する。indexが指定されなかった場合、選択されているタブを置換する。追加される内容がTab Page Boxでない場合、内容を格納するための新しいTab Page Boxを追加する。
...
**構文:** obj &lt;&lt; Set Min Title Width( number )

**説明:** Overflow Enabledオプションが指定された場合において、タブタイトルの表示幅に対する最小値を設定する。デフォルト設定の0を使うと、デフォルトの最小幅が適用される。
...
**構文:** obj &lt;&lt; Set Overflow Enabled( state=0|1 )

**説明:** Overflow Enabledが真の場合、タブリストはタブタイトルの幅に自動調整されない。タブリストの幅がすべてのタイトルには不十分な場合、新しいタブを選択できるオーバーフローメニューが表示される。このオプションは、スタイルがTabに設定されている場合のみ適用される。
...
**構文:** obj &lt;&lt; Set Selected( index )

**説明:** indexのタブにフォーカスを置く。
...
**構文:** obj &lt;&lt; Set Style( "タブ"|"コンボ"|"アウトライン"|"縦に展開"|"横に展開"|"最小サイズ" )

**説明:** タブコントロールの表示スタイルを変更する。
...
**構文:** obj &lt;&lt; Set Tab Changed( script|Function( {this}, &lt;script&gt; ) )

**説明:** 変更が行われた(たとえば新しくタブが選択された)後、scriptで指定されたスクリプトまたはfunctionで指定された関数を実行する。
...
**構文:** obj &lt;&lt; Set Tab Close( script|Function )

**説明:** スクリプトまたは閉じるアイコンでタブを閉じる際に実行されるscriptまたはFunctionを設定する。scriptまたはFunctionが1を戻すとタブが閉じ、0を戻すと閉じる処理がキャンセルされる。
...
**構文:** obj &lt;&lt; Set Tab Close Tip( "string" )

**説明:** タブを閉じるアイコンのツールヒントを設定する。
...
**構文:** obj &lt;&lt; Set Tab Margin( all | horizontal,vertical | left,top,right,bottom )

**説明:** タブの余白を設定する。入力されたパラメータは、上下左右すべてに適用するか、左右と上下に適用するか、または上下左右ともに異なる値を適用することもできる。
...
**構文:** obj &lt;&lt; Set Tab New( script|Function( {this}, &lt;script&gt; ) )

**説明:** 新しいタブがクリックされたときに実行されるscriptまたはFunctionを設定する。scriptのスクリプトまたはfunctionの関数によって新しいタブが作成される。
...
**構文:** obj &lt;&lt; Set Tab New Icon( "string" )
...
**構文:** obj &lt;&lt; Set Tab New Tip( "string" )

**説明:** 新しいタブのツールヒントを設定する。
...
**構文:** obj &lt;&lt; Set Title Location( "自動"|"上"|"下"|"左"|"右" )

**説明:** タブページのタイトルの位置を設定する。[Auto]にすると、コンテンツのサイズに応じて自動的に位置が選択される。このオプションは、スタイルがTabbedに設定されている場合のみ適用される。
