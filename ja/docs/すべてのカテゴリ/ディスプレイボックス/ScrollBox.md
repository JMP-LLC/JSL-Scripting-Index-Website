...
**説明:** 中身がScrollBoxのサイズより大きいときに下側と右側にスクロールバーがあるディスプレイボックスを戻す。
...
**構文:** obj << Get Auto Scrollable

**説明:** Auto Scrollableの設定を戻す。
...
**構文:** obj << Get Clip Printing( state=0|1 )

**説明:** 印刷時に行うスクロールボックスのコンテンツのクリッピングを行うか否か、現在の設定を戻す。
...
**構文:** pixels = obj << Get Height

**説明:** スクロール領域の高さを戻す。

### Get Scroll Extents

**構文:** {h pixels,v pixels} = obj << Get Scroll Extents

**説明:** 横と縦のスクロールの最大の位置を戻す。
...
**構文:** {h,v} = obj << Get Scroll Position

**説明:** 現在の横と縦のスクロール位置を戻す。
...
**構文:** {h,v} = obj << Get Scrollers

**説明:** 横と縦のスクロールのためのフラグを戻す。指定の方向のスクロールが無効になっている場合、Scroll Boxはその方向では通常のコンテナとして動作する。
...
**構文:** obj << Get Show Empty

**説明:** ScrollBoxが空の場合でも、スクロールバーを表示するか否か現在の設定を戻す。
...
**構文:** obj << Set Auto Scrollable( state=0|1 )

**説明:** 子ボックスの伸縮プロパティに従ってスクロールのパラメータを自動設定する。ボックスが伸縮しない方向でスクロールバーが動作する。
...
**構文:** obj << Set Clip Printing( state=0|1 )

**説明:** 印刷の際、スクロールボックスのコンテンツがスクロールボックスのサイズにクリッピングされるか、またはすべてのコンテンツを印刷するかを指定する。デフォルトではクリッピングはオフ。
...
**構文:** obj << Set Scroll Position( h,v )

**説明:** 横および縦のスクロール位置を設定する。位置はピクセルとして数値で指定するか、またはキーワードの「Start」と「End」を使って指定する。
...
**構文:** obj << Set Scrollers( h,v )

**説明:** 横と縦のスクロールのためのフラグを設定する。指定の方向のスクロールが無効になっている場合、Scroll Boxはその方向では通常のコンテナとして動作する。
...
**構文:** obj << Set Show Empty( state=0|1 )

**説明:** ScrollBoxが空でも、スクロールバーを表示する。
