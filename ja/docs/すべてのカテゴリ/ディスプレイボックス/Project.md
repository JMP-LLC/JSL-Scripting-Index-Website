...
**構文:** project &lt;&lt; Add Bookmarks( {&lt;File(path)&gt;, &lt;Folder( path, &lt;Expanded(boolean)&gt; )&gt;, &lt;Group(name, &lt;Expanded(boolean)&gt;, {contents}&gt;} )

**説明:** プロジェクトにブックマークを追加する。既存のブックマークも保持される。引数はブックマーク項目のリストで、それぞれFile()、Folder()、Group()を使用して指定する。Group()の中には、子としてFile、Folder、Groupを再帰的に同じ文法で使用できる。
...
**構文:** project &lt;&lt; Extract("file")

**説明:** プロジェクト内の特定のファイルを展開する。JMPのOpen()コマンドでは、JMPで直接開けないプロジェクトファイルは展開されない。Parquetファイルはその一例。
...
**構文:** project &lt;&lt; Extract All()

**説明:** プロジェクトのすべてのファイルを一時ディレクトリに展開する。
...
**構文:** project &lt;&lt; Get Bookmarks()

**説明:** プロジェクトのブックマークのリストを戻す。リストの形式は、Add Bookmarks()およびSet Bookmarks()で使用されたものと同じになる。
...
**構文:** project &lt;&lt; Get Layout()

**説明:** プロジェクトのウィンドウレイアウトを戻す。形式は、Set Layout()で使用されたものと同じになる。
...
**構文:** project &lt;&lt; Get Temp Dir()

**説明:** プロジェクトの一時ディレクトリについて、ディスク上のパスを取得する。
...
**構文:** project &lt;&lt; Is Extracted("file")

**説明:** プロジェクト内の特定のファイルが一時ディレクトリに展開されているかどうかを確認する。JMPのOpen()コマンドでは、JMPで直接開けないプロジェクトファイルは展開されない。Parquetファイルはその一例。
...
**構文:** project &lt;&lt; Project File Path()

**説明:** 保存されているプロジェクトファイルについて、ディスク上のパスを取得する。
...
**構文:** project &lt;&lt; Reset Layout()

**説明:** プロジェクトのウィンドウレイアウトをデフォルトの状態に戻す。
...
**構文:** project &lt;&lt; Run Script(script)

**説明:** プロジェクトのコンテキストでスクリプトを実行する。スクリプトにより開かれたウィンドウはすべてプロジェクトの一部となる。スクリプトはプロジェクトのコンテキストでJSL名にアクセスする。
...
**構文:** project &lt;&lt; Save Project(&lt;path&gt;)

**説明:** プロジェクトを保存する。保存する場所のパスおよびファイル名を指定する。プロジェクトがすでに保存されている場合、この引数を指定しなくてもよい。



プロジェクトの中に未保存のドキュメントがある場合は、プロジェクトの保存前にそのドキュメントを保存しなければならない。
...
**構文:** project &lt;&lt; Set Bookmarks( {&lt;File(path)&gt;, &lt;Folder( path, &lt;Expanded(boolean)&gt; )&gt;, &lt;Group(name, &lt;Expanded(boolean)&gt;, {contents}&gt;} )

**説明:** プロジェクトにブックマークを設定する。既存のブックマークは削除される。引数はブックマーク項目のリストで、それぞれFile()、Folder()、Group()を使用して指定する。Group()の中には、子としてFile、Folder、Groupを再帰的に同じ文法で使用できる。
...
**構文:** project &lt;&lt; Show Bookmarks(boolean)

**説明:** プロジェクトの「ブックマーク」ペインの表示/非表示を切り替える。
...
**構文:** project &lt;&lt; Show Contents(boolean)

**説明:** プロジェクトの「コンテンツ」ペインの表示/非表示を切り替える。
...
**構文:** project &lt;&lt; Show Log(boolean)

**説明:** プロジェクトの「ログ」ペインの表示/非表示を切り替える。
...
**構文:** project &lt;&lt; Show Recent Files(boolean)

**説明:** プロジェクトの「最近使ったファイル」ペインの表示/非表示を切り替える。
...
**構文:** project &lt;&lt; Show Workspace(boolean)

**説明:** プロジェクトの「ワークスペース」ペインの表示/非表示を切り替える。
