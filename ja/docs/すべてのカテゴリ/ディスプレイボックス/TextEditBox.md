...
**構文:** obj &lt;&lt; Bullet point( state=0|1 )

**説明:** テキストの先頭に箇条書き記号を表示する。
...
**構文:** obj &lt;&lt; Fixed Size( state=0|1, &lt;width&gt;, &lt;height&gt; )

**説明:** オンになっていると、テキストの幅がTextBoxの幅より広い場合でもTextBoxが拡張されません。
...
**構文:** bullet = obj &lt;&lt; Get Bullet

**説明:** テキストの前に箇条書き記号が描かれているかどうかを示す値を戻す。
...
**構文:** 0|1 = obj &lt;&lt; Get Hidden State( state=0|1 )
...
**構文:** "string" = obj &lt;&lt; Get Hint

**説明:** テキストボックスが空の場合にヒントとして表示されるテキスト。
...
**構文:** justify = obj &lt;&lt; Get Justify

**説明:** テキストを左、中央、または右に揃える。
...
**構文:** function = obj &lt;&lt; Get Text Changed

**説明:** テキストが変更される度に実行される関数を戻す。
...
**構文:** obj &lt;&lt; Justify Text( "左寄せ"|"中央寄せ"|"右寄せ" )
...
**構文:** obj &lt;&lt; Markup( state=0|1 )

**説明:** テキストにHTMLマークアップが含まれている可能性があり、マークアップとして解釈が必要であることを示す。使用できるHTMLタグは、太字(<b></b>)、斜体(<i></i>)、下線(<u></u>)、font face (<font face=&apos;&apos;></font>)、フォントの色(<font color=&apos;&apos;></font>)、フォントサイズ(<font size=&apos;&apos;></font>)、背景色(<background color=&apos;&apos;></background>)。フォントの色と背景色については、色を名前(例：blue)、16進数のHTMLカラーコード(例： #0000ff)、16進数カラーコード(例：0x0000ff)で指定できる。名前で色を指定する場合は、JMPの色の定義が使用される。
...
**構文:** obj &lt;&lt; Password Style( state=0|1 )

**説明:** 有効にすると、パスワードの代わりに*****がTextEditBoxに表示される。
...
**構文:** obj &lt;&lt; Preprocess Markup( state=0|1 )

**説明:** テキストを前処理して、JMPマークアップタグを適切な形式のHTMLタグで置き換える必要があることを示す。
...
**構文:** obj &lt;&lt; Rotate Text( left|right|horizontal )

**説明:** テキストを90度左または右に回転するか、水平に戻す。
...
**構文:** obj &lt;&lt; Set Script( script ) obj &lt;&lt; Script( script )

**説明:** テキストが編集された後に実行されるスクリプトを設定する。
...
**説明:** コントロールに関数を設定する。関数のスクリプトは、Text Edit Boxに変更が加えられたときに実行される。
...
**構文:** obj &lt;&lt; Set Text( "string", &lt;Run Script(bool)&gt; )

**説明:** ボックスのテキストを変更する。Text Edit Boxの場合、オプションのRun Script(bool)引数により、Set Textが実行されたときにText Edit Boxのスクリプトを実行するかどうかを指定できる。デフォルト値は、0 (スクリプトを実行しない)。
...
**構文:** obj &lt;&lt; Set Text Changed( Function( {this, value}, &lt;script&gt; ) )

**説明:** テキストが変更されるたびに実行されるfunctionを設定する。このコールバックは変更が確定する前の編集の間に呼び出される。編集がキャンセルされた場合、元の値に戻すための追加のコールバックが生成される。
...
**構文:** obj &lt;&lt; White Box Style( 0=ordinary text, 1=bordered white box entry field )

**説明:** 枠で囲まれた白背景のボックスにテキストを表示するか、ボックスなしのテキストとして表示するかを切り替える。
