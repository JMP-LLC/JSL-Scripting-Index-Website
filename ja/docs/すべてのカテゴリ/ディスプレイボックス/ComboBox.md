...
**説明:** テキスト項目のドロップダウンリストを表示するディスプレイボックスを戻す。コンボボックスの各項目には、オプションでツールヒントを含めることができる。その場合、ツールヒントのテキストは、括弧で囲んで項目のテキスト文字列のすぐ後に置く。オプションのEditable引数は、コンボテキストの編集を許可する。
...
**構文:** obj &lt;&lt; Append Item( string )

**説明:** コンボボックスのリストの末尾に新しい項目を追加する。
...
**説明:** indexで指定された項目を選択可能(1)または選択不可能(0)にする。
...
**構文:** obj &lt;&lt; Insert Item( string, &lt;position&gt; )

**説明:** 新しい文字列をリストの末尾、またはオプションのpositionで指定された位置に挿入する。
...
**説明:** indexの項目が選択可能の場合は1、選択不可能な場合は0を戻す。
...
**構文:** obj &lt;&lt; Set( index,&lt;run script(0|1)&gt; )

**説明:** 指定された添え字の項目がコンボボックスに表示されるようにする。
...
**説明:** コントロールに関数を設定する。関数のスクリプトは、Combo Boxで新しい項目が選択されたときに実行される。
