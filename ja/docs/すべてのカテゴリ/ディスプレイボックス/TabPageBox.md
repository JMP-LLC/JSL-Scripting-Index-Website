...
**構文:** obj &lt;&lt; Closeable( state=0|1 )

**説明:** タブを閉じることを可能にするか否かを設定する。
...
**構文:** obj &lt;&lt; Focus Color( color );color = obj &lt;&lt; Get Focus Color

**説明:** タブまたはタブ内のボックスにフォーカスがあるときに、そのタブのタイトルのフォーカスバーがフォーカスカラーで表示される。
...
**説明:** スクリプトまたは閉じるアイコンでタブを閉じる際に実行されるスクリプトを戻す。このスクリプトが1を戻すとタブが閉じ、0を戻すと閉じる処理がキャンセルされる。
...
**構文:** "bool" = obj &lt;&lt; Get Closeable

**説明:** タブを閉じることが可能か否かのステータスを戻す。
...
**説明:** ボックスのMoveableの設定を戻す。

### Get Scriptable Object

**構文:** scriptable object = obj &lt;&lt; Get Scriptable Object

**説明:** Tab Page Boxに関連付けられたスクリプト可能オブジェクトへの参照を戻す。
...
**説明:** タブページのタイトルをロックして、インタラクティブな変更を行えないようにする。
...
**構文:** obj &lt;&lt; Moveable( state=0|1 )

**説明:** 移動可能なボックスは、ドッキング可能なボックスの中でドラッグアンドドロップで配置できる。
...
**構文:** obj &lt;&lt; Set Close( script|Function( {this}, &lt;script&gt; ) )
...
**説明:** Tab Page Boxに関連付けられるスクリプト可能オブジェクトを設定する。Tab Page Boxには、スクリプト可能オブジェクトに送るメッセージを選択できる赤い三角ボタンメニューが表示される。引数が指定されない場合、スクリプト可能オブジェクトはクリアされる。
...
**説明:** タブページのタイトルが変更されたときに実行される関数を設定する。
...
**構文:** obj &lt;&lt; Title( "string", &lt;Run Script(0 | 1)&gt; )

**説明:** Tab Page Boxのタイトルを設定する。
