...
**Description :** Renvoie une boîte d’affichage affichant une colonne d’une table de données.
...
nw = New Window( "Example",
	Table Box(
		...
		col = pcb = Data Table Plot Col Box( :height )
	)
);
...
**Syntaxe :** obj << Add Element( number|string|list|matrix )

**Description :** Ajoute une nouvelle valeur à la boîte d’affichage.
...
New Window( "Example",
	col = Number Col Box( "Random Numbers",
		{Random Uniform(), Random Uniform() * 10, Random Uniform() * 100,
		Random Uniform() * 1000, Random Uniform() * 10000}
	)
);
...
**Syntaxe :** obj << Col ID( state=0|1 )

**JMP Version ajoutée :** 17

### Copy Column

**Syntaxe :** obj << Copy Column

**Description :** Copier le contenu de la colonne dans le presse-papiers

**JMP Version ajoutée :** 15

### Copy Selected Column Rows

**Syntaxe :** obj << Copy Selected Column Rows

**Description :** Copier les données des lignes sélectionnées au sein de la colonne dans le presse-papiers
...
**Description :** Affiche un point plutôt qu’une barre.
...
**Syntaxe :** list or matrix = obj << Get( <i>, <"Unsorted"> )

**Description :** Renvoie les valeurs dans une liste, ou la ie valeur. Par défaut, les valeurs sont renvoyées dans l&apos;ordre dans lequel elles apparaissent dans la table. Si Non trié est spécifié, les valeurs sont toujours dans le même ordre, peu importe comment la table de données est triée.
...
**Syntaxe :** matrix = obj << Get As Matrix

**Description :** Renvoie les valeurs dans une matrice, précisément un vecteur colonne.
...
**Syntaxe :** "Default"|"Top"|"Bottom" = obj << Get Axis Position

**Description :** Obtenir la position de l&apos;axe pour la colonne de graphiques. Par défaut, elle s&apos;affiche parfois en haut selon l&apos;échelle et le titre.
...
**Syntaxe :** obj << Get Bar Color

**Description :** Obtenir la couleur des barres du graphique
...
**Syntaxe :** obj << Get Bar Select Color

**Description :** Obtenir la couleur des barres du graphique correspondant à une ligne sélectionnée
...
**Syntaxe :** obj << Get Col ID( state=0|1 )
...
**Syntaxe :** data table column = obj << Get Data Table Column
...
**Syntaxe :** obj << Get Heading

**Description :** Renvoie le texte d’en-tête de la colonne.
...
**Syntaxe :** list = obj << Get Labels

**Description :** Renvoie les étiquettes pour chaque ligne.
...
**Syntaxe :** {color if upper < reference line, color if crosses reference line, color if lower > reference line, alpha, line width, pen style, cap length } = obj << Get primary interval line properties

### Get secondary interval line properties

**Syntaxe :** {color if upper < reference line, color if crosses reference line, color if lower > reference line, alpha, line width, pen style, cap length } = obj << Get secondary interval line properties

### Hide/Unhide

**Syntaxe :** obj << Hide/Unhide( state=0|1 )

**Description :** Masque ou affiche la boîte d’affichage.
...
**Syntaxe :** obj << Lock Title( boolean )

**Description :** Active ou désactive la modification des en-têtes de colonne
...
**Syntaxe :** obj << Lower( data table column )

**Description :** L’outil de limite inférieure trace une courbe passant par les points spécifiés.
...
**Syntaxe :** obj << Lower2( list|matrix )
...
**Description :** Affiche les marqueurs aux limites de chaque valeur.
...
**Syntaxe :** obj << Remove Element( row number )

**Description :** Supprime un élément de la colonne.
...
**Syntaxe :** obj << Set( <list>, <"Presorted"> )

**Description :** Définit les valeurs à partir d’une liste. Si Prétrié est spécifié, les valeurs s&apos;afficheront dans le même ordre que la liste passée, sinon les valeurs seront triées en fonction de l&apos;ordre de tri actuel de la table de données.
...
**Syntaxe :** obj << Set Axis Position( "Default"|"Top"|"Bottom" )

**Description :** Définir l&apos;axe pour la colonne de graphiques pour être toujours en haut ou en bas. La valeur par défaut est de l&apos;afficher parfois en haut selon l&apos;échelle et le titre.
...
**Syntaxe :** obj << Set Bar Color( color )

**Description :** Définir la couleur des barres du graphique.
...
**Syntaxe :** obj << Set Bar Select Color( color )

**Description :** Définir la couleur des barres du graphique correspondant à une ligne sélectionnée
...
**Syntaxe :** obj << Set Col ID( state=0|1 )
...
**Syntaxe :** obj << Set Data Table Column( data table column )
...
**Syntaxe :** obj << Set Heading( "string" )

**Description :** Modifie le texte d’en-tête de la colonne.
...
**Syntaxe :** obj << Set Labels( data table column )

**Description :** Définit les étiquettes pour chaque ligne.
...
**Syntaxe :** obj << Set Plot Style( "Default"|"Bar"|"Interval" )

**Description :** Modifie la représentation visuelle de la colonne de graphiques.
...
**Syntaxe :** obj << Set Reference Line( number )
...
**Syntaxe :** obj << Set Resizable( boolean )

**Description :** Active ou désactive la possibilité de redimensionner une colonne de manière interactive
...
**Syntaxe :** obj << Set Scale( minimum, maximum, <format name>, <width> | <width, decimal places>, < "Use thousands separator"> )

**Description :** Définit l&apos;échelle minimum et maximum du PlotColBox. Les paramètres de format spécifient un format utilisé uniquement si les deux premiers paramètres sont 0 et 1.
...
**Syntaxe :** obj << Set Smart Shrinking( boolean )

**Description :** Si activé, les colonnes dimensionnées en automatique ne se réduisent pas jusqu&apos;à ce que leur élément le plus large soit inférieur à 2/3 de leur largeur actuelle.
...
**Syntaxe :** obj << Set Values( <list> )

**Description :** Définit les valeurs à partir d’une liste.

```jsl

Names Default To Here( 1 );
New Window( "Example",
	scb = String Col Box( "Associated Text", {"a", "b", "c", "d", "e"} )
);
...
**Description :** Définit la largeur (en pixels) du diagramme PlotColBox.
...
**Syntaxe :** obj << Set primary interval line properties( {color if upper < reference line, color if crosses reference line, color if lower > reference line, alpha, line width, pen style, cap length } )

### Set secondary interval line properties

**Syntaxe :** obj << Set secondary interval line properties( {color if upper < reference line, color if crosses reference line, color if lower > reference line, alpha, line width, pen style, cap length } )

### Significance

**Syntaxe :** obj << Significance( data table column )

**Description :** Utilisé pour dimensionner le marqueur lorsque la colonne de graphiques est en mode d&apos;intervalle.
...
**Syntaxe :** obj << Upper( data table column )

**Description :** L’outil de limite supérieure trace une courbe passant par les points spécifiés.  Spécifier d’abord une limite inférieure.
...
**Syntaxe :** obj << Upper2( list|matrix )
