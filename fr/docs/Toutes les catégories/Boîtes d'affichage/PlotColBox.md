...
**Description :** Renvoie une boîte d&apos;affichage étiquetée avec string en tant que titre pour représenter graphiquement les nombres.
...
New Window( "World Population",
	Table Box(
		...
		pcb = Plot Col Box( "Population",
			dt:Name( "Population (1000)" ) << get values
		)
	)
);
pcb << Set Axis(
...
**Syntaxe :** obj << Set Values( matrix|list )

**Description :** Remplace les valeurs de PlotColBox par les valeurs fournies.
...
**Syntaxe :** obj << Significance( list|matrix )
