...
**Description :** Renvoie une boîte d’affichage pour afficher une liste ou des cases d’option. L’exécution du script est lancée lorsque vous cliquez sur la case d’option.
...
**Description :** Active ou désactive un seul élément dans une boîte de cases d’option
...
**Description :** Renvoie l’indice 1 de la case d’option sélectionnée.

```jsl

Names Default To Here( 1 );
New Window( "Example",
	r1 = Radio Box( {"Red", "Green", "Blue"}, Print( r1 << Get ) )
);
...
**Syntaxe :** selectedText = obj << Get Selected

**Description :** Obtient le texte de la case d’option sélectionnée.
...
**Syntaxe :** obj << Group( radiobox )

**Description :** Regroupe ensemble plusieurs boîtes de cases d’option de manière à ce que une seule case d’option du groupe soit sélectionnée à la fois.
...
**Syntaxe :** obj << Index Of( string )

**Description :** Renvoie l’index d’une chaîne, ou 0 si celui-ci n’a pas été trouvé.
...
**Syntaxe :** obj << Item Enabled( index )

**Description :** Vérifie si un élément particulier de la boîte de cases d’option est activé ou désactivé
...
**Description :** Définit la case d’option sélectionnée.
...
**Description :** Définit la fonction donnée pour le contrôle. Le script de la fonction s&apos;exécute lorsque l&apos;utilisateur clique sur une case d&apos;option.
