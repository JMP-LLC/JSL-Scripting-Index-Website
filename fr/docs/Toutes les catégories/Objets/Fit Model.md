...
**Description :** Affiche ou masque un graphique contenant un point pour chaque niveau de l&apos;effet autre que le contrôle. Chaque point indique la moyenne des moindres carrés de ce niveau par rapport à la moyenne des moindres carrés du niveau de contrôle. Les limites de décision supérieures (UDL) et les limites de décision inférieures (LDL) sont tracées.
