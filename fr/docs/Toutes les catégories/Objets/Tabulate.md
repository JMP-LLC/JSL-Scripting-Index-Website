...
**Syntaxe :** Analysis Columns( Column(s) )

**Description :** Ajoute les colonnes d&apos;analyse à la table de données active. Peut être utilisé avec les commandes Ajouter ou Modifier une table de données.

#### Ajouter à un élément existant
...
**Syntaxe :** Columns by Categories( column1, column2, ...) )

**Description :** Ajoute à la table de données un tableau croisé des noms de colonne et des catégories réunies pour des colonnes contenant des valeurs similaires.  Dans un script, le message Colonnes par catégories doit être inclus soit dans un message Table de données de colonnes, soit dans un message Table de données de lignes.
...
**Description :** Spécifier la colonne des fréquences à utiliser dans le calcul des statistiques

#### Défini dans un élément existant
...
#### Définir dans un élément nouveau
...
**Syntaxe :** Grouping Columns( Column(s) )

**Description :** Ajoute les colonnes de groupement à la table de données active. Peut être utilisé avec les commandes Ajouter ou Modifier une table de données.
...
#### Ajouter imbriqué à un élément nouveau
...
#### Ajouter un niveau imbriqué à un élément existant
...
**Description :** Permet de spécifier la colonne ID utilisée pour dénombrer les occurrences uniques.
...
**Syntaxe :** Page Column( Column )

**Description :** Spécifier la colonne des pages à utiliser pour configurer les pages du rapport

#### Colonne de page à réponses multiples
...
#### Définir la colonne de page dans un élément nouveau
...
#### Définir la colonne et le niveau de la page dans un élément existant
...
#### Définir la colonne et le niveau de la page dans un élément nouveau
...
**Syntaxe :** Weight( Column )

**Description :** Spécifier la colonne de pondération à utiliser dans le calcul des statistiques
...
#### Colonnes de groupement empilées
...
#### Colonnes de regroupement de réponses multiples
...
#### Tables de données à lignes et colonnes multiples
...
#### Tables de données à plusieurs lignes
...
**Description :** Utilisé avec Modifier la table de données pour ajouter des colonnes et des statistiques à une table de données existante. Sert également d&apos;alias pour Ajouter une table de données

#### Ajouter une colonne d'analyse avant le nom indiqué
...
#### Ajouter une statistique après le nom indiqué
...
#### Ajouter une statistique après le premier
...
#### Ajouter une statistique après l'indice
...
**Syntaxe :** Aggregate Statistics( column )

**Description :** Ajoute une colonne séparée pour chaque niveau de la colonne spécifiée ainsi qu&apos;une colonne de sommation à la table de données actuelle. Dans un script, le message Statistiques agrégées doit être inclus soit dans un message Table de données de colonnes, soit dans un message Table de données de lignes.

#### Définir lors de l'ajout à un élément existant
...
#### Définir lors de l'ajout à un élément nouveau
...
**Description :** Utilisé avec Modifier la table de données pour supprimer des colonnes et des statistiques d&apos;une table de données existante.

#### Supprimer une colonne d'analyse nommée
...
#### Supprimer une statistique à l'indice
...
#### Définir la largeur de l'étiquette de ligne
...
#### Redimensionner les colonnes de données à largeurs égales
...
**Description :** Ignore les réponses en doublon dans les colonnes à réponses multiples. Chaque réponse répétée est considérée comme une occurrence unique.
...
#### Transformer en une table de données
...
#### Transformer en une table de données invisible
...
#### Utiliser des noms de colonnes avec chemin complet
...
**Description :** Définissez le nombre maximum de colonnes à verrouiller pour le défilement. Toutes les colonnes de l&apos;en-tête de ligne seront verrouillées, ou aucune colonne. Actif par défaut.

**JMP Version ajoutée :** 19

#### La limite autorise le nombre d'en-têtes
...
#### Limite sous le nombre d'en-têtes
...
**Description :** Spécifie si les valeurs manquantes pour la statistique récapitulative de somme doivent être affichées comme 0 ou manquantes.
...
**Description :** Modifie une table existante.

#### Construire et éditer la table de données complète
...
#### Supprimer une colonne d'analyse
...
**Description :** Utilisé avec Modifier la table de données pour modifier les options de table dans une table de données existante.

#### Empiler les colonnes de regroupement dans l'existante
...
#### Modifier l'étiquette de groupe empilé dans l'existante
...
**Description :** Utilisé avec Modifier la table de données pour déplacer des colonnes et des statistiques dans une table de données existante.

**JMP Version ajoutée :** 19

#### Déplacer la colonne de regroupement de la table de données des colonnes vers la table des lignes
...
#### Déplacer une statistique après le nom indiqué
...
#### Grouper les colonnes d'analyse dans l'existante
...
#### Grouper les colonnes d'analyse dans un nouvel ensemble à l'aide d'un modèle
...
#### Les colonnes d'analyse de conditionnement sont nouvelles
...
**Description :** Utilisé avec Modifier la table de données pour convertir les colonnes d&apos;analyse en colonnes de groupement et vice-versa dans une table de données existante.

**JMP Version ajoutée :** 19

#### Remplacer la colonne d'analyse par une colonne de groupement
...
#### Remplacer la colonne de groupement par une colonne d'analyse
...
**Description :** Définit si les niveaux de groupement doivent être inclus comme balises de colonne dans la table de données. Actif par défaut.

**JMP Version ajoutée :** 19

#### Enregistrer les balises
...
#### Ne pas enregistrer les balises
...
**Description :** Définit si les colonnes contenant les en-têtes de ligne doivent être verrouillées pour le défilement. Actif par défaut.

**JMP Version ajoutée :** 19

#### Ne faites pas défiler les en-têtes de ligne verrouillés
...
#### Verrouiller le défilement des en-têtes de ligne
...
**Syntaxe :** Set Format( statistic( Column( format ) )

**Description :** Définit le format d&apos;affichage des colonnes d&apos;analyse.

#### Formater dans l'existante
...
#### Formater plusieurs statistiques et colonnes d'analyse
...
#### Formater une statistique sans colonne d'analyse
...
#### Formater une statistique unique et une colonne d'analyse
...
**Syntaxe :** Stack Grouping Columns(0 | 1)

**Description :** Empile les colonnes de regroupement en une seule colonne, en utilisant l&apos;indentation pour afficher la structure d&apos;imbrication.
...
**Syntaxe :** Statistics( N|Mean|Std Dev|Min|Max|Range|% of Total|N Missing|N Categories|Sum|Sum Wgt|Variance|Std Err|CV|Median|Interquartile Range|Quantiles|Column %|Row %|All )

**Description :** Ajoute des statistiques à une colonne ou à une ligne de la table de données. Dans un script, le message Statistiques() se trouve à côté du message identificateur Colonnes d&apos;analyse, les deux étant imbriqués dans une commande Table de données de lignes() ou Table de données de colonnes ().
...
**Description :** Dégroupe un groupe de colonnes groupées.
...
**Syntaxe :** obj = Tabulate(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;Élément lanceur : Oui&lt;/b&gt;
