...
**Description :** Spécifie la colonne X durant le lancement.
...
**Syntaxe :** obj &lt;&lt; ARL Profiler( state=0|1 )

**Description :** Affiche ou masque un profileur interactif pour la période opérationnelle moyenne lorsque les paramètres changent.
...
**Syntaxe :** obj = CUSUM Control Chart(...Data Units( state=0|1 )...)

**Description :** Spécifie que les unités de données sont utilisées dans le rapport plutôt que les unités d&apos;écart-type.
...
limitsTable = New Table( "Cusum Limits",
	...
	New Column( "_LimitsKey",
		Character,
		Set Values( {"_H", "_K", "_Std Dev", "_Mean"} )
	),
...
**Syntaxe :** obj &lt;&lt; H( number=5 )

**Description :** Spécifie la valeur du paramètre qui définit les limites. Si l&apos;option Unités des données n&apos;a pas été spécifiée, il s&apos;agit du paramètre h. Si l&apos;option Unités des données a été spécifiée, il s&apos;agit du paramètre H. Notez que H est égal à h fois Sigma. "5" par défaut.
...
**Syntaxe :** obj &lt;&lt; Head Start( number )

**Description :** Spécifie la valeur des sommes cumulées avant le premier échantillon. Le fait de commencer les sommes cumulées à une valeur différente de zéro augmente la sensibilité de la carte CUSUM près du début des échantillons. Ce paramètre est également connu sous le nom de valeur de la réponse initiale rapide (FMR). Par défaut, ce paramètre est défini à 0.
...
**Syntaxe :** obj &lt;&lt; K( number=0.5 )

**Description :** Spécifie la valeur du paramètre qui définit le plus petit changement de la moyenne qu&apos;il est important de détecter. Si l&apos;option Unités des données n&apos;a pas été spécifiée, il s&apos;agit du coefficient k. Si l&apos;option Unités des données a été spécifiée, il s&apos;agit du coefficient K. Notez que K est égal à k fois Sigma. "0.5" par défaut.
...
**Syntaxe :** obj &lt;&lt; Lower Side( state=0|1 )

**Description :** Affiche ou masque les valeurs négatives de la somme cumulée sur le graphique. Actif par défaut.
...
**Description :** Enregistre les paramètres du graphique dans une propriété de colonne ou une nouvelle table de données.



Si in Column est spécifié, la moyenne est enregistrée dans une propriété de colonne Limites de contrôle. 



Si in New Table est spécifié, les coefficients h, k, l&apos;écart-type, la moyenne et le début d&apos;en-tête sont enregistrés dans une nouvelle table de données.
...
**Description :** Crée une nouvelle table de données qui contient les statistiques de chaque sous-groupe dans la carte CUSUM.
...
**Description :** Affiche ou masque un rapport contenant la période opérationnelle moyenne calculée à partir de la carte de CUSUM associée.
...
**Syntaxe :** obj = CUSUM Control Chart(...Show Excluded Region( state=0|1 )...)

**Description :** Spécifie si les sous-groupes entièrement exclus sont affichés sur l&apos;axe horizontal dans la carte de contrôle CUSUM. Applicable uniquement lorsqu&apos;une variable X est spécifiée.
...
**Description :** Spécifie la valeur connue de l&apos;écart-type. Par défaut, ce paramètre est défini à l&apos;étendue mobile moyenne de la colonne Y. S&apos;il y a une variable X, le paramètre Sigma est défini comme l&apos;étendue mobile moyenne des données de résumé.
...
**Description :** Affiche ou masque un cercle rouge autour des points au-dessus de la limite supérieure ou au-dessous de la limite inférieure dans la carte CUSUM.
...
**Syntaxe :** obj &lt;&lt; Tune Chart( &lt;0|1&gt; | Min( value, Max( value ) ) )

**Description :** Affiche ou masque une commande qui vous permet de définir la valeur du paramètre k en spécifiant une étendue acceptable pour la variable Y. Dans un script, vous pouvez également spécifier directement l&apos;étendue acceptable.
...
**Syntaxe :** obj &lt;&lt; Upper Side( state=0|1 )

**Description :** Affiche ou masque les valeurs positives de la somme cumulée sur le graphique. Actif par défaut.
...
**Syntaxe :** obj = CUSUM Control Chart(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;Élément lanceur : Oui&lt;/b&gt;
