...
obj = dt << Manage Limits( Process Variables( dt << Get Column Group( "Processes" ) ), By( _bycol ) );
...
**구문:** obj = Manage Limits(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = Manage Limits( Process Variables( columns), Grouping( columns) )
...
dt = Open( "$SAMPLE_DATA/Semiconductor Capability.jmp" );
obj = dt << Manage Limits( Process Variables( :NPN1, :PNP1, :PNP2, :NPN2 ), Grouping( :site ) );
...
**구문:** obj = Manage Limits( Process Variables( columns) )
...
**설명:** 데이터 테이블에서 규격 한계를 벗어나는 관측값에 해당하는 셀에 색상을 지정합니다. 규격 하한보다 낮은 관측값 셀은 연한 빨간색으로 표시되고 규격 상한보다 높은 관측값 셀은 연한 파란색으로 표시됩니다.
...
**구문:** obj &lt;&lt; Load from Limits Table

**설명:** 데이터 테이블에서 한계 데이터를 불러옵니다.
...
**구문:** obj &lt;&lt; Round Decimals

**설명:** 모든 숫자 값을 지정된 소수 자릿수로 반올림합니다.
...
**구문:** obj &lt;&lt; Save to Column Properties

**설명:** 한계 데이터를 열 특성으로 저장합니다.
...
dt = Open( "$SAMPLE_DATA/Cities.jmp" );
obj = dt << Manage Limits( Process Variables( :OZONE, :CO, :SO2, :NO ), Load From Limits Table( dtLimits ) );
...
**구문:** obj &lt;&lt; Save to Column Properties in Split Table( &lt; Remaining Columns ( Drop All | Keep All | Split All )&gt; )

**설명:** 그룹화 변수에 의해 분할된 새 데이터 테이블에 한계 데이터를 열 특성으로 저장합니다.
...
**구문:** obj &lt;&lt; Save to Tall Limits Table

**설명:** 각 열에 대한 모든 한계 데이터를 포함하는 새 세로형 데이터 테이블을 생성합니다.
...
**구문:** obj &lt;&lt; Save to Wide Limits Table

**설명:** 각 열에 대한 모든 한계 데이터를 포함하는 새 가로형 데이터 테이블을 생성합니다.
...
**구문:** obj &lt;&lt; Show Limits All

**설명:** 모든 항목에 대해 &apos;한계 표시&apos; 체크박스를 선택합니다.
