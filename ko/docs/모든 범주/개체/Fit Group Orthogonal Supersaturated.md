...
Names Default To Here( 1 );
d = DOE( Group Orthogonal Supersaturated Design, Make Design, Simulate Responses( 1 ), By( _bycol ) );
dt = d << make table;
Wait( 1 );
...
**구문:** Fit Group Orthogonal Supersaturated( Y( column ), X( columns ) )

**설명:** 그룹 직교 과포화 설계를 분석합니다. 그룹 직교 과포화 설계는 관측값 수가 모형 모수보다 적은 설계이며 주효과를 모두 동시에 추정할 수는 없습니다.
