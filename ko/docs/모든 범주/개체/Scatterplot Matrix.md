...
obj = dt << Scatterplot Matrix( Y( :Sepal length, :Sepal width, :Petal length, :Petal width ), By( _bycol ) );
...
**구문:** obj = Scatterplot Matrix(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj &lt;&lt; Density Ellipses( state=0|1 )
...
Names Default To Here( 1 );
dt = Open( "$SAMPLE_DATA/Cities.jmp" );
obj = dt << Scatterplot Matrix( Y( :POP, :Max deg. F Jan, :OZONE, :CO, :SO2, :NO, :PM10, :Lead ) );
...
**설명:** 산점도 행렬에 표시되는 타원에 대해 포함 수준을 (1-α)로 설정합니다.
...
**설명:** 산점도 행렬에 표시되는 타원의 투명도를 설정합니다.
...
**설명:** 산점도 행렬에 표시되는 별도의 타원을 생성하는 데 해당 수준이 사용되는 그룹화 변수를 식별합니다.
...
**설명:** 데이터 또는 필터링 변경의 영향으로 변경되지 않도록 축 범위를 고정합니다.
...
**구문:** obj = Scatterplot Matrix(...Matrix Format( "하삼각"|"상삼각"|"정사각" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj &lt;&lt; Nonpar Density( state=0|1 )
...
**설명:** 산점도 행렬에서 점을 조금 이동하여 점을 지터링합니다. 기본값은 "자동"입니다.

```jsl

Names Default To Here( 1 );
dt = Open( "$SAMPLE_DATA/Iris.jmp" );
obj = dt << Scatterplot Matrix( Y( :Sepal length, :Sepal width, :Petal length, :Petal width, Species ) );
...
**구문:** obj &lt;&lt; Shaded Ellipses( state=0|1 )
