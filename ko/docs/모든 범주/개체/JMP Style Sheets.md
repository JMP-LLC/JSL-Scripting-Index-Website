...
**설명:** JMP Style Sheets (JSS) helps us separate DisplayBox content from DisplayBox presentation. The system is modeled on CSS. CSS is to HTML as JSS is to DisplayBox trees in JMP.
...
**구문:** And( selector, ..., selector )

**설명:** Combine two or more selectors, making a new selector that matches only when all of those match. Use the functional form And(...) or wrap in parentheses before sending messages.
...
New Window( "And",
	JSS Context Box(
		H List Box(
			Platform(
				dt,
				Bivariate( Y( :weight ), X( :height ), Fit Line( {Line Color( {212, 73, 88} )} ) )
			),
		...
		<<Set JSS(
			Expr(
				(Type( OutlineBox ) & Class( Platform ) & Class( Oneway )) << Background Color( "Red" )
			)
		)
	)
);
...
**구문:** Child( parentSelector, childSelector )

**설명:** Combines two selectors into a direct parent/child path. A box matches when it and its direct parent both match their individual selectors.
...
New Window( "Child",
	JSS Context Box(
		Outline Box( "Outline",
			Text Box( "Hello" ),
			Panel Box( "Conversation", Text Box( "...A..." ), Text Box( "...B..." ), Text Box( "...C..." ), ),
			Text Box( "Bye" ),

		), 
        // CSS> OutlineBox > TextBox { color: blue; }
		// Set color for all TextBox that are direct children (not just first child) of OutlineBox
		<<Set JSS( Expr( Child( Type( OutlineBox ), Type( TextBox ) ) << Font Color( "Blue" ) ) )
	)
);
...
**설명:** Matches boxes that have the given JSS class (loose comparison).
...
**구문:** Constants( var1 = expr1; var2 = expr2; ... )

**설명:** Defines constants that can be used within rule declarations. Basic operations are allowed in the expressions and the names can reference each other. Names are global within a Theme and the last declaration of a name is used.
...
**구문:** Descend( selector, ..., selector )

**설명:** Combines two or more selectors into a path. A box matches when the path to the box matches. Note that the path can skip levels in the display tree.
	...
	JSS Context Box(
		...
		<<Set JSS(
			Expr(
				...
				Descend( Type( OutlineBox ), Type( PanelBox ), Id( foo ) ) << Font Color( "Red" );
			)
		)
	)
);
...
**설명:** Matches when a box is the first child of its parent.
...
**설명:** Matches boxes that have the given JSS id (loose comparison).
...
**구문:** If( condA, rulesA, ..., rulesElse )

**설명:** The allowable condition expressions are limited and resolved when the JSS is parsed. No branch testing is done during the JSS application passes. In other words, these are for testing the state of the system and JMP, not individual boxes or box trees.
...
New Window( "If Language Is",
	JSS Context Box(
		...
		<<Set JSS( Expr( If( Language Is( "es" ), Type( TextBox ) << Font Color( "Blue " ) ) ) )
	)
);
...
New Window( "If Theme Is",
	JSS Context Box(
		...
		<<Set JSS(
			Expr(
				If( Theme Is( "Dark" ),
					Descend( Class( Graph Builder ), Type( Frame Box ) ) << Background Color( "Light Gray" );
					Descend( Class( Graph Builder ), Type( Legend Box ) ) << Background Color( "Light Gray" );
				)
			)
		)
	)
...
**설명:** Matches when a box is the last child of its parent.
...
**구문:** Next Sibling( precedingSelector, selector )

**설명:** Box must match the second selector and be immediately preceded by a box matching the first selector (sharing the same parent).
...
New Window( "Next Sibling",
	JSS Context Box(
		...
		<<Set JSS( Expr( Next Sibling( Type( PanelBox ), Type( TextBox ) ) << Font Color( "Blue" ) ) )
	)
);
...
**구문:** Or( selector, ..., selector )

**설명:** Combines two or more selectors, making a new selector that matches when any of the individual selectors match. Use the functional form Or(...) or wrap in parentheses before sending messages.
...
**설명:** Matches boxes of the given type (loose comparison).
...
**설명:** Matches all boxes. Useful mainly for clearly expressing intent.
