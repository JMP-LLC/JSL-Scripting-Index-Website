...
**구문:** obj = Normalization(...Window View( "Visible"|"Invisible"|"Private" )...) &lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
dt = Open( "$SAMPLE_DATA/Iris.jmp" );obj = dt << Normalization( Y( :Sepal length, :Sepal width, :Petal length, :Petal width ), By( :species ) );
...
**구문:** obj &lt;&lt; Length( column )

```jsl

dt = Open( "$SAMPLE_DATA/Iris.jmp" );obj = dt << Normalization(	Y( :Sepal length, :Sepal width, :Petal length, :Petal width ),	Method( "Read/Fragment Per Kilobase of Transcript Per Million Mapped Reads/Fragments (RPKM/FPKM)"	),	Length( :Sepal length ));

```

### X (Control) Columns

**구문:** obj &lt;&lt; X (Control) Columns( column(s) )
...
**구문:** Mean Expression Threshold ( number = 0.05 ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;

**설명:** MA 그림의 X 축(A 값) 임계 - TMM|KDMM 방법 기본값은 "0.05"입니다.
...
**구문:** Create New Tables for Results ( state =0|1 ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** Interpolated Quantiles( state =0|1 ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;

**설명:** TMM|KDMM 방법에서 참조 열을 결정하기 위한 보간 분위수
...
**구문:** Log Transformation( state =0|1 ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;

**설명:** TMM|KDMM 방법의 로그 변환
...
**구문:** Logratio Threshold ( number = 0.3 ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;

**설명:** MA 그림의 Y 축(M 값) 임계 - TMM|KDMM 방법 기본값은 "0.3"입니다.
...
**구문:** Method( "Row Standardize | Relative Abundance Percentage (RAP) | Relative Abundance Ratio (RAR) | Centered Log Ratio (CLR) | Count/Read Per Million (CPM/RPM) | Read/Fragment Per Kilobase of Transcript Per Million Mapped Reads/Fragments (RPKM/FPKM) | Transcript Per Million (TPM) | Trimmed Mean of M-Values (TMM) | Kernel Density Mean of M-Component (KDMM)" ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**설명:** 다변량 SVD(특이값 분해) 기법을 사용하여 데이터의 결측값을 대치하도록 지정합니다.
...
**구문:** Total Prior Count ( number = 0.25 ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;

**설명:** 라이브러리 크기 조정을 위해 모든 샘플에 적용되는 사전 카운트 - TMM|KDMM 방법 기본값은 "0.25"입니다.
...
**구문:** Replace Y Data( state =0|1 ) &lt;b&gt;실행기 항목: 예&lt;/b&gt;

**설명:** 데이터 테이블에서 Y 변수의 데이터를 바꾸도록 지정합니다.
