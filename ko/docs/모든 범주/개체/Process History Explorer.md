...
**구문:** obj = Process History Explorer(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**구문:** obj = Process History Explorer(...Goal is to Minimize Y( state=0|1 )...)

**설명:** 분석 목표가 반응을 최소화하는 것임을 지정합니다.
...
**구문:** obj << Levels with Lowest Yield( state=0|1 )

**설명:** 각 X, 공정 변수의 각 수준에 대한 수율 요약 통계량 테이블을 표시하거나 숨깁니다.
...
**구문:** obj << Levels with Lowest Yield with Time Filter( state=0|1 )

**설명:** 수율이 가장 낮은 상위 50개 수준의 시간 구간에 대한 요약 통계량 테이블을 표시하거나 숨깁니다.
...
**구문:** obj << Order Interaction

**설명:** 순서 열을 기준으로 수율에 대한 일원 분석 보고서를 표시하거나 숨깁니다.

**JMP추가된 버전:** 14

### Save Count Table

**구문:** obj << Save Count Table

**설명:** X, 공정 변수의 각 수준을 거친 유닛 수가 포함된 새 데이터 테이블을 저장합니다.
...
**구문:** obj << Save Waiting Time

**설명:** 대기 시간 분석 보고서의 각 테이블을 별도의 데이터 테이블에 저장합니다.
...
**구문:** obj << Stepwise Regression( Goal( "Biggest Individual Difference" | "Biggest Total Difference" | "Most Predictive"), Time Filtering( "None" | "Starting Time"), X Transform( "None" | "Log(Count+1)"), N Steps( number ) )

**설명:** 지정된 옵션을 기반으로 단계별 회귀 분석을 수행합니다. 각 회귀 단계에서 항이 모형에 입력된 순서를 보여 주는 테이블이 제공됩니다.
...
**구문:** obj << Transition Analysis( columns, {"level1", "level2", ... } )

**설명:** 결과 수율에 문제가 되는 성분 간 전환을 식별하는 데 유용한 보고서를 표시합니다.
...
**구문:** obj << Waiting Time Analysis

**설명:** 각 ID에 대한 대기 시간 요약 통계량 테이블을 제공합니다. 이 테이블에서 대기 시간은 ID가 한 단계에서 완료된 후 다른 단계를 시작할 때까지의 시간입니다.
...
**구문:** Yield Columns( columns )

**설명:** 수율 값을 포함하는 수율 데이터 테이블의 열을 지정합니다.
...
**구문:** Yield Table( data table )

**설명:** 각 ID의 수율이 포함된 데이터 테이블을 지정합니다.
