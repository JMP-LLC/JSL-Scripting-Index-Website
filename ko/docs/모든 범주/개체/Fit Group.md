...
obj = dt << Fit Group(
	Fit Model(
		Y( :ABRASION ),
		Effects(
			:SILICA & RS, :SILANE & RS, :SULFUR & RS, :SILICA * :SILICA, :SILICA * :SILANE, :SILANE * :SILANE,
			:SILICA * :SULFUR, :SILANE * :SULFUR, :SULFUR * :SULFUR
		),
...
**구문:** Fit Group( model1, model2, ... );Fit Group( model1; model2; ... )

**설명:** 공유 프로파일러를 사용하여 동일한 출력 창에 별도의 수행된 최소 제곱, 비선형, 신경망, 가우시안 공정 및 혼합 모형 적합을 그룹화합니다.
...
**설명:** 플랫폼 분석 보고서를 지정된 수의 행으로 재배열합니다.
...
**설명:** 모든 반응에 대한 등고선 프로파일러를 표시하거나 숨깁니다. 모든 반응에 대한 모형 항의 효과를 동시에 탐색할 수 있습니다.
...
**구문:** obj &lt;&lt; Order by Goodness of Fit

**설명:** 각 모형의 R² 통계량으로 측정되는 적합 유의성을 기준으로 보고서를 정렬합니다. 이 옵션은 R² 통계량이 플랫폼 수준으로 표시되는 플랫폼에만 적용할 수 있습니다.
...
**설명:** 모든 반응에 대한 예측 프로파일러를 표시하거나 숨깁니다. 모든 반응에 대한 모형 항의 효과를 동시에 볼 수 있습니다. 반응 간에 여러 최적화를 수행할 수도 있습니다.
...
**설명:** 각 반응에 대한 개별 표면 프로파일러 보고서를 표시하거나 숨깁니다.
