...
**구문:** obj = CUSUM Control Chart(...Window View( "Visible"|"Invisible"|"Private" )...) &lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj &lt;&lt; ARL Profiler( state=0|1 )

**설명:** 모수가 변경되면 평균 런 길이에 대한 대화식 프로파일러를 표시하거나 숨깁니다.
...
**구문:** obj = CUSUM Control Chart(...Data Units( state=0|1 )...)

**설명:** 표준편차 단위 대신 데이터 단위를 보고서에 사용하도록 지정합니다.
...
**설명:** 한계를 정의하는 모수의 값을 지정합니다. &apos;데이터 단위&apos; 옵션을 지정하지 않은 경우 h 모수이고, &apos;데이터 단위&apos; 옵션을 지정한 경우 H 모수입니다. H는 h에 시그마를 곱한 값입니다. 기본값은 "5"입니다.
...
**구문:** obj &lt;&lt; Head Start( number )

**설명:** 첫 번째 표본 이전의 누적합 값을 지정합니다. 0이 아닌 값에서 누적합을 시작하면 표본 시작 근처에서 CUSUM 차트의 민감도가 증가합니다. 이 모수는 FIR(Fast Initial Response) 값이라고도 합니다. 기본적으로 이 모수는 0으로 설정됩니다.
...
**설명:** 감지할 최소 평균 변화를 정의하는 모수의 값을 지정합니다. &apos;데이터 단위&apos; 옵션을 지정하지 않은 경우 k 모수이고, &apos;데이터 단위&apos; 옵션을 지정한 경우 K 모수입니다. K는 k에 시그마를 곱한 값입니다. 기본값은 "0.5"입니다.
...
**구문:** obj &lt;&lt; Lower Side( state=0|1 )

**설명:** 차트에 누적합에 대한 음수 값을 표시하거나 숨깁니다. 기본적으로 설정되어 있습니다.
...
in Column를 지정하면 평균이 관리 한계 열 특성에 저장됩니다. 



in New Table를 지정하면 h, k, 표준편차, 평균 및 시작값 모수가 새 데이터 테이블에 저장됩니다.
...
**설명:** CUSUM 차트의 각 부분군에 대한 통계량을 포함하는 새 데이터 테이블을 생성합니다.
...
**설명:** 관련 CUSUM 차트에서 계산된 평균 런 길이가 포함된 보고서를 표시하거나 숨깁니다.
...
**구문:** obj = CUSUM Control Chart(...Show Excluded Region( state=0|1 )...)

**설명:** 완전히 제외된 부분군을 CUSUM 관리도의 가로 축에 표시할지 여부를 지정합니다. X 변수가 지정된 경우에만 적용할 수 있습니다.
...
**설명:** 알려진 표준편차 값을 지정합니다. 기본적으로 이 모수는 Y 열의 평균 이동 범위로 설정됩니다. X 변수가 있는 경우 시그마 모수는 요약 데이터의 평균 이동 범위로 설정됩니다.
...
**설명:** CUSUM 차트에서 상한선 위에 있거나 하한선 아래에 있는 모든 점 주위에 빨간색 원을 표시하거나 숨깁니다.
...
**구문:** obj &lt;&lt; Tune Chart( &lt;0|1&gt; | Min( value, Max( value ) ) )

**설명:** Y 변수에 대한 허용 범위를 지정하여 k 모수의 값을 설정할 수 있는 컨트롤을 표시하거나 숨깁니다. 스크립트에 허용 범위를 직접 지정할 수도 있습니다.
...
**구문:** obj &lt;&lt; Upper Side( state=0|1 )

**설명:** 차트에 누적합에 대한 양수 값을 표시하거나 숨깁니다. 기본적으로 설정되어 있습니다.
