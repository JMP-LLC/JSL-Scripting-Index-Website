...
dt << Treemap( Categories( :State ), Coloring( :"2004 Verbal"n ), Sizes( :Population ), Ordering( :X, :Y ) );
...
**구문:** obj = Treemap(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** Treemap( Categories( column ), &lt;Sizes( column )&gt;, &lt;Coloring( column )&gt; )

**설명:** 명목형 또는 순서형 변수에 대한 직사각형 바둑판식 배열을 생성하는 그래프 빌더 방법입니다. 바둑판식 범주 크기는 선택한 변수 내의 범주 수에 비례합니다.
...
**구문:** obj &lt;&lt; Change Color Column( column )

**설명:** 범주에 색상을 적용하는 데 사용되는 열을 변경합니다.
...
**구문:** obj &lt;&lt; Color Range( low, high )

**설명:** 색상 그래디언트에 대한 범위를 설정합니다. 기본값은 색상 열의 low=min 및 high=max입니다.
...
If( prefVal == 1,
	...
	report << Dispatch( {}, "400", ScaleBox,
		{Legend Model( 1, Properties( 0, {gradient( {Scale Values( [1 20] )} )}, Item ID( "CO", 1 ) ) )}
	);
,
			...
			Properties(
				...
				{gradient(
					{Color Theme(
						...
						HLS Color( 0 / 360, .5, 1 ), Missing( "Black" )}}
					)}
				)},
				Item ID( "POP", 1 )
			)
...
**구문:** obj = Treemap(...Layout( "분할"|"정사각형"|"혼합" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;

### Legend

**구문:** obj &lt;&lt; Legend( state=0|1 )

**설명:** 실제 색상에 매핑된 색상 열의 값을 보여 주는 범례를 표시하거나 숨깁니다.
