...
**구문:** obj = Marker Statistics(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**구문:** obj = Marker Statistics(...Marker Format( "숫자"|"단일 문자"|"문자"|"단일 코드 핵산"|"핵산"="숫자" )...)

<b>실행기 항목: 예</b>

**설명:** 유전자형 형식을 선택하는 옵션을 제공합니다. 옵션에는 단일 문자(A, B, H), 단일 코드 핵산(IUPAC: A,C,G,T,R,Y,S,W,K,M,+,0,-,N), 핵산(AA, CC, GG, TT, AC, AT 등), 숫자(0, 1, 2, ..., p) 및 문자(ApBp 또는 Ap/Bp)가 포함됩니다. 숫자 형식과 문자 형식은 p-배수성을 가진 유전자형을 처리합니다. 기본값은 "숫자"입니다.
...
**구문:** obj = Marker Statistics(...Ploidy( number=2 )...)
...
**구문:** obj = Marker Statistics(...Unthreaded( state=0|1 )...)
...
**구문:** obj = Marker Statistics(...Use Annotation Table( state=0|1 )...)
