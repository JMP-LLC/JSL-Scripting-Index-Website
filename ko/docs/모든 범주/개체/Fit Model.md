...
**설명:** 일부 모수 값을 수정하고 분석을 다시 실행할 수 있습니다. 이 옵션은 IPWR 및 AIPW 모형에만 사용할 수 있습니다.
...
**설명:** Shows or hides a chart that contains a point for each level of the effect other than the control. Each point shows the least squares mean for that level in relation to the least squares mean for the control level. Upper decision limits (UDL) and lower decision limits (LDL) are plotted.
