...
**설명:** 모형의 효과에 대한 처리 수준을 통계적으로 대조하기 위한 사용자 정의 F-검정을 실행합니다. 각 대조를 행 벡터로 지정하고 효과 이름을 문자열로 지정하십시오.
...
**설명:** 모형의 여러 효과를 대조하는 사용자 정의 F-검정을 실행합니다.
...
**설명:** 효과의 여러 수준을 통계적으로 대조하기 위한 사용자 정의 F-검정을 실행합니다.
...
**설명:** 대조 수준을 제외한 각 효과 수준의 점을 포함하는 차트를 표시하거나 숨깁니다. 각 점은 대조 수준의 최소 제곱 평균과 비교한 해당 수준의 최소 제곱 평균을 나타냅니다. UDL(결정 상한)과 LDL(결정 하한)도 표시됩니다.
...
**설명:** 슬라이스 효과 F-검정을 표시하거나 숨깁니다. 기본적으로 설정되어 있습니다.
...
**구문:** obj = Summary of Fit(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 조건부 예측의 개별값에 대한 신뢰 구간이 포함됩니다. 신뢰 구간은 임의 효과가 있는 모형의 임의 효과 추정값을 포함합니다. 이 옵션은 REML 분석 방법에만 사용할 수 있습니다. Shift 키를 누른 채 유의 수준 또는 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 조건부 예측의 기대값에 대한 신뢰 구간이 포함됩니다. 신뢰 구간은 임의 효과가 있는 모형의 임의 효과 추정값을 포함합니다. 이 옵션은 REML 분석 방법에만 사용할 수 있습니다. Shift 키를 누른 채 유의 수준 또는 접미사를 입력합니다.
...
**설명:** 새 계산식 열을 데이터 테이블에 저장합니다. 새 열에는 임의 효과가 있는 모형의 임의 효과 추정값을 사용하는 계산식이 포함됩니다. 이 옵션은 REML 분석 방법에만 사용할 수 있습니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 임의 효과의 BLUP(최량 선형 비편향 예측 변수) 계수를 사용하여 계산된 조건부 예측값이 포함됩니다. 이 옵션은 REML 분석 방법에만 사용할 수 있습니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 조건부 예측의 잔차가 포함됩니다. 잔차 값은 임의 효과가 있는 모형의 임의 효과 추정값을 포함합니다. 이 옵션은 REML 분석 방법에만 사용할 수 있습니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 각 관측값이 모형 추정에 미치는 영향을 나타내는 측도가 포함됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 효과 레버리지 그림에 표시되는 X 및 Y 값이 포함됩니다. Y 값은 편잔차이고 X 값은 효과 레버리지 그림의 회귀변수 축소입니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 외부 스튜던트화 잔차(잔차를 현재 행을 제외한 표준 오차 추정값으로 나눈 값)가 포함됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 xInv(x`x)x` 행렬의 대각 값이 포함됩니다. 이러한 값을 해트 값 또는 레버리지 값이라고도 합니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 반응의 개별 실현값에 대한 신뢰 구간의 한계가 포함됩니다. 이 값은 반응의 변동과 추정의 변동을 모두 포함합니다. Shift 키를 누른 채 유의 수준 또는 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 기대값에 대한 신뢰 구간의 한계가 포함됩니다. 이 값은 추정의 변동을 포함하지만 반응의 변동은 포함하지 않습니다. Shift 키를 누른 채 유의 수준 또는 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 적합 모형에 대한 예측값이 포함됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 계산식 열을 데이터 테이블에 저장합니다. 새 열에는 적합 모형에 대한 예측 계산식이 포함됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 데이터 테이블에 새 열을 저장합니다. 이 열에는 예측, 신뢰 한계 및 예측 한계에 대한 계산식이 포함됩니다. 이 옵션으로 생성된 한계 열에는 예측 프로파일러에 사용되는 특성이 포함됩니다. Shift 키를 누른 채 유의 수준 또는 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 적합 모형에 대한 잔차 값이 포함됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**구문:** obj &lt;&lt; Show CV( state=0|1 )

**설명:** REML 분산 성분 추정값 보고서에 변동 계수 열을 표시하거나 숨깁니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 개별 예측값의 표준 오차가 포함됩니다. 이 값은 개별값 신뢰 구간을 계산하는 데 사용됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 예측값의 표준 오차가 포함됩니다. 이 값은 평균 신뢰 구간을 계산하는 데 사용됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 잔차 값의 표준 오차가 포함됩니다. 이 값은 스튜던트화 잔차를 계산하는 데 사용됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 계산식 열을 데이터 테이블에 저장합니다. 새 열에는 예측값의 표준 오차 계산식이 회귀변수의 함수로 포함됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**설명:** 새 열을 데이터 테이블에 저장합니다. 새 열에는 스튜던트화 잔차(잔차를 표준 오차로 나눈 값)가 포함됩니다. Shift 키를 누른 채 접미사를 입력합니다.
...
**구문:** obj = Fit Least Squares(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
obj = dt << Fit Model(
	Y( :HARDNESS ),
	Effects(
		:SILICA, :SILANE, :SULFUR, :SILICA * :SILICA, :SILANE * :SILICA, :SILANE * :SILANE, :SULFUR * :SILICA,
...
obj = dt << Fit Model(
	Y( :Y ),
	Effects(
		:F, :Ct, :A, :T, :Cn, :F * :Ct, :F * :A, :F * :T, :F * :Cn, :Ct * :A, :Ct * :T, :Ct * :Cn, :A * :T,
		:A * :Cn, :T * :Cn
	),
	Personality( "Standard Least Squares" ),
...
**설명:** 모형의 효과에 대한 처리 수준을 통계적으로 대조하기 위한 사용자 정의 F-검정을 실행합니다. 대조를 벡터 인수로 지정하십시오. 절편 항은 Effect[0]로 지정됩니다.
...
**구문:** Fit Model( Y( columns ), Effects( columns ), Random Effects( columns ), Repeated Effects( columns ), Repeated Structure( type ), Personality( "Mixed Model" ) )
...
**설명:** 보고서 전체에서 표준 오차를 비수정 추정값 및 군간-군내 기반 자유도로 바꿉니다. 다중 비교 보고서에서 군간-군내 자유도를 사용하려면 다중 비교 보고서를 추가하기 전에 이 옵션을 선택해야 합니다.
...
**설명:** 보고서 전체에서 표준 오차를 비수정 추정값 및 포함 기반 자유도로 바꿉니다. 다중 비교 보고서에서 포함 자유도를 사용하려면 다중 비교 보고서를 추가하기 전에 이 옵션을 선택해야 합니다.
...
**설명:** 모형 모수에 대한 프로필-가능도(1 - fraction)% 신뢰 구간을 표시하거나 숨깁니다. fraction 인수는 플랫폼 시작 창에 설정된 유의 수준을 재정의합니다.
...
**설명:** 모수 추정값 테이블에 표시되는 신뢰 구간 유형을 지정합니다. 이 옵션을 선택하면 프로필 가능도 신뢰 구간이 나타나고 그렇지 않으면 Wald 구간이 나타납니다. 이 옵션은 프로필 가능도 신뢰 구간의 계산 시간이 길지 않을 때 기본적으로 설정됩니다.
...
**설명:** 모든 슬라이스 최소 제곱 평균 차이가 포함된 새 데이터 테이블을 생성합니다.
...
**설명:** 주효과와 2요인 교호작용/3요인 교호작용 슬라이스 효과에 대한 모든 최소 제곱 평균을 비교하는 검정을 계산합니다.
...
**구문:** obj = Fit Stepwise(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
obj = dt << Fit Model(
	Y( :Dissolution ),
	Effects(
		:Mill Time, :Screen Size, :Mag. Stearate Supplier, :Lactose Supplier, :Sugar Supplier, :Talc Supplier,
		:Blend Time, :Blend Speed, :Compressor, :Force, :Coating Supplier, :Coating Viscosity, :Inlet Temp,
		:Exhaust Temp, :Spray Rate, :Atomizer Pressure
	),
	Personality( "Stepwise" ),
	Run
);
...
**구문:** Fit Model( Y( columns ), Effects( columns ), Random Effects( columns ), Repeated Effects( columns ), Repeated Structure( type ), Personality( "Generalized Linear Mixed Model" ) )

**설명:** 일반화 선형 혼합 모형을 적합시킵니다. 반응이 가우시안 분포를 따르지 않을 때 랜덤 계수, 분할구 및 블록 설계에 이러한 모형을 사용할 수 있습니다. 반응 분포는 연속형, 범주형, 개수 및 사건 발생 시간 반응 데이터를 수용할 수 있습니다.
...
**설명:** 반응 변수에 대한 기준 비례 위험 함수의 생존 함수와 위험 함수를 보여 주는 &apos;기준 생존율 및 위험 함수&apos; 그림을 표시하거나 숨깁니다. 그림 아래에는 표시된 값이 포함된 테이블이 있습니다. 이 옵션은 Cox 비례 위험 분포가 지정된 경우에만 사용할 수 있습니다.
...
**설명:** 새 계산식 열을 원래 데이터 테이블에 저장합니다. 새 열에는 관측된(비표준화) 데이터 값이 주어진 경우 예측 계산식이 포함됩니다. 예측 계산식은 0인 항을 포함하지 않습니다.
...
obj = dt << Fit Model(
	Y( :Y ),
	Effects(
		Factorial To Degree( :Age, :Gender, :BMI, :BP, :Total Cholesterol, :LDL, :HDL, :TCH, :LTG, :Glucose )
	),
	Personality( "Generalized Regression" ),
	Generalized Distribution( "Normal" ),
	Run(
...
obj = dt << Fit Model(
	Y( :Percent Reacted ),
	Effects(
		:Feed Rate, :Catalyst, :Stir Rate, :Temperature, :Concentration, :Feed Rate * :Catalyst,
		:Feed Rate * :Stir Rate, :Feed Rate * :Temperature, :Feed Rate * :Concentration,
		:Catalyst * :Stir Rate, :Catalyst * :Temperature, :Catalyst * :Concentration,
...
obj = dt << Fit Model(
	...
	Personality( "Generalized Regression" ),
	Generalized Distribution( "Normal" ),
	Run( Fit( Estimation Method( SVEM Forward Selection( Samples( 200 ) ) ), Profiler( 1 ) ) )
...
obj = dt << Fit Model(
	...
	Personality( "Generalized Regression" ),
	Generalized Distribution( "Binomial" ),
	Run( Fit( Estimation Method( "Elastic Net" ), Validation Method( "AICc" ), Early Stopping( 1 ) ) )
);
...
**구문:** obj = Fit Model(...Run( Fit( Estimation Method( method ) ) )... )

**설명:** 모형 적합에 사용되는 추정 방법 또는 변수 선택을 지정합니다.
...
fm = dt << Fit Model(
	...
	Personality( "Generalized Regression" ),
	Generalized Distribution( "Normal" ),
	Run( Fit( Estimation Method( "Forward Selection" ), Validation Method( "AICc" ), Profiler ) )
);
...
**구문:** obj = Fit Model(...Generalized Distribution( distribution )... )

**설명:** 예측 변수 열이 주어졌을 때 모형 적합에 사용되는 반응 열의 분포를 지정합니다.
...
fm = dt << Fit Model(
	...
	Effects(
		...
		:Culmen Length * :Culmen Depth, :Culmen Length * :Flipper Length, :Culmen Length * :Body Mass,
		:Culmen Length * :Sex, :Culmen Depth * :Flipper Length, :Culmen Depth * :Body Mass,
		:Culmen Depth * :Sex, :Flipper Length * :Body Mass, :Flipper Length * :Sex, :Body Mass * :Sex
	),
	Personality( "Generalized Regression" ),
	Generalized Distribution( "Multinomial" ),
	Run( Fit( Estimation Method( "Elastic Net" ), Validation Method( "AICc" ), Confusion Matrix ) )
);
...
**구문:** obj &lt;&lt; Validation Method( "K 폴드"|"홀드백"|"Leave-One-Out"|"BIC"|"AICc"|"ERIC"|"없음"|"검증 열" )

**설명:** 모델링에 사용되는 검증 방법을 지정합니다.
