...
dt = Open( "$SAMPLE_DATA/Big Class.jmp" );
obj = dt << Bivariate( X( :height ), Y( :weight ), By( :age ), Group Options( Return Group( 1 ) ) );
...
**구문:** obj = Group Options(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = Platform( &lt;Roles&gt;, &lt;Platform Options&gt;, Group Options(&lt;Return Group(0|1)&gt;, &lt;options&gt;))

**설명:** 그룹 플랫폼은 기준 역할 또는 여러 Y 역할을 사용하여 시작할 때 생성되는 플랫폼에 대한 보고서를 배열합니다. 그룹 플랫폼을 직접 생성할 수는 없지만 시작 시 여러 플랫폼이 생성될 수 있을 때 생성됩니다. 그룹 동작을 위한 옵션은 시작할 때 Group Options 인수를 사용하여 지정됩니다.
...
**구문:** obj &lt;&lt; Auto Broadcast( state=0|1 )

**설명:** 보고서의 대화식 작업에 대해 브로드캐스트를 자동으로 적용합니다.
...
**구문:** obj &lt;&lt; Get Auto Broadcast

**설명:** 보고서의 대화식 작업에 대해 브로드캐스트를 자동으로 적용하기 위한 설정을 반환합니다.
...
**구문:** obj &lt;&lt; Get Hide Empty Platforms

**설명:** 빈 플랫폼을 숨기기 위한 설정을 반환합니다.
...
**설명:** Arrange in Rows 레이아웃을 위한 플랫폼 수를 반환합니다.
...
**구문:** list = Get Platforms(&lt;Visible Only(0|1)&gt;)

**설명:** 그룹의 플랫폼 목록을 반환합니다. VisibleOnly 옵션을 지정하면 숨겨진 플랫폼이 포함되지 않으며 표시된 순서대로 목록이 정렬됩니다.
...
**구문:** obj &lt;&lt; Hide Empty Platforms( state=0|1 )

**설명:** 계산할 수 없는 빈 플랫폼을 숨깁니다.
...
**구문:** obj &lt;&lt; Layout( "세로 목록"|"가로 목록"|"행으로 배열"|"탭으로 배열" )

**설명:** 그룹의 플랫폼 레이아웃을 변경합니다.
...
**구문:** obj &lt;&lt; N Across( number )

**설명:** Arrange in Rows 레이아웃을 위한 플랫폼 수를 설정합니다.
...
**구문:** Order By(&lt;mode&gt;, &lt;Descending(0|1)&gt;, &lt;Order Statistic("Count" | "Mean" | "Median" | "Mode" | "Geometric Mean" | "Min" | "Max" | "Range" | "Sum" | "Std Dev" | "Variance" | "Std Err" | "CV" | "Interquartile Range" | "Median Absolute Deviation" | "Q10%" | "Q1" | "Q3" | "Q90%")&gt;

**설명:** 그룹 내 플랫폼의 순서를 설정합니다. 순서는 플랫폼에서 계산된 값, 열 통계량(기준 그룹과 함께 사용할 경우) 또는 반응 변수 통계량(여러 Y를 사용하여 그룹을 시작한 경우)을 기반으로 할 수 있습니다. 기본적으로 "평균" 통계량을 사용한 내림차순으로 정렬됩니다.
...
dt = Open( "$SAMPLE_DATA/Cities.jmp" );
obj = dt << Contour Plot( X( :Longitude, :Latitude ), Y( :OZONE, :CO ), Group Options( Return Group( 1 ) ) );
Wait( 1 );
