...
obj = dt << Predictor Screening( Y( :Banding? ), X( Column Group( "Predictors" ) ), By( _bycol ) );
...
**구문:** obj = Predictor Screening(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = Predictor Screening(...&lt;By( column(s) )&gt;...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = Predictor Screening(...Response( column(s) )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = Predictor Screening(...X( column(s) )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = Predictor Screening(...Y( column(s) )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = Predictor Screening(...Number Trees( number=100 )...)

**설명:** 붓스트랩 포레스트 모형의 의사 결정 나무 수를 지정합니다. 기본값은 "100"입니다.
...
dt = Open( "$SAMPLE_DATA/Bands Data.jmp" );
obj = dt << Predictor Screening( Y( :Banding? ), X( Column Group( "Predictors" ) ), Number Trees( 200 ) );
...
**설명:** 보고서의 예측 변수 순위 테이블을 포함하는 새 데이터 테이블을 생성합니다.
...
**구문:** obj = Predictor Screening(...Set Random Seed( number )...)

**설명:** 적합 절차에 사용되는 시작 값의 시드를 설정합니다.
...
dt = Open( "$SAMPLE_DATA/Bands Data.jmp" );
obj = dt << Predictor Screening( Y( :Banding? ), X( Column Group( "Predictors" ) ), Set Random Seed( 1047 ) );
