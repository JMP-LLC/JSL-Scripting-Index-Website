...
**설명:** 기술적 편향을 수정하고 후속 분석의 적합성을 향상시킵니다.
...
#### XBar-R 관리도 지표를 사용한 여러 공정 변수 선별
...
#### XBar-S 관리도 지표를 사용한 여러 공정 변수 선별
...
#### 개별값 및 이동 범위 관리도 지표를 사용한 여러 공정 변수 선별
...
dt = Open( "$SAMPLE_DATA/Semiconductor Capability.jmp" );
obj = dt << Process Screening( Process Variables( Column Group( "Processes" ) ), Grouping( :Site ) );
...
#### 삼원 차트(XBar MR 및 R)를 사용한 공정 변수 선별
...
#### 삼원 차트(XBar MR 및 S)를 사용한 공정 변수 선별
...
#### 환경 모니터링을 위한 경보 그래프를 사용한 개수(Count) 공정 변수 선별
...
#### 환경 모니터링을 위한 비음 연속형 데이터 공정 변수 선별
...
#### 4개 반응과 26개 예측 변수를 사용한 반응 변수 선별
...
#### 네 개의 각 예측 변수에 대한 여러 열 반응 변수 선별
...
dt = Open( "$Sample_Data/Life Sciences/Genotypes Pedigree.jmp" );
obj = dt << Response Screening( Y( Column Group( "Markers" ) ), X( :Trait1, :Trait2, :Trait3, :Trait4 ) );

```

#### 로버스트 적합을 사용한 반응 변수 선별
...
obj = dt << Response Screening( Y( Column Group( "Responses" ) ), X( :Process ), Robust( 1 ) );
...
#### 평균 차이 화산 그림을 사용한 여러 열 반응 변수 선별
...
obj = dt << Structural Equation Models(
	...
	Fit(
		...
		Loadings(
			...
			{"QuadSlope", {:Multiple Choice Year1, :Multiple Choice Year2, :Multiple Choice Year3,
			:Multiple Choice Year4}, {0, 1, 4, 9}}
		),
		Variances(
...
dt << Structural Equation Models(
	...
	Fit(
		...
		Variances( {:Leadership_Avg, {:Leadership_Avg}}, {:Satisfaction_Avg, {:Satisfaction_Avg}} )
	)
);
...
dt << Structural Equation Models(
	Model Variables(
		:Support_L, :Goal_L, :Work_L, :Interact_L, :Person_C, :Intra_C, :Inter_C, :General_S, :Growth_S,
		:Coworker_S, :Supervisor_S
	),
	Fit(
		Model Name( "Higher Order CFA" ),
		New Latent( "Leadership", "Conflict", "Satisfaction", "General" ),
		Means(
			{"Constant", {:Support_L, :Goal_L, :Work_L, :Interact_L, :Person_C, :Intra_C, :Inter_C,
			:General_S, :Growth_S, :Coworker_S, :Supervisor_S}}
		),
		Loadings(
...
dt = Open( "$SAMPLE_DATA/Variability Data/2 Factors Nested.jmp" );
obj = dt << Variability Chart( Y( :Y ), Model( "Nested" ), X( :Operator, :Part ), Variance Components( 1 ) );
...
링크 ID 열 특성은 보조 테이블의 열을 ID 열로 표시합니다.



링크 참조 열 특성은 주 테이블의 열을 보조 테이블의 ID 열에 매핑합니다.

링크 참조 특성을 사용하면 데이터 테이블 참조나 연결할 데이터 테이블의 경로를 설정할 수 있습니다.
...
**설명:** 테이블에서 활성 열 필터를 조작하기 위한 개체를 가져옵니다.
...
**설명:** 이름으로 필터 보기를 가져오거나 <<Temporary 또는 <<Unfiltered를 사용하여 특수 필터 보기 중 하나를 가져옵니다. 지정된 이름의 필터 보기가 없으면 Empty()를 반환합니다.
...
**설명:** 소스 테이블의 데이터를 참조된 열에 병합하고 연결을 끊어 테이블을 독립 실행형으로 설정합니다. 참조하는 열의 링크 참조 특성도 제거됩니다.
...
**설명:** 데이터 테이블을 지원되는 형식으로 저장합니다. 가능한 형식에는 .jmp, .xls, .xlsx, .txt, .csv, .tsv, .xpt, .v8xpt, .stx, .sqlite, .db, .sqlite3 및 .db3가 포함되며 일부 형식은 Windows에서만 지원됩니다. 자세한 내용은 "JMP 사용" 설명서를 참조하십시오.
...
**설명:** 현재 필터 보기에서 열의 필터 아이콘을 표시하거나 숨깁니다.
...
**설명:** 데이터 테이블 표시에서 머리글 그래프를 표시하거나 숨깁니다.
...
**설명:** 데이터 테이블 표시에서 열 그룹을 표시하거나 숨깁니다.
...
**설명:** 데이터 테이블 표시에서 머리글 통계량을 표시하거나 숨깁니다.
...
**설명:** 데이터 테이블 표시에서 열 태그를 표시하거나 숨깁니다.
