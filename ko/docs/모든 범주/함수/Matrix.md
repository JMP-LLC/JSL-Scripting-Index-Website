...
**설명:** 지정된 웨이블릿 모형에 대해 점 x에서의 예측을 반환합니다. grid 모수는 웨이블릿 모형의 데이터 격자를 지정하는 벡터입니다. coef 모수는 웨이블릿 계수의 벡터이고, wavelet 모수는 웨이블릿 모형의 이름입니다. 선택적 param 모수는 웨이블릿 모형 모수(필요한 경우, 기본값 0)입니다.
