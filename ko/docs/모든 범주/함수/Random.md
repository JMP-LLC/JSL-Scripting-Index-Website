...
**구문:** points = Random Linearly Constrained Uniform( numSamples, A, b, L, U, neq, nle, nge, &lt;nwarm=200&gt;, &lt;nstride=25&gt;, &lt;tol=1e-8&gt;, &lt;G&gt;, &lt;LC&gt;, &lt;UC&gt; )

**설명:** 지정된 구성 요소 부분군 변수에 대한 선형 제약 조건, 변수 경계 제약 조건 및 카디널리티 제약 조건을 따르는 랜덤 표본을 생성합니다. numSamples 인수는 생성할 랜덤 데이터 점의 수를 지정합니다. A 인수는 선형 제약 조건 계수 행렬입니다. b 인수는 선형 제약 조건의 우변 값 벡터입니다. L 및 U 인수는 각각 변수의 하한 및 상한 벡터입니다. neq, nle, nge 인수는 각각 등식 제약 조건, 작거나 같음 제약 조건 및 크거나 같음 제약 조건의 수입니다. nwarm 인수는 점이 출력 행렬에 기록되기 전의 워밍업 반복 횟수입니다. nstride 인수는 출력 행렬에 기록되는 각 점 사이의 반복 횟수입니다. tol 인수는 공차입니다. G 인수는 변수를 제약된 구성 요소 부분군에 할당하는 인덱스 벡터로, 결측값 또는 0 값은 제약된 부분군에 속하지 않습니다. LC 및 UC 인수는 각각 제약된 구성 요소 부분군의 하한 및 상한 카디널리티 제약 조건입니다. 제약 조건은 등식 조건, 작거나 같음 조건, 크거나 같음 조건의 순서로 나열되어야 합니다.
...
UC = [3 5];
points = Random Linearly Constrained Uniform( 100, A, b, L, U, 1, 0, 0, nwarm, nstride, tol, G, LC, UC );
dt = As Table( points );
...
**설명:** 복원 추출을 위한 빈도 수를 생성하며 붓스트랩 표본에 유용합니다. 인수를 지정하지 않으면 100% 재표집 표본이 생성됩니다. rate 인수는 재표집 비율을 지정합니다. column 인수를 지정할 경우 선택된 표본 크기는 rate에 지정한 열의 합을 곱한 값입니다. rate가 음수이면 빈도 비율이 허용됨을 나타냅니다.
