...
**설명:** 타원의 호(arc)를 그립니다. 각도는 도(degree) 단위로 지정되며 0도는 12시 방향, 90도는 3시 방향입니다. sin() 및 cos()에 사용되는 라디안 값과 일치시키려면 회전 방향을 반대로 하고 90도 위상 이동을 추가해야 합니다. 예를 들면 radians = 2 \* pi() \* (90 - degrees)/360입니다. 호는 시작점부터 끝점까지 시계 방향으로 움직입니다.
