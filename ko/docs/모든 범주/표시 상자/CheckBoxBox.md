...
**설명:** 입력된 list의 각 항목에 대해 체크박스를 표시하는 표시 상자를 반환합니다.
...
**구문:** obj &lt;&lt; Enable Item( index, 0|1 )

**설명:** index에 있는 항목을 활성화됨(1) 또는 비활성화됨(0)으로 설정합니다.
...
**구문:** 0|1 = obj &lt;&lt; Get( &lt;index&gt; )

**설명:** 3상태 체크박스에 대해 해당 체크박스가 선택되었는지(1), 선택 해제되었는지(0) 또는 미확정인지(empty())를 반환합니다.
...
New Window( "Example",
	cb = Check Box( {"One", "Two", "Three"}, Print( "Selected: " || Char( cb << Get Selected() ) ) )
);
Wait( 1 );
...
New Window( "Example",
	Check Box(
		...
		<<setfunction(/*anonymous function...*/Function(
				...
				ivalue << SetText(
					Char( thisBox << Get( iCheck )/*<<Get returns the value of the iCheck box*/ )
				);
			)
		)
	),
...
**구문:** indices = obj &lt;&lt; Get Selected Indices As Matrix

**설명:** Returns a list of all of the selected indices as a matrix
...
**구문:** obj &lt;&lt; Get Three State( &lt;Boolean&gt; )

**설명:** 3상태 체크박스를 생성합니다. 모든 항목은 0, 1 또는 empty()입니다.
...
**구문:** obj &lt;&lt; Set( &lt;index&gt;,&lt;Boolean|empty()&gt;,&lt;Run Script(0|1)&gt; )

**설명:** 체크박스를 선택됨(1) 또는 선택 해제됨(0)으로 설정합니다. 인수가 위치와 관련 있으므로 RunScript 옵션을 사용하려면 Boolean 인수도 지정해야 합니다.
...
**구문:** obj &lt;&lt; Set All( &lt;Boolean|empty()&gt;,&lt;Run Script(0|1)&gt; )

**설명:** 모든 체크박스를 선택됨(1) 또는 선택 해제됨(0)으로 설정합니다. 인수가 위치와 관련 있으므로 RunScript 옵션을 사용하려면 Boolean 인수도 지정해야 합니다.
...
**구문:** obj &lt;&lt; Set Function( Function( {thisBox, index}, &lt;script&gt; ) )

**설명:** 컨트롤에 대해 제공된 함수를 설정합니다. 체크박스를 클릭하면 함수의 스크립트가 실행됩니다.
	...
	cb = Check Box(
		...
		<<SetFunction(
			Function( {thisBox, index},
				...
				((thisBox << sib) << sib) << settext( "Selected: " || Char( thisBox << Get Selected ) );
			)
		)
	),
	Text Box( "" ),
...
**구문:** obj &lt;&lt; Set Three State( &lt;Boolean&gt; )
