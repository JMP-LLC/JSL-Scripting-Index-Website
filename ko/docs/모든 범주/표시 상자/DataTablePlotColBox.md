...
**설명:** 데이터 테이블의 열을 표시하는 표시 상자를 반환합니다.
...
**구문:** obj &lt;&lt; Add Element( number|string|list|matrix )

**설명:** 표시 상자에 새 값을 추가합니다.
...
**설명:** 열 내용을 클립보드에 복사합니다.

**JMP추가된 버전:** 15

### Copy Selected Column Rows

**구문:** obj &lt;&lt; Copy Selected Column Rows

**설명:** 선택된 행의 열 데이터를 클립보드에 복사합니다.
...
**구문:** list or matrix = obj &lt;&lt; Get( &lt;i&gt;, &lt;"Unsorted"&gt; )

**설명:** 값 목록 또는 i번째 값을 반환합니다. 기본적으로 값은 테이블에 표시된 순서대로 반환됩니다. "정렬되지 않음"이 지정된 경우 값은 테이블 정렬 방식에 관계없이 항상 동일한 순서로 표시됩니다.
...
**구문:** matrix = obj &lt;&lt; Get As Matrix

**설명:** 값을 행렬(열 벡터)로 반환합니다.
...
**구문:** "Default"|"Top"|"Bottom" = obj &lt;&lt; Get Axis Position

**설명:** 그림 열의 축 위치를 가져옵니다. 기본값은 척도와 제목에 따라 때때로 맨 위에 표시되는 것입니다.
...
**구문:** obj &lt;&lt; Get Bar Select Color

**설명:** 선택된 행에 있을 때의 그림 막대 색상을 가져옵니다.

```jsl


New Window( "Example", tb = Table Box( pcb = Plot Col Box( "Probability", {25, 45, 5, 10, 10, 5} ) ) );
...
**구문:** data table column = obj &lt;&lt; Get Data Table Column

**설명:** Returns a reference to the data table column that corresponds to this table box column.
...
**구문:** {color if upper &lt; reference line, color if crosses reference line, color if lower &gt; reference line, alpha, line width, pen style, cap length } = obj &lt;&lt; Get primary interval line properties

### Get secondary interval line properties

**구문:** {color if upper &lt; reference line, color if crosses reference line, color if lower &gt; reference line, alpha, line width, pen style, cap length } = obj &lt;&lt; Get secondary interval line properties

### Hide/Unhide

**구문:** obj &lt;&lt; Hide/Unhide( state=0|1 )

**설명:** 표시 상자를 숨기거나 숨기기 해제합니다.
...
**구문:** obj &lt;&lt; Lock Title( boolean )

**설명:** 열 머리글 편집을 활성화하거나 비활성화합니다.
...
**구문:** obj &lt;&lt; Lower( data table column )

**설명:** 하한은 지정된 점에 곡선을 그립니다.
...
**구문:** obj &lt;&lt; Remove Element( row number )
...
**구문:** obj &lt;&lt; Set( &lt;list&gt;, &lt;"Presorted"&gt; )

**설명:** 목록에서 값을 설정합니다. "사전 정렬"이 지정된 경우 값은 전달된 목록과 동일한 순서로 표시되고, 그렇지 않은 경우 값은 현재 테이블 정렬 순서에 따라 정렬됩니다.
...
**구문:** obj &lt;&lt; Set Axis Position( "Default"|"Top"|"Bottom" )

**설명:** 그림 열의 축이 항상 맨 위 또는 맨 아래에 있도록 설정합니다. 기본값은 척도와 제목에 따라 때때로 맨 위에 표시되는 것입니다.
...
**구문:** obj &lt;&lt; Set Bar Select Color( color )

**설명:** 선택된 행에 있을 때의 그림 막대 색상을 설정합니다.
...
**구문:** obj &lt;&lt; Set Data Table Column( data table column )

**설명:** Sets the data table column that corresponds to this table box column.
...
**구문:** obj &lt;&lt; Set Labels( data table column )
...
**구문:** obj &lt;&lt; Set Plot Style( "Default"|"Bar"|"Interval" )

**설명:** 그림 열의 시각적 표현을 변경합니다.
...
**구문:** obj &lt;&lt; Set Reference Line( number )
...
**구문:** obj &lt;&lt; Set Resizable( boolean )

**설명:** 열 크기를 대화식으로 조정하는 기능을 설정 또는 해제합니다.
...
**구문:** obj &lt;&lt; Set Scale( minimum, maximum, &lt;format name&gt;, &lt;width&gt; | &lt;width, decimal places&gt;, &lt; "Use thousands separator"&gt; )

**설명:** PlotColBox의 minimum 및 maximum 척도를 설정합니다. 형식 파라미터는 처음 두 파라미터가 0과 1인 경우에만 사용되는 형식을 지정합니다.
...
**구문:** obj &lt;&lt; Set Smart Shrinking( boolean )

**설명:** 활성화된 경우, 자동 크기 조정 열이 가장 넓은 요소가 현재 너비의 2/3보다 작아질 때까지 대기합니다.
...
New Window( "Smart Shrinking",
	Table Box(
		...
		c = String Col Edit Box( "Smart", {"I only get smaller when my text is less than 2/3 my width"} )
	)
);
...
**구문:** obj &lt;&lt; Set primary interval line properties( {color if upper &lt; reference line, color if crosses reference line, color if lower &gt; reference line, alpha, line width, pen style, cap length } )

### Set secondary interval line properties

**구문:** obj &lt;&lt; Set secondary interval line properties( {color if upper &lt; reference line, color if crosses reference line, color if lower &gt; reference line, alpha, line width, pen style, cap length } )

### Significance

**구문:** obj &lt;&lt; Significance( data table column )

**설명:** 그림 열이 구간 모드일 때 표식의 크기를 조정하는 데 사용됩니다.
...
**구문:** obj &lt;&lt; Upper( data table column )

**설명:** 상한은 지정된 점에 곡선을 그립니다. 하한이 먼저 지정되어야 합니다.
