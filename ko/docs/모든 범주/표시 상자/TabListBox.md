...
**설명:** 다른 표시 상자를 포함하는 페이지가 있는 표시 상자를 반환합니다.
...
**구문:** obj &lt;&lt; Add(&lt;index&gt;,"title",contents);obj &lt;&lt; Add(&lt;index&gt;,tabPageBox)

**설명:** 새 탭을 추가합니다. index를 지정하지 않으면 현재 탭 뒤에 탭이 추가됩니다. 추가될 내용이 Tab Page Box가 아닐 경우 내용을 포함하기 위해 새 Tab Page Box가 추가됩니다.
...
**설명:** index에 있는 탭을 제거합니다. index가 지정되지 않은 경우 선택된 탭이 제거됩니다.
...
**구문:** obj &lt;&lt; DockName( text )

### Dockable

**구문:** obj &lt;&lt; Dockable( bool )

**설명:** 컨테이너가 도킹 가능한 경우 이동 가능한 하위 항목을 드래그앤드롭으로 재배열할 수 있습니다.
...
(log << Report)[Frame Box( 1 )] << Set Stretch( "Window", "Window" ) << Set Max Size( 1000, 1000 ) <<
Set Min Size( 20, 20 );
(biv << Report)[Frame Box( 1 )] << Set Stretch( "Window", "Window" ) << Set Max Size( 1000, 1000 ) <<
Set Min Size( 20, 20 );
...
**구문:** bool = obj &lt;&lt; Get Dockable

**설명:** 컨테이너 상자에 대한 도킹 가능 설정을 반환합니다.
...
**구문:** obj &lt;&lt; Get Min Title Width

**설명:** Overflow Enabled가 설정된 경우 잘리는 탭 제목의 최소 너비를 가져옵니다. 기본 설정 0은 기본 최소 너비를 적용합니다.
...
**구문:** obj &lt;&lt; Get Overflow Enabled

**설명:** Overflow Enabled가 1이면 탭 목록이 강제로 탭 제목 너비로 설정되지 않습니다. 탭 목록이 모든 제목을 포함할 만큼 넓지 않을 경우 새 탭을 선택할 수 있도록 오버플로우 메뉴가 표시됩니다.
...
**구문:** obj &lt;&lt; Get Show Tabs

**설명:** 탭 표시의 현재 상태를 가져옵니다.
...
**구문:** style = obj &lt;&lt; Get Style

**설명:** 탭 컨트롤의 현재 시각적 모양을 가져옵니다.
...
**구문:** obj &lt;&lt; Get Tab Changed

**설명:** 다른 탭을 선택하면 실행되는 스크립트를 반환합니다.
...
**구문:** obj &lt;&lt; Get Tab Close

**설명:** 스크립트에서 탭을 닫거나 닫기 아이콘을 통해 탭을 닫으면 실행되는 스크립트를 반환합니다. 스크립트는 1(닫기 허용) 또는 0(닫기 취소)을 반환해야 합니다.
...
**구문:** "string" = obj &lt;&lt; Get Tab Close Tip

**설명:** 탭 닫기 아이콘의 툴팁을 반환합니다.
...
**구문:** all | horizontal,vertical | left,top,right,bottom = obj &lt;&lt; Get Tab Margin
...
**설명:** 새로 만들기 탭을 클릭하면 실행되는 스크립트를 반환합니다. 이 스크립트는 새로 만들기 탭 생성을 담당합니다.
...
**구문:** "string" = obj &lt;&lt; Get Tab New Icon

**설명:** 새로 만들기 탭의 아이콘을 반환합니다.
...
**구문:** "string" = obj &lt;&lt; Get Tab New Tip

**설명:** 새로 만들기 탭의 툴팁을 반환합니다.
...
**구문:** style = obj &lt;&lt; Get Title Location

**설명:** 현재 제목 위치 설정을 가져옵니다.
...
**구문:** index = obj &lt;&lt; Index of Tab( "string" )

**설명:** 지정된 제목을 가진 탭의 인덱스를 반환합니다.
...
**구문:** obj &lt;&lt; Insert(&lt;index&gt;,"title",contents);obj &lt;&lt; Insert(&lt;index&gt;,tabPageBox)

**설명:** 새 탭을 삽입합니다. index를 지정하지 않으면 현재 탭 뒤에 새 탭이 추가됩니다. 추가될 내용이 Tab Page Box가 아닐 경우 내용을 포함하기 위해 새 Tab Page Box가 추가됩니다.
...
**구문:** obj &lt;&lt; Move Tab( fromIndex, toIndex )

**설명:** fromIndex 위치의 탭을 toIndex 위치로 이동합니다.
...
**구문:** obj &lt;&lt; Replace(&lt;index&gt;,"title",contents);obj &lt;&lt; Replace(&lt;index&gt;,tabPageBox)

**설명:** 탭을 바꿉니다. index를 지정하지 않으면 선택한 탭이 바뀝니다. 추가될 내용이 Tab Page Box가 아닐 경우 내용을 포함하기 위해 새 Tab Page Box가 추가됩니다.
...
nw = New Window( "Test",
	...
	titlebox = Text Box( "" ),
	Lineup Box( N Col( 4 ),
		Button Box( "add 1", tb << Add( 1, "created " || Char( title++ ), Text Box( "content" ) ) ),
		Button Box( "add 2", tb << Add( 2, "created " || Char( title++ ), Text Box( "content" ) ) ),
		Button Box( "add 3", tb << Add( 3, "created " || Char( title++ ), Text Box( "content" ) ) ),
		Button Box( "add 4", tb << Add( 4, "created " || Char( title++ ), Text Box( "content" ) ) ),
		...
		Button Box( "replace 1", tb << replace( 1, "replaced " || Char( title++ ), Text Box( "replaced" ) ) ),
		Button Box( "replace 2", tb << replace( 2, "replaced " || Char( title++ ), Text Box( "replaced" ) ) ),
		Button Box( "replace 3", tb << replace( 3, "replaced " || Char( title++ ), Text Box( "replaced" ) ) ),
		Button Box( "replace 4", tb << replace( 4, "replaced " || Char( title++ ), Text Box( "replaced" ) ) ),
...
nw = New Window( "Test",
	...
	Lineup Box( N Col( 4 ),
		Button Box( "add 1",
			tb << Add( 1, Tab Page Box( Title( "created " || Char( title++ ) ), Text Box( "content" ) ) )
		),
		Button Box( "add 2",
			tb << Add( 2, Tab Page Box( Title( "created " || Char( title++ ) ), Text Box( "content" ) ) )
		),
		Button Box( "add 3",
			tb << Add( 3, Tab Page Box( Title( "created " || Char( title++ ) ), Text Box( "content" ) ) )
		),
		Button Box( "add 4",
			tb << Add( 4, Tab Page Box( Title( "created " || Char( title++ ) ), Text Box( "content" ) ) )
		),
		Button Box( "remove 1", tb << delete( 1 ) ),
		Button Box( "remove 2", tb << delete( 2 ) ),
		Button Box( "remove 3", tb << delete( 3 ) ),
		...
		Button Box( "replace 1",
			tb << replace(
				1,
				Tab Page Box( Title( "replaced " || Char( title++ ) ), Text Box( "replaced" ) )
			)
		),
		...
		Button Box( "replace 3",
			Tab Page Box(
				Title( tb << replace( 3, "replaced " || Char( title++ ) ), Text Box( "replaced" ) )
			)
		),
...
**구문:** obj &lt;&lt; Set Min Title Width( number )

**설명:** Overflow Enabled가 설정된 경우 잘리는 탭 제목의 최소 너비를 설정합니다. 기본 설정 0은 기본 최소 너비를 적용합니다.
...
**구문:** obj &lt;&lt; Set Overflow Enabled( state=0|1 )

**설명:** Overflow Enabled가 true이면 탭 목록이 강제로 탭 제목 너비로 설정되지 않습니다. 탭 목록이 모든 제목을 포함할 만큼 넓지 않을 경우 새 탭을 선택할 수 있도록 오버플로우 메뉴가 표시됩니다. 이 옵션은 스타일이 Tab으로 설정된 경우에만 적용됩니다.
...
**구문:** obj &lt;&lt; Set Selected( index )

**설명:** index의 탭으로 포커스를 가져옵니다.
...
**구문:** obj &lt;&lt; Set Style( "탭"|"콤보"|"개요"|"세로로 보기"|"가로로 보기"|"크기 최소화" )

**설명:** 탭 컨트롤의 시각적 모양을 변경합니다.
...
**구문:** obj &lt;&lt; Set Tab Changed( script|Function( {this}, &lt;script&gt; ) )

**설명:** 변경(예: 새 탭 선택)이 수행된 후 지정된 script 또는 function이 실행됩니다.
...
**구문:** obj &lt;&lt; Set Tab Close( script|Function )

**설명:** 스크립트에서 또는 닫기 아이콘을 통해 탭을 닫을 때 실행될 script 또는 Function을 설정합니다. script 또는 Function은 1(닫기 허용) 또는 0(닫기 취소)을 반환해야 합니다.
...
**구문:** obj &lt;&lt; Set Tab Close Tip( "string" )

**설명:** 탭 닫기 아이콘의 툴팁을 설정합니다.
...
**구문:** obj &lt;&lt; Set Tab Margin( all | horizontal,vertical | left,top,right,bottom )

**설명:** 탭의 여백을 설정합니다. 입력된 파라미터는 모든 측면, 가로 및 세로 크기 또는 각 측면에 대한 다양한 값에 적용될 수 있습니다.
...
**구문:** obj &lt;&lt; Set Tab New( script|Function( {this}, &lt;script&gt; ) )

**설명:** 새로 만들기 탭을 클릭하면 실행할 script 또는 Function을 설정합니다. script 또는 function이 새로 만들기 탭의 생성을 담당합니다.
...
**구문:** obj &lt;&lt; Set Tab New Icon( "string" )

**설명:** 새로 만들기 탭의 아이콘을 설정합니다.
...
**구문:** obj &lt;&lt; Set Tab New Tip( "string" )

**설명:** 새로 만들기 탭의 툴팁을 설정합니다.
...
**구문:** obj &lt;&lt; Set Title Location( "자동"|"위쪽"|"아래쪽"|"왼쪽"|"오른쪽" )

**설명:** 탭 페이지 제목의 위치를 설정합니다. "자동"으로 설정하면 콘텐츠 크기에 따라 위치가 자동으로 선택됩니다. 이 옵션은 스타일이 Tabbed로 설정된 경우에만 적용됩니다.
