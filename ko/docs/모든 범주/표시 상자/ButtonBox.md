...
**설명:** 버튼 클릭을 시뮬레이션하여 적절한 경우 콜백을 실행합니다. 전환 버튼은 콜백이 호출되기 전에 상태를 변경합니다. 메뉴 버튼에는 클릭할 항목을 지정하는 인수가 필요합니다.
...
**설명:** 전환 버튼이 눌려 있으면 1을 반환하고 눌려 있지 않으면 0을 반환합니다. Style("Toggle")이 있는 버튼에만 적용됩니다.
...
New Window( "Toggle Button Example",
	ex = Button Box( "Toggle", If( ex << Get, Print( "Button is down" ), Print( "Button is up" ) ) )
);
ex << Style( "Toggle" );
...
**구문:** left|right = obj &lt;&lt; Get Icon Location

**설명:** 버튼 상자 내 아이콘의 위치를 반환합니다. 아이콘은 버튼 텍스트의 왼쪽 또는 오른쪽에 표시될 수 있습니다.
...
**구문:** obj &lt;&lt; Get Menu Choice

**설명:** 마지막 사용자 메뉴 선택의 인덱스를 가져옵니다.
...
**구문:** list of menu items = obj &lt;&lt; Get Menu Items

**설명:** 버튼을 클릭할 때 팝업 메뉴에 사용되는 메뉴 항목을 가져옵니다.
...
**구문:** style = obj &lt;&lt; Get Underline Style

**설명:** 버튼의 현재 밑줄 스타일을 반환합니다.
...
**구문:** obj &lt;&lt; Open Next Outline

**설명:** 상위 개요 상자를 닫고 다음 개요 상자를 엽니다.
...
**구문:** obj &lt;&lt; Restore Focus After Click( state=0|1 )

**설명:** 클릭한 후 편집 포커스를 유지하도록 버튼을 설정합니다. Windows에서의 기본값은 포커스를 가져오는 것입니다. Macintosh에서의 기본값은 포커스를 유지하는 것입니다.

```jsl


New Window( "Example", Script Box( "" ), ex = Button Box( "Leave Focus", 0 ), Button Box( "Take Focus", 0 ) );
...
**구문:** obj &lt;&lt; Set( pressed, &lt;run script(0|1)&gt; )

**설명:** 전환 버튼의 전환 상태를 눌림(1) 또는 눌리지 않음(0)으로 설정합니다. Style("Toggle")이 있는 버튼에만 적용됩니다.
...
**구문:** obj &lt;&lt; Set Button Name( name )

**설명:** 버튼 이름을 제공된 텍스트로 설정합니다.
...
**구문:** obj &lt;&lt; Set Function( Function( {thisBox}, &lt;script&gt; ) | Method({thisBox}, &lt;script&gt;) | &lt;symbol&gt; )

**설명:** 컨트롤에 대해 제공된 함수를 설정합니다. 버튼을 누르면 함수의 스크립트가 실행됩니다.
...
**구문:** obj &lt;&lt; Set Icon( icon name|image filepath )

**설명:** 버튼에 JMP 아이콘 또는 이미지를 표시합니다. JMP 아이콘 이름 또는 이미지 파일 경로를 지정하십시오.
...
**구문:** obj &lt;&lt; Set Icon Location( left,right )

**설명:** 아이콘 위치를 버튼 텍스트의 왼쪽 또는 오른쪽으로 설정합니다.
...
**구문:** obj &lt;&lt; Set Menu Choice( index )

**설명:** 사용자 메뉴 선택의 인덱스를 설정합니다.
...
**구문:** obj &lt;&lt; Set Menu Items( list of menu items )

**설명:** 버튼을 클릭할 때 팝업 메뉴에 사용되는 메뉴 항목을 설정합니다.
...
New Window( "Underline Button Example", ex = Button Box( "Start", ex << Set Button Name( "Stop" ) ) );
...
**구문:** obj &lt;&lt; Underline Style( state=0|1 )

**설명:** 버튼이 표준 버튼 스타일 대신 밑줄 그리기 스타일을 사용하도록 설정합니다.
...
**구문:** obj &lt;&lt; Unhide Next Item

**설명:** 상위 개요 상자의 숨겨진 항목을 표시합니다.
