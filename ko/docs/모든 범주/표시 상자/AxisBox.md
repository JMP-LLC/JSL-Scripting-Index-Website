...
**구문:** obj &lt;&lt; Add Axis Label( label )

**설명:** 지정된 문자열을 사용하여 축 라벨을 추가합니다.
...
**구문:** obj &lt;&lt; Add Ref Line( value, &lt;"Solid|Dashed|Double"&gt;, &lt;Color&gt;, &lt;"label"&gt;, &lt;line width&gt;, &lt;transparency&gt; )

**설명:** 격자의 value에 참조선을 추가합니다.
...
**구문:** obj &lt;&lt; Axis Settings

**설명:** 축 대화상자를 표시하는 방법을 제공하거나 지정된 축에 대한 구성 가능한 축 옵션을 설정합니다.
...
**구문:** obj &lt;&lt; Copy Axis Settings

**설명:** 현재 축 설정이 포함된 스크립트를 만들고 클립보드에 복사합니다.
...
gbr = Report( gb );
gbr[axisbox( 1 )] << scale( custom scale, custom scale name( "log2" ) ) << Min( 32 ) << Max( 100 );

```

### Decimal

**구문:** obj &lt;&lt; Decimal( number )

**설명:** 숫자 라벨의 소수점 이하 자릿수를 설정합니다.
...
**구문:** obj &lt;&lt; Edit Value Order

**설명:** 편집할 값 순서 열 특성을 엽니다.
...
**설명:** Returns the format specification for axis values.
...
**설명:** 축 상자를 다시 생성하기 위한 스크립트를 반환합니다.
...
**구문:** obj &lt;&lt; Interval( "숫자"|"연도"|"분기"|"월"|"주"|"일"|"시"|"분"|"초" )
...
plot = Overlay Plot(
	...
	Right Axis << {{Format( "Best", 9 ), Min( 0 ), Max( 800000000 ), Inc( 100000000 ), Minor Ticks( 0 )}},
...
**구문:** obj &lt;&lt; Label Row Nesting( number )

**설명:** 날짜 형식 축에 대한 라벨 행 수를 설정합니다.
...
**구문:** obj &lt;&lt; Minor Ticks( number )

**설명:** 라벨 사이에 배치되어야 할 눈금 수를 설정합니다.
...
**구문:** obj &lt;&lt; Paste Axis Settings

**설명:** 클립보드에 축 설정에 적합한 스크립트가 포함되어 있습니다. 적용하십시오.
...
**구문:** obj &lt;&lt; Remove Axis Label

**설명:** "축 라벨 추가"를 사용하여 추가된 모든 라벨을 제거합니다.
...
**구문:** obj &lt;&lt; Reversed Scale

**설명:** 상한값이 왼쪽 또는 아래쪽에 오도록 척도 방향을 역순으로 바꿉니다.
...
**설명:** 다른 사용자 정의를 변경하지 않고 축 척도를 복원합니다.
...
**구문:** obj &lt;&lt; Save To Column Property

**설명:** 축 설정을 이 축과 관련된 데이터 열에 축 특성으로 저장합니다.
...
**구문:** obj &lt;&lt; Scale( "선형"|"로그"|"지수 확률"|"Weibull 확률"|"로지스틱 확률"|"Frechet 확률"|"정규 확률"|"세제곱근"|"Johnson Su"|"측지"|"측지(미국)"|"거듭제곱"|"감마 확률"|"베타 확률"|"2 정규 혼합 확률"|"3 정규 혼합 확률"|"사용자 척도" )

**설명:** 축에 적용되어야 하는 척도의 유형을 지정합니다. 유형이 사용자 척도면 이 메시지에 두 개의 추가 절이 필요합니다. 내부로 척도화 및 외부로 척도화를 참조하십시오.
...
**구문:** obj &lt;&lt; Scale ID( number )

**설명:** 여러 축에 사용할 특정 ID를 축에 추가합니다.
...
**구문:** obj &lt;&lt; Scale To External

**설명:** 지정된 사용자 계산식에 따라 외부 좌표계를 척도화합니다.
...
**구문:** obj &lt;&lt; Scale To Internal

**설명:** 지정된 사용자 계산식에 따라 내부 좌표계를 척도화합니다
...
**설명:** 주 눈금 표시에 대한 축 라벨을 표시하거나(1) 숨깁니다(0). 기본적으로 설정되어 있습니다.
...
**구문:** obj &lt;&lt; Show Major Grid( state=0|1 )

**설명:** 숫자 라벨과 동일한 위치에 배치된 격자를 표시하거나(1) 숨깁니다(0).
...
**구문:** obj &lt;&lt; Show Major Labels( state=0|1 )
...
**구문:** obj &lt;&lt; Show Major Ticks( state=0|1 )

**설명:** 숫자 라벨과 동일한 위치에 배치된 눈금 표식을 표시하거나(1) 숨깁니다(0). 기본적으로 설정되어 있습니다.
...
**구문:** obj &lt;&lt; Show Minor Grid( state=0|1 )

**설명:** 숫자 라벨 사이에 배치된 격자를 표시하거나(1) 숨깁니다(0).
...
**구문:** obj &lt;&lt; Show Minor Labels( state=0|1 )

**설명:** 보조 눈금 표시에 대한 축 라벨을 표시하거나(1) 숨깁니다(0).
...
**구문:** obj &lt;&lt; Show Minor Ticks( state=0|1 )

**설명:** 숫자 라벨 사이에 배치된 눈금 표식을 표시하거나(1) 숨깁니다(0). 기본적으로 설정되어 있습니다.
...
**구문:** &lt;X / Y&gt; Axis( Side ( &lt;1/2&gt; ) )

**설명:** 축의 사이드(기본(1) 또는 보조(2))을 변경합니다.
...
**구문:** obj &lt;&lt; Tick Label List

**설명:** 축 눈금 라벨의 값 및 위치를 설정합니다.
...
**구문:** obj &lt;&lt; Update Ref Line( ID, index, value, &lt;"Solid|Dashed|Double"&gt;, &lt;Color&gt;, &lt;"label"&gt;, &lt;line width&gt;, &lt;transparency&gt; )
