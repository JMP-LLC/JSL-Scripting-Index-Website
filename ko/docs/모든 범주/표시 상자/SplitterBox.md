...
**설명:** 대화식 크기 컨트롤을 사용하여 다른 표시 상자를 가로로 또는 세로로 구성할 수 있는 표시 상자를 반환합니다. 하위 크기는 Splitter Box의 너비 또는 높이의 비율로 지정됩니다. 선택적 Size 인수는 최상위 분할 상자에만 사용됩니다. 하위 수준 상자는 다른 하위 상자와 마찬가지로 크기가 조정됩니다.
...
**구문:** obj &lt;&lt; Close Panel( index,&lt;previous&gt; )

**설명:** 제공된 인덱스의 패널을 닫습니다. previous 인수가 1이면 목록의 이전 상자에 공백이 할당되고 그렇지 않으면 다음 상자에 할당됩니다.
...
**구문:** obj &lt;&lt; Divider Color( color );color = obj &lt;&lt; Get Divider Color

**설명:** 각 분할 구분선의 색상을 지정하거나 반환합니다. 설정하지 않으면 배경색을 기반으로 한 색상이 사용됩니다.
...
**구문:** obj &lt;&lt; Divider Length( number );number = obj &lt;&lt; Get Divider Length

**설명:** 각 분할 구분선의 길이를 백분율로 지정하거나 반환합니다. 값이 0.0이면 구분선이 표시되지 않고 1.0(기본값)이면 전체 길이의 구분선이 표시됩니다.
...
**구문:** obj &lt;&lt; Divider Thickness( number );number = obj &lt;&lt; Get Divider Thickness

**설명:** 각 분할 구분선의 두께를 지정하거나 반환합니다. 이 값은 0이 아니어야 합니다.
...
**구문:** {size1,...} = obj &lt;&lt; Get Sizes

**설명:** Splitter Box의 각 패널에 대한 비율을 반환합니다.
...
**구문:** obj &lt;&lt; Open Panel( index,&lt;previous&gt; )

**설명:** 제공된 인덱스의 패널을 엽니다. previous 인수가 1이면 목록의 이전 상자에서 공백을 가져오고 그렇지 않으면 다음 상자에서 공백을 가져옵니다.
...
**구문:** obj &lt;&lt; Set Sizes( {size1,...} )

**설명:** Splitter Box의 각 패널에 대한 비율을 설정합니다.
...
**설명:** Splitter Box의 마지막 패널에 대한 비율을 설정합니다.
...
**구문:** state=0|1 = obj &lt;&lt; get horizontal

**설명:** 목록 상자가 가로(1)인지 세로(0)인지를 반환합니다.
...
**구문:** obj &lt;&lt; set horizontal( state=0|1 )

**설명:** 목록 상자를 가로(1) 또는 세로(0)로 설정합니다.
