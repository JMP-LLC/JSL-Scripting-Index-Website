...
**구문:** obj &lt;&lt; Add Text( "string" )

**설명:** 고정된 가리키기 라벨에 사용자 정의 텍스트 블록을 추가합니다.
...
**구문:** obj &lt;&lt; Click Link( "&lt;index, integer&gt;" )

**설명:** 제공된 텍스트 또는 인덱스와 연결된 링크에서 클릭 작업을 시뮬레이션합니다. 제공된 항목이 없으면 마지막 링크(있는 경우 대개 Graphlet)를 가정합니다.
...
frame << Set Gridlet(
	Annex( {{Matcher( "My Link" ), value( "Click Me!" ), click( Web( "https://jmp.com" ) )}} )
);
gpin = frame << Add Pin Annotation(
	Seg( BarSeg( 1 ) ),
	Index( {0, 0} ),
...
**설명:** 가리키기 라벨의 콘텐츠를 클립보드에 복사합니다. 

작업이 변수에 할당되면 가리키기 라벨 텍스트 콘텐츠가 변수에 저장됩니다.
...
**설명:** 컨텍스트 메뉴에서 "삭제"를 제거할 수 있도록 허용합니다.
...
**설명:** 가리키기 라벨 콘텐츠에 대한 시각적 편집기를 시작합니다.
...
**구문:** obj &lt;&lt; Filled( state=0|1 )

### Font

**구문:** obj &lt;&lt; Font

**설명:** 고정된 가리키기 라벨 글꼴을 설정합니다.
...
**설명:** 가리키기 라벨에서 클릭 가능한 항목과 연결된 텍스트 목록을 반환합니다. 

그림과 Graphlet은 빈 문자열과 연결되어 있습니다.
...
**구문:** Associative Array = obj &lt;&lt; Get Local Context

**설명:** 가리키기 라벨 실행 컨텍스트의 일부로 제공된 지역 변수의 이름과 값을 사용하여 연관 배열을 반환합니다.

사용 예제에 대해서는 Graphlet 사전 설정 정의를 참조하십시오.
...
**구문:** obj &lt;&lt; Launch graphlet

**설명:** 새 창에서 Graphlet 시각화 또는 플랫폼을 시작합니다.

실행 취소 명령을 사용하여 실행기/이전 콘텐츠를 복구할 수 있습니다.

Graphlet 썸네일을 클릭하면 동일한 기능이 제공됩니다.
...
**구문:** obj &lt;&lt; Launch in last

**설명:** 새 창 또는 마지막으로 시작한 창에서 Graphlet 시각화 또는 플랫폼을 시작하여 콘텐츠를 바꿉니다.

실행 취소 명령을 사용하여 실행기/이전 콘텐츠를 복구할 수 있습니다.

Alt 키를 누른 채 Graphlet 썸네일을 클릭하면 동일한 기능이 제공됩니다.
...
**구문:** obj &lt;&lt; Launch in place

**설명:** 현재 창에서 Graphlet 시각화 또는 플랫폼을 시작하여 콘텐츠를 바꿉니다.

실행 취소 명령을 사용하여 실행기/이전 콘텐츠를 복구할 수 있습니다.

Ctrl 키를 누른 채 Graphlet 썸네일을 클릭하면 동일한 기능이 제공됩니다.
...
**구문:** obj &lt;&lt; Replace Text( "string" )

**설명:** 고정된 가리키기 라벨 텍스트 콘텐츠를 지정된 텍스트로 바꿉니다.
...
**설명:** 고정된 가리키기 라벨 텍스트 콘텐츠를 원래 텍스트로 되돌립니다.
...
**설명:** 가리키기 라벨 아래에 시각적 요소와 연결된 데이터의 부분집합만 표시하는 로컬 데이터 필터를 사용하여 데이터 테이블을 시작합니다.
