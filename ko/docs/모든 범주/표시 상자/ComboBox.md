...
**설명:** 텍스트 항목의 드롭다운 목록을 보여 주는 표시 상자를 반환합니다. 필요한 경우 콤보 상자의 각 항목에 툴팁(항목 텍스트 문자열 다음에 괄호로 묶은 문자열 추가)을 지정할 수 있습니다. 선택적 Editable 인수를 사용하여 콤보 텍스트를 편집할 수 있습니다.
...
**구문:** obj &lt;&lt; Append Item( string )

**설명:** 새 문자열을 콤보 상자 항목 목록의 끝에 추가합니다.
...
**설명:** index에 있는 항목을 선택 가능(1) 또는 선택 취소 가능(0)으로 설정합니다.
...
**구문:** obj &lt;&lt; Insert Item( string, &lt;position&gt; )

**설명:** 목록 끝 또는 선택적 position에 표시된 위치에 새 문자열을 삽입합니다.
...
**설명:** index의 항목을 선택할 수 있는지(1) 또는 선택할 수 없는지(0)를 반환합니다.
...
**구문:** obj &lt;&lt; Set( index,&lt;run script(0|1)&gt; )

**설명:** 콤보 상자를 제공된 인덱스에 있는 항목으로 설정합니다.
...
**설명:** 컨트롤에 대해 제공된 함수를 설정합니다. 새 콤보 상자 항목을 선택하면 함수의 스크립트가 실행됩니다.
	...
	Combo Box(
		...
		<<SetFunction(
			Function( {thisBox, index},
				...
				((((lb << child) << sib) << sib) << sib) << Set Text( Char( thisBox << get selected ) );
			)
		)
	),
	Lineup Box( N Col( 2 ),
