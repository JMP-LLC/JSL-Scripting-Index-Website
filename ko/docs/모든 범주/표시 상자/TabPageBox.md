...
**구문:** obj &lt;&lt; Closeable( state=0|1 )

**설명:** 탭의 닫기 가능 상태를 설정합니다.
...
**구문:** obj &lt;&lt; Focus Color( color );color = obj &lt;&lt; Get Focus Color

**설명:** 포커스 색상이 설정되면 탭 또는 탭 내부 상자에 포커스가 있을 때 탭 제목의 포커스 막대에 해당 색상이 사용됩니다.
...
New Window( "Example",
	V List Box(
		Tab Box(
			tp = Tab Page Box( Title( "alpha" ), Text Edit Box( "text" ), <<Focus Color( "Cyan" ) ),
			Tab Page Box( Title( "beta" ), Text Edit Box( "more text" ), <<Focus Color( "Fuchsia" ) ),
...
**설명:** 스크립트에서 또는 닫기 아이콘을 통해 탭을 닫을 때 실행되는 스크립트를 반환합니다. 스크립트는 1(닫기 허용) 또는 0(닫기 취소)을 반환해야 합니다.
...
**구문:** "bool" = obj &lt;&lt; Get Closeable

**설명:** 탭의 닫기 가능 상태를 반환합니다.
...
**설명:** 상자의 이동 가능 설정을 반환합니다.

### Get Scriptable Object

**구문:** scriptable object = obj &lt;&lt; Get Scriptable Object

**설명:** 탭 페이지 상자와 연결된 스크립트 가능 개체에 대한 참조를 반환합니다.
...
**설명:** 대화식으로 변경할 수 없도록 탭 페이지 제목을 잠급니다.
...
**구문:** obj &lt;&lt; Moveable( state=0|1 )

**설명:** 이동 가능한 상자를 드래그앤드롭으로 도킹이 가능한 상자 내에 재배열할 수 있습니다.
...
**구문:** obj &lt;&lt; Set Close( script|Function( {this}, &lt;script&gt; ) )

**설명:** 스크립트에서 또는 닫기 아이콘을 통해 탭을 닫을 때 실행될 script 또는 Function을 설정합니다. 또는 script 또는 Function은 1(닫기 허용) 또는 0(닫기 취소)을 반환해야 합니다.
...
**설명:** 탭 페이지 상자와 연결된 스크립트 가능 개체를 설정합니다. 탭 페이지 상자는 스크립트 가능 개체로 보낼 수 있는 빨간색 삼각형 메시지 메뉴를 표시합니다. 인수를 제공하지 않으면 스크립트 가능 개체가 지워집니다.
...
**설명:** 탭 페이지 제목이 변경될 때 실행되는 함수를 설정합니다.
...
**구문:** obj &lt;&lt; Title( "string", &lt;Run Script(0 | 1)&gt; )

**설명:** Tab Page Box의 제목을 설정합니다.
