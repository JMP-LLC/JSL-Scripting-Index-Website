...
**설명:** 계층적 트리를 보여 주는 표시 상자를 반환합니다. 기본적으로 단일 항목을 선택할 수 있습니다. 다중 선택(multiselect)을 지정하면 트리에서 둘 이상의 항목을 선택할 수 있습니다.
...
**구문:** obj &lt;&lt; Enable Item( node, 0|1 )

**설명:** 트리의 노드를 활성화 또는 비활성화합니다. 노드를 활성화하려면 상태를 1로 설정하고 비활성화하려면 상태를 0으로 설정하십시오.
...
**구문:** script = obj &lt;&lt; Get Context Menu Script

**설명:** 사용자가 트리 또는 트리 노드를 마우스 오른쪽 버튼으로 클릭하면 실행되는 스크립트, 함수 또는 메서드를 반환합니다.
	...
	Function( {thistree, thisnode},
		If( Is Empty( thisnode ),
			{  /*Whole Tree Menu*/ {"Tree Menu", "", Function( {tree, node}, /*dont use node*/
				Print( "Tree one" )
			)}, {"", "separator", ""}, {"Tree - Menu 2", "", Function( {tree, node}, /*dont use node*/
				Print( "Tree two" )
			)}}
		,
...
**구문:** script = obj &lt;&lt; Get Node Double Click Script

**설명:** 노드를 두 번 클릭하면 실행되는 스크립트, 함수 또는 메서드를 반환합니다.
...
New Window( "TreeBox Tests", tree = Tree Box( {root1, root2}, Size( 300, 200 ) ) );
tree << Set Node Double Click Script( Function( {thistree, thisnode}, Print( thisnode << get label ) ) );
...
**구문:** obj &lt;&lt; Get Node Select Script( script|Function( {thistree, thisnode}, &lt;script&gt; ) )

**설명:** 트리에서 선택이 변경되면 실행되는 스크립트, 함수 또는 메서드를 반환합니다.
...
**설명:** 루트 트리 노드 목록을 가져옵니다. 루트가 없으면 빈 목록이 반환됩니다.
...
**설명:** 단일 선택 트리의 경우 선택한 트리 노드를 가져오거나 비어 있음(노드가 선택되지 않은 경우)을 가져옵니다. 다중 선택 트리의 경우 선택된 노드 목록을 가져오거나 비어 있음(노드가 선택되지 않은 경우)을 가져옵니다.
...
**구문:** collapsed = obj &lt;&lt; Is Collapsed( node )

**설명:** 노드가 트리에서 접혀져 있으면 true를 반환합니다.
...
**구문:** expanded = obj &lt;&lt; Is Expanded( node )

**설명:** 노드가 트리에서 펼쳐져 있으면 true를 반환합니다.
...
**구문:** enabled = obj &lt;&lt; Is Item Enabled( node )

**설명:** 노드가 트리에서 활성화되어 있으면 true를 반환합니다.
...
**구문:** obj &lt;&lt; Is Multiselect

**설명:** 다중 선택 트리면 1을 반환합니다. 단일 선택 트리면 0을 반환합니다.
...
**설명:** 노드가 트리의 루트 노드이면 true를 반환합니다.
...
**구문:** selected = obj &lt;&lt; Is Selected( node )

**설명:** 노드가 트리에서 선택되어 있으면 true를 반환합니다.
...
**구문:** obj &lt;&lt; Make Visible( node )

**설명:** 노드를 트리 내의 보기로 가져옵니다.
...
**구문:** obj &lt;&lt; Remove Root( node )

**설명:** 트리에서 루트 노드를 제거합니다.
...
**구문:** obj &lt;&lt; Set Context Menu Script( Function( {thistree, thisnode}, &lt;script&gt; ) )

**설명:** 사용자가 트리 또는 트리 노드를 마우스 오른쪽 버튼으로 클릭하면 실행할 스크립트, 함수 또는 메서드를 설정합니다.
...
**구문:** obj &lt;&lt; Set Multiselect( state=0|1 )

**설명:** 트리를 단일 선택(0) 또는 다중 선택(1)으로 설정합니다.
...
**구문:** obj &lt;&lt; Set Node Double Click Script( script )

**설명:** 노드를 두 번 클릭하면 실행할 스크립트, 함수 또는 메서드를 설정합니다.
...
**구문:** obj &lt;&lt; Set Node Select Script( script|Function( {thistree, thisnode}, &lt;script&gt; ) )

**설명:** 트리에서 선택이 변경되면 실행할 스크립트, 함수 또는 메서드를 설정합니다. 선택이 지워진 경우 Empty가 스크립트에 전달됩니다.
...
**구문:** obj &lt;&lt; Set Roots( roots )

**설명:** 트리에 표시될 루트 노드 목록을 설정합니다.
...
**구문:** obj &lt;&lt; Set Selected( node|{nodes}, &lt;0|1&gt; )

**설명:** 트리 표시 상자에서 지정된 트리 노드를 선택합니다. 트리 노드 목록이 사용될 경우 다중 선택 트리는 목록의 모든 노드를 선택하고, 단일 선택 트리는 목록의 첫 번째 노드를 선택합니다.
