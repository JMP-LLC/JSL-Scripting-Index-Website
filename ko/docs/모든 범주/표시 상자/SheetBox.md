...
**설명:** 다른 표시 상자를 가로로 또는 세로로 구성할 수 있는 표시 상자를 반환합니다.
...
New Window( "Example",
	V Sheet Box(
		...
		<<Hold(
			Distribution(
				Continuous Distribution( Column( :height ), Horizontal Layout( 1 ), Outlier Box Plot( 0 ) )
			)
		),
...
**설명:** SheetBox가 가로이면 너비를 설정하고 그렇지 않으면 높이를 설정합니다.
...
**구문:** obj &lt;&lt; Sizes( {width,height} )

**설명:** SheetBox의 너비 및 높이를 설정합니다.
