...
**Descripción:** Use DisplayBoxPtr::jssClasses and DisplayBoxPtr::jssID and then target those using selectors. OutlineBoxes and some others automatically set classes from their Scriptable, etc.
...
**Descripción:** The main unit of a JSS file. Composed of a Selector and multiple Declarations/Messages.

**JMP Versión agregada:** 19

### <Selector>

**Sintaxis:** obj << <Selector>

**Descripción:** The first part of a Rule; Used to select boxes. You can compose selectors together to make more complex selectors.

**JMP Versión agregada:** 19

### <Specificity>

**Sintaxis:** obj << <Specificity>

**Descripción:** This helps us be more declarative with our Rules. See CSS documentation for details.
...
**Sintaxis:** And( selector, ..., selector )

**Descripción:** Combine two or more selectors, making a new selector that matches only when all of those match. Use the functional form And(...) or wrap in parentheses before sending messages.
...
**Sintaxis:** Attr( <<Boolean Box Message == 0|1 )

**Descripción:** Matches boxes with the given attribute. Where the box responds to the given message with that value. Currently only supports boolean attributes.
...
**Sintaxis:** Child( parentSelector, childSelector )

**Descripción:** Combines two selectors into a direct parent/child path. A box matches when it and its direct parent both match their individual selectors.
...
**Sintaxis:** Class( className )

**Descripción:** Matches boxes that have the given JSS class (loose comparison).
...
**Sintaxis:** Constants( var1 = expr1; var2 = expr2; ... )

**Descripción:** Defines constants that can be used within rule declarations. Basic operations are allowed in the expressions and the names can reference each other. Names are global within a Theme and the last declaration of a name is used.
...
**Sintaxis:** Descend( selector, ..., selector )

**Descripción:** Combines two or more selectors into a path. A box matches when the path to the box matches. Note that the path can skip levels in the display tree.
...
**Descripción:** Matches when a box is the first child of its parent.
...
**Descripción:** Matches boxes that have the given JSS id (loose comparison).
...
**Sintaxis:** If( condA, rulesA, ..., rulesElse )

**Descripción:** The allowable condition expressions are limited and resolved when the JSS is parsed. No branch testing is done during the JSS application passes. In other words, these are for testing the state of the system and JMP, not individual boxes or box trees.
...
**Sintaxis:** Include( path, <<NoWarnIfMissing )

**Descripción:** Includes another JSS file from a path relative to this one. Errors within that file will be ignored. Errors accessing that file will not, unless you use the <<NoWarnIfMissing flag.
...
**Sintaxis:** JMP Style Sheets

**Descripción:** JMP Style Sheets (JSS) helps us separate DisplayBox content from DisplayBox presentation. The system is modeled on CSS. CSS is to HTML as JSS is to DisplayBox trees in JMP.
...
**Descripción:** Matches when a box is the last child of its parent.
...
**Sintaxis:** Next Sibling( precedingSelector, selector )

**Descripción:** Box must match the second selector and be immediately preceded by a box matching the first selector (sharing the same parent).
...
**Sintaxis:** Or( selector, ..., selector )

**Descripción:** Combines two or more selectors, making a new selector that matches when any of the individual selectors match. Use the functional form Or(...) or wrap in parentheses before sending messages.
...
**Sintaxis:** Type( boxTypeName )

**Descripción:** Matches boxes of the given type (loose comparison).
...
**Descripción:** Matches all boxes. Useful mainly for clearly expressing intent.
