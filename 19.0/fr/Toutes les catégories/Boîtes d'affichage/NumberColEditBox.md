...
**Description :** Renvoie une boîte d’affichage affichant les nombres modifiables dans une colonne.
...
col << Set Function(
	Function( {this, which},
		Print(
			"row " || Char( which ) || " changed to " || Char( col << Get( which ) )
		)
	)
);
col << Get Function;
...
**Syntaxe :** obj &lt;&lt; Set Function( Function( {thisBox, which}, &lt;script&gt; ) )

**Description :** Définit la fonction donnée pour le contrôle. Le script de la fonction s&apos;exécute lorsqu&apos;une modification est prise en compte. which est le numéro de ligne de la cellule modifiée.
...
col << Set Function(
	Function( {thisBox, which},
		Print(
			"row " || Char( which ) || " changed to " ||
			Char( thisBox << Get( which ) )
		)
	)
);
