...
**Syntaxe :** obj &lt;&lt; Timestamp( column(s) )
...
**Syntaxe :** obj = Process History Explorer(...Goal is to Minimize Y( state=0|1 )...)

**Description :** Spécifie que la cible de l&apos;analyse est la minimisation de la réponse.
...
**Syntaxe :** obj &lt;&lt; Levels with Lowest Yield( state=0|1 )

**Description :** Affiche ou masque une table de données des statistiques de résumé pour le rendement de chaque X, variable du processus.
...
**Syntaxe :** obj &lt;&lt; Levels with Lowest Yield with Time Filter( state=0|1 )

**Description :** Affiche ou masque une table de données des statistiques de résumé pour les intervalles temporels des 50 premiers niveaux ayant les plus faibles rendements.
...
**Syntaxe :** obj &lt;&lt; Order Interaction

**Description :** Affiche ou masque un rapport d&apos;une analyse univariée du rendement sur la colonne Ordre.

**JMP Version ajoutée :** 14

### Save Count Table

**Syntaxe :** obj &lt;&lt; Save Count Table

**Description :** Enregistre une nouvelle table de données qui contient les dénombrements des unités ayant passé chaque niveau de X, variables du processus.
...
**Syntaxe :** obj &lt;&lt; Save Log Count Table

**Description :** Enregistre une nouvelle table de données au même format que la table de données Dénombrement, à l&apos;exception des entrées de Log(Dénombrement + 1).
...
**Syntaxe :** obj &lt;&lt; Save Waiting Time

**Description :** Enregistre chaque table de données dans le rapport Analyse du temps d&apos;attente pour séparer la table de données.
...
**Syntaxe :** obj &lt;&lt; Stepwise Regression( Goal( "Biggest Individual Difference" | "Biggest Total Difference" | "Most Predictive"), Time Filtering( "None" | "Starting Time"), X Transform( "None" | "Log(Count+1)"), N Steps( number ) )

**Description :** Réalise une analyse par régression pas à pas en fonction des options spécifiées. Une table de données est fournie indiquant l&apos;ordre dans lequel les termes ont été entrés dans le modèle à chaque étape de la régression.
...
**Syntaxe :** obj &lt;&lt; Transition Analysis( columns, {"level1", "level2", ... } )

**Description :** Affiche un rapport qui vous permet d&apos;identifier les transitions d&apos;une composante à une autre qui posent problème pour le rendement résultant.
...
**Syntaxe :** obj &lt;&lt; Utilization

**JMP Version ajoutée :** 14

### Waiting Time Analysis

**Syntaxe :** obj &lt;&lt; Waiting Time Analysis

**Description :** Fournit une table de données des statistiques de résumé du temps d&apos;attente pour chaque ID. Dans cette table de données, le temps d&apos;attente est la durée entre la fin d&apos;une étape pour un ID et le démarrage d&apos;une autre étape.
...
**Syntaxe :** Yield Columns( columns )

**Description :** Spécifie la colonne dans la table de données des rendements qui contient les valeurs de rendement.
...
**Syntaxe :** Yield Table( data table )

**Description :** Spécifie la table de données qui contient les rendements de chaque ID.
...
**Syntaxe :** obj = Process History Explorer(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;Élément lanceur : Oui&lt;/b&gt;
