...
**構文:** "Promise" = obj &lt;&lt; Execute Script

**説明:** 現在のページのコンテキストで、指定のJavaScriptを非同期に実行する。スクリプトの実行結果は、プロミスオブジェクトに文字列で戻す。
...
**説明:** Sets the width and height of the web browser box. The web browser box has a default Stretch setting of &apos;Window&apos; that must be changed to &apos;Off&apos; to honor the requested size.
