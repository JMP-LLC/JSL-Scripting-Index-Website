...
**構文:** obj &lt;&lt; Before( "string" )

**説明:** HierBoxの前に新しい文字列を挿入する。
...
**構文:** obj &lt;&lt; Change Type( "フィッシュボーン"|"階層化"|"入れ子" )

**説明:** Hier Boxのスタイルを設定する。
...
**説明:** 指定のHier Boxの下位にある子をすべて非表示にする。
...
**構文:** obj &lt;&lt; Direction( 0|1 )

**説明:** HierBox内の文字列を右(1)と左(0)のどちらに置くかを設定する。
...
**構文:** obj &lt;&lt; Force Alternate

**説明:** HierBoxを兄弟HierBoxと交互に並ぶよう移動させる。
...
**説明:** HierBox内の文字列を右(1)と左(0)のどちらに傾けるかを設定する。
...
**説明:** HierBox内の文字列を含んだ新しいデータテーブルを作成する。
...
**説明:** HierBoxを他のHierBoxの反対側に移動させる。
...
**構文:** obj &lt;&lt; Override Descendant Padding( state=0|1 )

**説明:** すべての子孫に同じ所定のパディング設定を適用する場合に、1を指定する。
...
**構文:** obj &lt;&lt; Pad Current Node( state=0|1 )

**説明:** 現在のノードに所定のパディング設定を適用する場合に、1を指定する。
...
**構文:** obj &lt;&lt; Siblings Horizontal Pad( number )

**説明:** Hire Box間の通常のパディング。
...
**構文:** obj &lt;&lt; Siblings Total Pad( number )

**説明:** 兄弟のHire Boxボックスとその親ボックスの間の縦方向のパディング。
...
**構文:** obj &lt;&lt; Siblings Vertical Pad( number )

**説明:** 90度回転させたときのHireBox間のパディング。
...
**構文:** obj &lt;&lt; Uneditable( state=0|1 )

**説明:** 有効(1)なら、文字列をマウスでクリックして編集することができる。
