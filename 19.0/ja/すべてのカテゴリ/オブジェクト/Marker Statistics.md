...
**構文:** obj = Marker Statistics(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**説明:** 指定された列の水準ごとに、個別に分析を実行する。すべての結果が、1つのデータテーブルや1つのレポートにまとめる。
...
**構文:** obj &lt;&lt; With Marker( column(s) )

**説明:** ペアごとに連鎖不平衡分析を行うため、ペアのもう一方のほうの遺伝子マーカーを含む列を指定する。
...
**構文:** obj &lt;&lt; Counts Table( state=1 )

**説明:** 遺伝子型の度数表の表示/非表示を切り替える。 デフォルトではオン。
...
**構文:** obj &lt;&lt; LD Decay Plot( state=1 )

**説明:** LD減衰プロットの表示/非表示を切り替える。 デフォルトではオン。
...
**構文:** obj &lt;&lt; Levels Table( state=1 )

**説明:** 遺伝子型の水準数を列挙した表の表示/非表示を切り替える。 デフォルトではオン。
...
**構文:** obj = Marker Statistics(...Marker Format( "数値"|"一文字"|"文字"|"1コードのヌクレオチド"|"ヌクレオチド"="数値" )...)

**説明:** 遺伝子型の形式を指定する。指定できるオプションは、「一文字(Single-Character)」(A, B, H)、「1コードのヌクレオチド(Single-Code Nucletide)」(IUPAC: A,C,G,T,R,Y,S,W,K,M,+,0,-,N)、「ヌクレオチド(Nucleotide)」(AA, CC, GG, TT, AC, AT...)、「数値(Numeric)」(0, 1, 2, ..., p)、「文字(Character)」(ApBpまたはAp/Bp)。「数値」と「文字」のどちらの形式も、p倍数性の遺伝子型を扱える。 デフォルトの値は"数値"。

### PValue Plot

**構文:** obj &lt;&lt; PValue Plot( state=1 )

**説明:** p値プロットの表示/非表示を切り替える。 デフォルトではオン。
...
**構文:** obj = Marker Statistics(...PValues Table on Launch( state=0 )...)

**説明:** プラットフォームの起動時にp値のデータテーブルを作成するかどうかを指定する。 デフォルトの値は"0"。
...
**構文:** obj = Marker Statistics(...Ploidy( number=2 )...)
...
**構文:** obj &lt;&lt; Recode Marker

**説明:** 元のデータテーブルにおける選択されたマーカー列のデータにおいて、マイナーアレルとメジャーアレルを入れ替える。
...
**構文:** obj &lt;&lt; Result Table( state=1 )

**説明:** 結果の表を表示する。 デフォルトではオン。
...
**構文:** obj &lt;&lt; Save Levels Table

**説明:** 水準を示した表をデータテーブルに保存する。
...
**構文:** obj &lt;&lt; Save Proportion of Missing Markers

**説明:** 元のデータテーブルに、遺伝子型が欠測となっている割合を示す列を追加する
...
**構文:** obj &lt;&lt; Select and Recode Marker

**説明:** 元のデータテーブルのマーカー列において、マイナーアレルの値とメジャーアレルの値を入れ替える。
...
**構文:** obj = Marker Statistics(...Unthreaded( state=0|1 )...)
...
**構文:** obj = Marker Statistics(...Use Annotation Table( state=0|1 )...)
