...
**说明:** 返回一个显示框，它可以交互式控制大小，以水平或垂直方式组织其他显示框。子级大小指定为一定比例的 Splitter Box 宽度或高度。可选 Size 参数仅用于最顶层的拆分窗口框；低级别框的大小与其他子级框类似。
...
**语法:** obj &lt;&lt; Close Panel( index,&lt;previous&gt; )

**说明:** 关闭给定索引处的面板。若 previous 参数为 1，则空间将分配给列表中的前一个框，否则它将分配给下一个框。
...
**语法:** obj &lt;&lt; Divider Color( color );color = obj &lt;&lt; Get Divider Color

**说明:** 指定或返回每条分隔线的颜色。若未设置，则使用基于背景颜色的颜色。
...
**语法:** obj &lt;&lt; Divider Length( number );number = obj &lt;&lt; Get Divider Length

**说明:** 指定或返回每条分隔线的长度百分比。值为 0.0 会导致分隔线不可见。值为 1.0 时将生成（默认）全长分隔线。
...
**语法:** obj &lt;&lt; Divider Thickness( number );number = obj &lt;&lt; Get Divider Thickness

**说明:** 指定或返回每条分隔线的线宽。该值必须为非零。
...
**语法:** {size1,...} = obj &lt;&lt; Get Sizes

**说明:** 返回 Splitter Box 每个面板的比例。
...
**语法:** obj &lt;&lt; Open Panel( index,&lt;previous&gt; )

**说明:** 打开给定索引处的面板。若 previous 参数为 1，则空间将从列表中的前一个框获得，否则它将从下一个框获得。
...
**语法:** obj &lt;&lt; Set Sizes( {size1,...} )

**说明:** 设置 Splitter Box 每个面板的比例。
...
**语法:** state=0|1 = obj &lt;&lt; get horizontal

**说明:** 返回列表框是水平的 (1) 还是垂直的 (0)。
...
**语法:** obj &lt;&lt; set horizontal( state=0|1 )

**说明:** 将列表框设置为水平 (1) 或垂直 (0)。
