...
**语法:** obj = Marker Statistics(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;启动窗口项: 是&lt;/b&gt;
...
**说明:** 单独分析指定给指定列的每个水平的行。所有结果显示在单个表和报表中。
...
**语法:** obj &lt;&lt; With Marker( column(s) )

**说明:** 指定包含基因型标记的列，它使用“标记”中指定的基因型标记的列进行配对连锁不平衡估计。
...
**语法:** obj &lt;&lt; Counts Table( state=1 )

**说明:** 显示或隐藏按基因型水平计数的表。 默认开启。
...
**语法:** obj &lt;&lt; LD Decay Plot( state=1 )

**说明:** 显示或隐藏“LD 衰减图”。 默认开启。
...
**语法:** obj &lt;&lt; Levels Table( state=1 )

**说明:** 显示或隐藏基因型水平的表。 默认开启。
...
**语法:** obj = Marker Statistics(...Marker Format( "数值"|"单字符"|"字符"|"单码核苷酸"|"核苷酸"="数值" )...)&lt;b&gt;启动窗口项: 是&lt;/b&gt;

**说明:** 提供用于选择基因型格式的选项: 单字符 (A, B, H)，单码核苷酸 (IUPAC: A,C,G,T,R,Y,S,W,K,M,+,0,-,N)，核苷酸（AA、CC、GG、TT、AC、AT 等），数值 (0, 1, 2, ..., p) 以及字符（ApBp 或 Ap/Bp）。“数值”和“字符”格式均处理具有 p 倍性的基因型。 默认为“数值”。

### PValue Plot

**语法:** obj &lt;&lt; PValue Plot( state=1 )
...
**语法:** obj = Marker Statistics(...PValues Table on Launch( state=0 )...)

**说明:** 指定当平台启动时是否创建“p 值”数据表。 默认为“0”。
...
**语法:** obj = Marker Statistics(...Ploidy( number=2 )...)&lt;b&gt;启动窗口项: 是&lt;/b&gt;

**说明:** 指定指示倍性水平的正偶数。 默认为“2”。
...
**语法:** obj &lt;&lt; Recode Marker

**说明:** 通过切换次要和主要等位基因，重新编码原始数据表中的所有选定标记列。
...
**语法:** obj &lt;&lt; Save Proportion of Missing Markers

**说明:** 将一列添加至包含缺失基因型比例的原始数据表
...
**语法:** obj &lt;&lt; Select and Recode Marker

**说明:** 通过切换次要和主要等位基因，选择原始数据表中的标记列并对其重新编码。
...
**语法:** obj = Marker Statistics(...Unthreaded( state=0|1 )...)&lt;b&gt;启动窗口项: 是&lt;/b&gt;
...
**语法:** obj = Marker Statistics(...Use Annotation Table( state=0|1 )...)&lt;b&gt;启动窗口项: 是&lt;/b&gt;
