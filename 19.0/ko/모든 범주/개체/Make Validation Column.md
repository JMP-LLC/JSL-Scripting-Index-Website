...
**구문:** obj = Make Validation Column(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj &lt;&lt; Cutpoint Batch ID( column )
...
**구문:** obj &lt;&lt; Grouping Columns( column(s) )
...
**구문:** obj &lt;&lt; Stratification Columns( column(s) )
...
**구문:** obj &lt;&lt; Assign Extra Rows( "훈련 데이터"|"검증 데이터"|"테스트 데이터" )

**설명:** 훈련, 검증 또는 테스트 데이터 집합에 추가 행을 할당할지 여부를 지정합니다. 이 옵션은 &apos;행 수&apos;를 &apos;절단점 결정 기준&apos; 옵션으로 지정하고, 절단점 배치 ID 변수를 지정한 경우에만 사용할 수 있습니다.
...
**구문:** obj &lt;&lt; Determine cutpoints using( "비율"|"행 수"|"고정 시간 또는 날짜"|"경과 시간" )

**설명:** 절단점을 결정하는 데 사용되는 방법을 지정합니다.
...
**구문:** obj &lt;&lt; New Column Name( text )

**설명:** 새로 생성되는 검증 열의 이름을 지정합니다.
...
**구문:** obj &lt;&lt; Number of Folds( number=4 )

**설명:** K 폴드 교차 검증에 사용되는 폴드 수를 지정합니다. 기본값은 "4"입니다.
...
**설명:** 검증 열 생성을 위한 난수 시드값을 지정합니다. 동일한 방법을 사용할 경우 동일한 검증 열을 다시 생성하려면 동일한 난수 시드값을 사용합니다.

```jsl

dt = Open( "$SAMPLE_DATA/Big Class.jmp" );
dt << Make Validation Column( Training Set( 0.75 ), Validation Set( 0.25 ), Random Seed( 12321 ), Go );

```

### Test Set

**구문:** obj &lt;&lt; Test Set( number=0.00 )

**설명:** 테스트 데이터 집합에 할당할 관측값의 비율을 지정합니다. 테스트 데이터 집합은 선택적 데이터 집합으로, 모형 선택 후 모형의 예측 능력을 확인하는 데 사용됩니다. 기본값은 "0.00"입니다.
...
**구문:** obj &lt;&lt; Training Set( number=0.75 )

**설명:** 훈련 데이터 집합에 할당할 관측값의 비율을 지정합니다. 훈련 데이터 집합은 모형을 추정하는 데 사용됩니다. 기본값은 "0.75"입니다.
...
**구문:** obj &lt;&lt; Validation Column Type( 고정|계산식 )

**설명:** 새 검증 열의 열 유형(fixed 또는 formula)을 선택합니다. 기본값은 고정 열입니다.
...
**구문:** obj &lt;&lt; Validation Set( number=0.25 )

**설명:** 검증 데이터 집합에 할당할 관측값의 비율을 지정합니다. 검증 데이터 집합은 보다 정확한 예측이 가능한 모형을 선택하는 데 사용됩니다. 기본값은 "0.25"입니다.
