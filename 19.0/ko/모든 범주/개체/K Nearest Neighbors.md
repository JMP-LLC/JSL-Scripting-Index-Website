...
**구문:** obj = K Nearest Neighbors(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;실행기 항목: 예&lt;/b&gt;
...
**구문:** obj = K Nearest Neighbors(...&lt;By( column(s) )&gt;...)
...
**구문:** obj = K Nearest Neighbors(...Factor( column(s) )...)
...
**구문:** obj = K Nearest Neighbors(...&lt;Validation( column )&gt;...)
...
obj = dt << K Nearest Neighbors( Y( :Y ), X( :Age, :Gender, :BMI ), Validation( :Validation ), K( 10 ) );

```

### X

**구문:** obj = K Nearest Neighbors(...X( column(s) )...)
...
**구문:** obj = K Nearest Neighbors(...&lt;Y( column(s) )&gt;...)
...
**구문:** obj = K Nearest Neighbors(...Category Bias( number=0.5 )...)

**설명:** 범주형 반응의 적합 확률이 항상 양수가 되도록 보증하는 조정 모수를 지정합니다. 기본값은 "0.5"입니다.
...
**구문:** obj = K Nearest Neighbors(...K( number=10 )...)

**설명:** 분석할 최근접 이웃의 최대 개수를 설정합니다. 기본값은 "10"입니다.
...
dt = Open( "$SAMPLE_DATA/Car Poll.jmp" );
obj = K Nearest Neighbors( Y( :country ), X( :sex, :marital status, :age, :type, :size ), K( 8 ) );

```

### Response

**구문:** obj &lt;&lt; Response( "&lt;Response Variable Name&gt;", &lt;Set K( number )&gt;, &lt;Mosaic Plot ( state=0|1 ) &gt;, &lt;Plot Actual by Predicted( state=0|1 )&gt;, &lt;Plot Residual by Predicted( state=0|1 )&gt; )

**설명:** 모형 반응에 사용할 수 있는 보고서 옵션을 지정합니다. 사용 가능한 옵션은 반응 유형에 따라 달라집니다.
...
obj = dt << K Nearest Neighbors(
	...
	K( 10 ),
	Response( "Y", Set K( 6 ), Plot Actual by Predicted( 1 ), Plot Residual by Predicted( 1 ) )
);
...
**구문:** obj &lt;&lt; Save Near Neighbor Distances

**설명:** k번째 최근접 데이터 점까지의 거리를 저장합니다.
...
dt = Open( "$SAMPLE_DATA/Diabetes.jmp" );
obj = K Nearest Neighbors( Y( :Y ), X( :Age, :Gender, :BMI ), Validation( :Validation ), K( 10 ) );
...
**구문:** obj &lt;&lt; Save Near Neighbor Rows

**설명:** K 최근접 이웃의 행 번호를 저장합니다.
...
**구문:** obj = K Nearest Neighbors(...Set Random Seed( number )...)
...
**구문:** obj &lt;&lt; Use Excluded Rows for Validation( state=0|1 )
...
**구문:** obj = K Nearest Neighbors(...Validation Portion( fraction=0 )...)
...
obj2 = K Nearest Neighbors( Y( :Y ), X( :Age, :Gender, :BMI ), Validation( :Validation ), K( 5 ) );
Wait( 1 );
...
**구문:** obj &lt;&lt; (Response[number] &lt;&lt; Get Best K)

**설명:** 최상의 K 이웃의 값을 반환합니다.
...
**구문:** obj &lt;&lt; (Response[number] &lt;&lt; Get Prediction Formula( k ))
...
**설명:** 데이터에 대한 모자이크 그림을 표시하거나 숨깁니다. 기본적으로 설정되어 있습니다.
...
**구문:** obj &lt;&lt; (Response[number] &lt;&lt; Plot Actual By Predicted( state=0|1 ))
...
**구문:** obj &lt;&lt; ( Response[number] &lt;&lt; Plot Residual By Predicted( state=0|1 ) )

**설명:** Y 축에 잔차를 표시하고 X 축에 예측값을 표시합니다. 적합 또는 변동에서 패턴을 파악하려면 산점도를 사용하십시오.
...
**구문:** obj &lt;&lt; (Response[number] &lt;&lt; Publish Prediction Formula( k ))
...
**구문:** obj &lt;&lt; (Response[number] &lt;&lt; Save Predicteds)
...
**구문:** obj &lt;&lt; (Response[number] &lt;&lt; Save Prediction Formula( k ))
...
**구문:** obj &lt;&lt; ( Response[number] &lt;&lt; Set K( number ) )
