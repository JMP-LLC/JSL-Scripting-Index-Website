...
**说明:** 模拟对按钮的点击，若合适的话运行任何回调。切换按钮将在调用回调之前更改状态。菜单按钮需要一个参数以指定要点击的项。
...
**说明:** 若按下切换按钮则返回 1，若没按下则返回 0。仅适用于具有 Style("Toggle") 的按钮。
...
**语法:** left|right = obj << Get Icon Location

**说明:** 返回按钮框中图标的位置。图标可以显示在按钮文本的左侧或右侧。
...
**语法:** list of menu items = obj << Get Menu Items

**说明:** 获取点击按钮时所弹出菜单中的菜单项
...
**语法:** style = obj << Get Underline Style
...
**语法:** obj << Open Next Outline

**说明:** 关闭包含按钮的分级显示框，然后打开下一个。
...
**语法:** obj << Restore Focus After Click( state=0|1 )

**说明:** 将按钮设置为点击后保留编辑焦点。Windows 的默认设置为获取焦点。Macintosh 的默认设置为保留焦点。
...
**语法:** obj << Set( pressed, <run script(0|1)> )

**说明:** 将切换按钮的切换状态设置为按下 (1) 或未按下 (0)。仅适用于具有 Style("Toggle") 的按钮。
...
**语法:** obj << Set Function( Function( {thisBox}, <script> ) | Method({thisBox}, <script>) | <symbol> )

**说明:** 设置控件的指定函数。当按下按钮时该函数的脚本运行。
...
**语法:** obj << Set Icon( icon name|image filepath )

**说明:** 在按钮上显示 JMP 图标或图像。使用 JMP 图标名称或图像的文件路径。
...
**语法:** obj << Set Icon Location( left,right )

**说明:** 设置图标在按钮文本左侧或右侧的位置
...
**语法:** obj << Set Menu Items( list of menu items )

**说明:** 设置点击按钮时所弹出菜单中的菜单项
...
**语法:** obj << Underline Style( state=0|1 )

**说明:** 将按钮设置为使用下划线绘制样式，而不是标准按钮样式。
...
**语法:** obj << Unhide Next Item

**说明:** 在包含按钮的分级显示框中显示隐藏项。
