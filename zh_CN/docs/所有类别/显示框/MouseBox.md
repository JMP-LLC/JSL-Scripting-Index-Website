...
**语法:** GetChildBox( { x, y } | [ x y ] | x, y )

**说明:** 返回该鼠标框内指定坐标处的零个或多个嵌套显示框的列表: { 子级, 孙级, 重孙级, ... }。在很多情况下，在每个子级周围使用一个单独的鼠标框可能更加简便。
...
**语法:** obj << GetClickEnable

**说明:** 报告由 SetClickEnable 设置的值
...
**语法:** obj << GetDefaultCursor

**说明:** 检索由 SetDefaultCursor 指定的值

### GetDefaultCursorEnable

**语法:** obj << GetDefaultCursorEnable

**说明:** 检索由 SetDefaultCursorEnable 指定的值
...
**说明:** 返回由 <<SetDragBegin 指定的函数

### GetDragEnable

**语法:** obj << GetDragEnable

**说明:** 询问当前是否允许 MouseBox 作为拖放操作的始发端
...
**说明:** 返回由 <<SetDragEnd 指定的函数

### GetDragText

**语法:** obj << GetDragText

**说明:** 通常不需要。返回由 SetDragText 提供的文本，该文本可能不是发送到释放点的文本。
...
**语法:** obj << GetDropCommit

**说明:** 返回由 <<setDropCommit 设置的函数
...
**语法:** obj << GetDropEnable

**说明:** 报告由 <<SetDropEnable 储存的值
...
**语法:** obj << GetEditClear( state=0|1 )

### GetEditCopy

**语法:** obj << GetEditCopy( state=0|1 )

### GetEditCopyLabel

**语法:** obj << GetEditCopyLabel( state=0|1 )

### GetEditCopyText

**语法:** obj << GetEditCopyText( state=0|1 )

### GetEditCut

**语法:** obj << GetEditCut( state=0|1 )

### GetEditEnable

**语法:** obj << GetEditEnable

### GetEditJournal

**语法:** obj << GetEditJournal( state=0|1 )

### GetEditPaste

**语法:** obj << GetEditPaste( state=0|1 )

### GetEditPasteJSL

**语法:** obj << GetEditPasteJSL( state=0|1 )

### GetEditPasteLabel

**语法:** obj << GetEditPasteLabel( state=0|1 )

### GetEditSaveSelectionAs

**语法:** obj << GetEditSaveSelectionAs( state=0|1 )

### GetEditSubmit

**语法:** obj << GetEditSubmit( state=0|1 )

### GetEditSubmitDebug

**语法:** obj << GetEditSubmitDebug( state=0|1 )
...
**语法:** obj << GetMarkEnable

**说明:** 检索由 <<SetMarkEnable 设置的值
...
**说明:** 通常不需要。返回由 SetTooltip 提供的文本。工具提示是自动的，不需要该消息。
...
**语法:** obj << GetTrackEnable

**说明:** 检索由 <<SetTrackEnable 设置的值
...
**语法:** obj << SetClick( Function( {this, clickpt, event}, <script> ) )

**说明:** 提供处理按钮按下（或首次按下或释放）时的鼠标事件的函数。关于按钮未按下状态时的信息，请参见 <<SetTrack()。clickpt 是该 MouseBox 内被点击的 {x,y} 位置。event 是点击事件。值为（按它们发生的顺序）:“Pressed”、“Ticked”或“Released”。
...
**语法:** obj << SetClickEnable( state=0|1 )

**说明:** 使 MouseBox 能够通过 <<SetClick 函数处理按钮按下、鼠标移动、按钮释放等操作。

### SetCursor

**语法:** obj << SetCursor( "箭头"|"上下箭头"|"左右箭头"|"左上到右下箭头"|"右上到左下箭头"|"手指"|"手形" )

**说明:** <<SetCursor("Hand")（或 Finger、Arrow 或 NS EW NWSE NESW）将设置光标。在 <<SetTrack 或 <<SetClick 函数中使用它
...
**语法:** obj << SetDefaultCursor( "箭头"|"上下箭头"|"左右箭头"|"左上到右下箭头"|"右上到左下箭头"|"手指"|"手形" )

**说明:** 若还指定了 SetDefaultCursorEnable(1)，则指定要显示的默认光标；使用“Arrow”、“NS”、“EW”、“NWSE”、“NESW”、“Finger”或“Hand”之一。不同于 SetCursor，其在 SetTrack 脚本中使用，而 SetDefaultCursor 不需要脚本。
...
**语法:** obj << SetDefaultCursorEnable( state=0|1 )

**说明:** 使 MouseBox 显示由 SetDefaultCursor 指定的光标

### SetDragBegin

**语法:** obj << SetDragBegin( Function( {this, clickpt}, <script> ) )

**说明:** 提供在拖放操作开始时调用的函数。该函数可以通过返回 0.0 禁止拖动，或者通过返回字符串（不使用 <<SetDragText）或返回 1.0（使用 <<SetDragText）允许拖动。clickpt 是 MouseBox 内拖动开始的 {x,y} 位置。
...
**语法:** obj << SetDragEnable( state=0|1 )

**说明:** MouseBox 可以作为拖放操作的始发端
...
**语法:** obj << SetDragEnd( Function( {this, clickpt, how}, <script> ) )

**说明:** 提供在拖放操作结束时调用的函数。how 报告操作的结束情况，值为“移动”或“忽略”。若操作以“移动”结束，则可以使用该函数清空拖放源位置。clickpt 是 MouseBox 内拖动开始的 {x,y} 位置。
...
**语法:** obj << SetDropCommit( Function( {this, clickpt, text}, <script> ) )

**说明:** 提供在按钮释放事件发生、拖放过程接近结束时调用的函数。该提交函数可以接收拖放源提供的 text，并根据需要使用它，例如设置 TextBox 的内容。clickpt 是该 MouseBox 内拖动结束的 {x,y} 位置。
...
**语法:** obj << SetDropEnable( state=0|1 )
...
**语法:** obj << SetDropTrack( Function( {this, clickpt}, <script> ) )

**说明:** 当拖放操作拖过 MouseBox 时提供要调用的函数。提供的函数返回 0.0 以禁止释放或返回 1.0 允许释放。在鼠标按钮松开前不会发生实际释放。在释放按钮时会调用 SetDropCommit 函数以处理释放的文本，前提是 setDropTrack 返回 1.0。clickpt 是 MouseBox 内正在拖动另一 MouseBox 的 {x,y} 位置。当拖动不是发生在 this MouseBox 上时，它为 {-1, -1}。
...
**语法:** obj << SetEditClear( state=0|1 )

### SetEditCopy

**语法:** obj << SetEditCopy( state=0|1 )

### SetEditCopyLabel

**语法:** obj << SetEditCopyLabel( state=0|1 )

### SetEditCopyText

**语法:** obj << SetEditCopyText( state=0|1 )

### SetEditCut

**语法:** obj << SetEditCut( state=0|1 )

### SetEditEnable

**语法:** obj << SetEditEnable

### SetEditJournal

**语法:** obj << SetEditJournal( state=0|1 )

### SetEditPaste

**语法:** obj << SetEditPaste( state=0|1 )

### SetEditPasteJSL

**语法:** obj << SetEditPasteJSL( state=0|1 )

### SetEditPasteLabel

**语法:** obj << SetEditPasteLabel( state=0|1 )

### SetEditSaveSelectionAs

**语法:** obj << SetEditSaveSelectionAs( state=0|1 )

### SetEditSubmit

**语法:** obj << SetEditSubmit( state=0|1 )

### SetEditSubmitDebug

**语法:** obj << SetEditSubmitDebug( state=0|1 )
...
**语法:** obj << SetKey( Function( {this, key}, <script> ) )

**说明:** 设置当鼠标框具有焦点时按下某个键时要调用的函数。若处理了键，该函数应返回 1，若未处理，则应返回 0。
...
**语法:** obj << SetMark( Function( {this}, <script> ) )

**说明:** SetMark 指定框具有焦点状态下点击鼠标或按下回车键时将调用的函数。标记可用于实现与 Mouse Box 关联的选择状态。要实现点击操作，使用 <<SetClick。
...
**语法:** obj << SetMarkEnable( state=0|1 )

**说明:** 使 MouseBox 在框具有焦点状态下点击鼠标或按下回车键时调用 <<SetMark 函数。
...
**语法:** obj << SetMarked( state=0|1 )

**说明:** 设置 Mouse Box 的已标记标志。已标记框以突出显示的背景色显示。
...
**语法:** obj << SetTrack( Function( {this, clickpt}, <script> ) )

**说明:** 指定鼠标按钮未按下状态下滑过 MouseBox 时将调用的函数。关于按钮按下状态时的信息，请参见 <<SetClick。clickpt 是该 MouseBox 内光标当前位置的 {x,y} 位置。
...
**语法:** obj << SetTrackEnable( state=0|1 )

**说明:** 使 MouseBox 在鼠标按钮未按下状态下移动时调用 <<SetTrack 函数。

### SetUserData

**语法:** obj << SetUserData

**说明:** 在 MouseBox 中储存 JSL 值；值可以为数字、字符串、列表、关联数组或其他 JSL 类型
