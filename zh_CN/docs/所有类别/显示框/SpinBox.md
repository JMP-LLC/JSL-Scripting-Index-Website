...
**说明:** 返回显示框以显示带向上和向下控件的按钮。调用 script 参数，其带一个参数指示所点击箭头的方式（负值表示向下，正值表示向上）。数量级 1 表示单击。较大的值可用于表示重复操作。
...
**语法:** obj << Decrement( <value> )

**说明:** 模拟点击向下箭头，运行任意附带的脚本。
...
**语法:** obj << Increment( <value> )

**说明:** 模拟点击向上箭头，运行任意附带的脚本。
...
**语法:** obj << Set Function( Function( {thisBox, increment}, <script> ) )

**说明:** 设置控件的指定函数。当按下数字显示框的按钮时该函数的脚本运行。[increment] 是数字框的增量设置。
