...
**语法:** result = jmp.from_arrow(&lt;library.Dataframe&gt;, requested_schema=&lt;schema&gt;, visibility=&lt;string&gt;, preserve_index=&lt;bool&gt;)

**说明:** 从实现 PyArrow Capsule 接口的 Dataframe 创建 jmp.DataTable。
...
**说明:** 小型表更新速度快，但对大型表执行列批量更改时同步更新用户界面会非常耗时。begin_update() 方法会暂停数据表 GUI 更新，直至执行对应的 end_update()。
