...
**语法:** obj = Explore Missing Values(...Window View( "Visible"|"Invisible"|"Private" )...)&lt;b&gt;启动窗口项: 是&lt;/b&gt;
...
**语法:** obj &lt;&lt; ADI Loading Matrix( state=0|1 )

**说明:** 显示或隐藏报表，该报表显示与每个成分的因子载荷对应的列。
...
**语法:** obj &lt;&lt; Automated Data Imputation

**说明:** 使用低秩矩阵近似方法插补缺失值。该方法自动根据数据选择低秩近似的最佳维。
...
**说明:** 对数据表中的单元格着色，这些单元格在您在“缺失列”报表中选择的列中包含缺失值。
...
**说明:** 对数据表中的行着色，这些行在您在“缺失列”报表中选择的列中包含缺失值。
...
**说明:** 对数据表中的行应用已排除行状态，这些行在您在“缺失列”报表中选择的列中包含缺失值。
...
**语法:** obj &lt;&lt; Get U V Sigma ADI Matrices

**说明:** 从 ADI 方法中的低秩近似返回 U、V 和 Sigma 矩阵。
...
**语法:** obj &lt;&lt; Maximum Dimension( number )
...
**语法:** obj &lt;&lt; Maximum Iteration( number=10 )

**说明:** 设置自动数据插补的最大迭代次数。 默认为“10”。
...
**语法:** obj &lt;&lt; Missing Value Clustering
...
**语法:** obj &lt;&lt; Missing Value Report

**说明:** 打开“缺失列”报表，它列出每列的名称和该列中的缺失值个数。
...
**语法:** obj &lt;&lt; Missing Value Snapshot

**说明:** 显示缺失值的方格图。黑色方格指示缺失值。
...
**语法:** obj &lt;&lt; Multivariate Normal Imputation( Shrink Covariances( state=0|1 ) )

**说明:** 基于多元正态分布对缺失值进行插补。为了改进协方差矩阵的估计，使用收缩估计量选项。
...
**语法:** obj &lt;&lt; Multivariate RPCA Imputation( Lambda( number ), Tolerance( number = 1e-7 ), MaxIt( number ) )

**说明:** 使用稳健主成分插补缺失值，它使用对离群值稳健的低秩矩阵因子分解 (SVD) 替换缺失值。该方法对于宽问题很有用。lambda 的默认值是 2/sqrt(max(n, p))，其中 n 是行数，p 是列数。若 min(n, p) < 100，则最大迭代次数 (MaxIt) 的默认值是 75。若 100<= min(n, p) < 1000，则 MaxIt 的默认值是 100。若 min(n,p) >= 1000，则 MaxIt 的默认值是 200。若算法在指定的最大迭代次数之后不收敛，则接受 MaxIt 处的解并在报表中显示。
...
**语法:** obj &lt;&lt; Multivariate SVD Imputation( Number of Singular Vectors( number ), Maximum Iterations( number ), Show Iteration Log( state=0|1 ) )

**说明:** 使用迭代的低秩 SVD 矩阵完成方法快速为大问题插补缺失值。
...
**语法:** obj &lt;&lt; Options for Saving Imputed Values(1|2|3)

**说明:** 指定用于保存 ADI 方法的插补值的方法。输入 1 以指定“新建数据表”选项，2 指定“将得分公式保存至当前数据表”选项，3 指定“原位插补值”选项。
...
**说明:** 选择数据表中的行，这些行在您在“缺失列”报表中选择的列中包含缺失值。
...
**语法:** obj &lt;&lt; Show only columns with missing( state=0|1 )
...
**语法:** obj &lt;&lt; Validation Proportion( number=0.3 )

**说明:** 设置用作自动数据插补的验证行的行比例。 默认为“0.3”。
