...
**说明:** 选择 XGBoost 或 LightGBM 作为梯度提升拟合的方法。 默认为“xgboost”。
...
**说明:** 指定分类特征使用“一对其他”拆分算法时可具有的最大类别数。超过此最大类别数的分类特征将采用其他算法进行处理。 默认为“4”。
...
**说明:** 指定在指定了预测提前停止时，预测提前停止的阈值裕度。若裕度离阈值足够远，该参数支持预测过程提前停止。 默认为“10”。
...
**语法:** obj &lt;&lt; top_k( number=0 )

**说明:** 指定要在贪婪和节俭特征选择器中选择的顶级特征个数。该选项仅适用于 gblinear 提升器。 默认为“0”。
...
**说明:** 指定是否在 DART 提升中使用均匀概率选择要丢弃的树。 默认为“0”。
