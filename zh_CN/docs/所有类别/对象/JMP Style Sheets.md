...
**说明:** JMP Style Sheets (JSS) helps us separate DisplayBox content from DisplayBox presentation. The system is modeled on CSS. CSS is to HTML as JSS is to DisplayBox trees in JMP.
...
**说明:** Use DisplayBoxPtr::jssClasses and DisplayBoxPtr::jssID and then target those using selectors. OutlineBoxes and some others automatically set classes from their Scriptable, etc.
...
**说明:** The main unit of a JSS file. Composed of a Selector and multiple Declarations/Messages.
...
**说明:** The first part of a Rule; Used to select boxes. You can compose selectors together to make more complex selectors.
...
**说明:** This helps us be more declarative with our Rules. See CSS documentation for details.
...
**语法:** And( selector, ..., selector )

**说明:** Combine two or more selectors, making a new selector that matches only when all of those match. Use the functional form And(...) or wrap in parentheses before sending messages.
...
**语法:** Attr( <<Boolean Box Message == 0|1 )

**说明:** Matches boxes with the given attribute. Where the box responds to the given message with that value. Currently only supports boolean attributes.
...
**语法:** Child( parentSelector, childSelector )

**说明:** Combines two selectors into a direct parent/child path. A box matches when it and its direct parent both match their individual selectors.
...
**说明:** Matches boxes that have the given JSS class (loose comparison).
...
**语法:** Constants( var1 = expr1; var2 = expr2; ... )

**说明:** Defines constants that can be used within rule declarations. Basic operations are allowed in the expressions and the names can reference each other. Names are global within a Theme and the last declaration of a name is used.
...
**语法:** Descend( selector, ..., selector )

**说明:** Combines two or more selectors into a path. A box matches when the path to the box matches. Note that the path can skip levels in the display tree.
...
**说明:** Matches when a box is the first child of its parent.
...
**说明:** Matches boxes that have the given JSS id (loose comparison).
...
**语法:** If( condA, rulesA, ..., rulesElse )

**说明:** The allowable condition expressions are limited and resolved when the JSS is parsed. No branch testing is done during the JSS application passes. In other words, these are for testing the state of the system and JMP, not individual boxes or box trees.
...
**语法:** Include( path, <<NoWarnIfMissing )

**说明:** Includes another JSS file from a path relative to this one. Errors within that file will be ignored. Errors accessing that file will not, unless you use the <<NoWarnIfMissing flag.
...
**说明:** Matches when a box is the last child of its parent.
...
**语法:** Next Sibling( precedingSelector, selector )

**说明:** Box must match the second selector and be immediately preceded by a box matching the first selector (sharing the same parent).
...
**语法:** Or( selector, ..., selector )

**说明:** Combines two or more selectors, making a new selector that matches when any of the individual selectors match. Use the functional form Or(...) or wrap in parentheses before sending messages.
...
**说明:** Matches boxes of the given type (loose comparison).
...
**说明:** Matches all boxes. Useful mainly for clearly expressing intent.
