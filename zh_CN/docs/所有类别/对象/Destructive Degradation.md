...
**语法:** obj = Destructive Degradation(...Window View( "Visible"|"Invisible"|"Private" )...)
...
**语法:** obj << Control( yTrans, timeTrans, distribution, path definition )

**说明:** 在报表的“路径定义”面板中选中模型。所选的模型还显示在退化图中。
...
**语法:** obj << Model( yTrans, timeTrans, distribution, path definition )
...
**语法:** obj << Shade( state=0|1 )

**说明:** 显示或隐藏显示模型的上限和下限。该选项还显示或隐藏界限之间的着色。 默认开启。
...
**语法:** obj << Shade Coverage( proportion )

**说明:** 指定比例以增加或减少着色覆盖率的量。
...
**语法:** obj << Use Condition Temperature( "Celsius"|"Fahrenheit"|"Kelvin", temperature )

**说明:** 指定一阶动力学模型中的使用条件 (X0) 的尺度和温度。
