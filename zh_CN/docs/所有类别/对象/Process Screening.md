...
**说明:** 指定在 Holt 双指数平滑线中用于漂移检测的权重。 默认为“.05”。
...
**说明:** 存储所有偏移信息以便可以稍后使用“保存偏移表”选项保存至数据表。该选项必须在启动脚本中指定。
...
**说明:** 显示或隐藏“计数最大值”和“非负连续”图表类型。 默认开启。
...
**说明:** 重新启动“过程筛选”平台，以创建仅包含原始报表中选定过程的新报表。
...
**说明:** 创建一个包含保存的偏移间距数据的新数据表。该选项要求在启动脚本中指定了“生成详细偏移数据”选项。
...
**说明:** 显示或隐藏汇总表中的“操作”列。当某个点大于“操作上限”或小于“操作下限”时触发该检验。 默认开启。
...
**说明:** 显示或隐藏汇总表中的“警示”列。当某个点大于“警示上限”或小于“警示下限”时触发该检验。
...
**说明:** 显示或隐藏汇总表中的“警示正在增加”列。该列统计以下情况: 当过程正在增强且前一点高于警示上限时，或过程正在减缓且前一点低于警示下限时。
...
**说明:** 指定是否使用 K-Sigma 下限。该选项仅适用于比例图。
...
**说明:** 指定是否使用 K-Sigma 上限。该选项仅适用于比例图。 默认开启。
