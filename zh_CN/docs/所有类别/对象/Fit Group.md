...
**语法:** Fit Group( model1, model2, ... );Fit Group( model1; model2; ... )

**说明:** 将单独的最小二乘、非线性、神经、高斯过程和混合模型拟合组合在共享刻画器的同一输出窗口中。
...
**说明:** 显示或隐藏所有响应的等高线刻画器。您可以同时探索模型项对所有响应的影响。
...
**语法:** obj &lt;&lt; Order by Goodness of Fit

**说明:** 按拟合显著性对报表排序，其中显著性是按每个模型的 R 方统计量来测量的。该选项仅适用于将 R 方统计量显示到平台级别的平台。
...
**说明:** 显示或隐藏所有响应的预测刻画器。您可以同时查看模型项对所有响应的影响。您还可以跨响应执行多个优化。
