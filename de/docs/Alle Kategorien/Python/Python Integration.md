...
**Beschreibung:** DataConnector objects support shallow copying with the standard copy.copy function.
...
**Beschreibung:** Subclasses should override this function to create a connection. The returned connection, in the form of a subclass of jmp.DataSource, enables integration with Query Builder.
...
**Beschreibung:** Subclasses can override this function to open a table directly. This function implements the Open message in JSL (New Data Connector(...) << Open()).
...
**Beschreibung:** DataConnectorField defines a field.
...
**Beschreibung:** jmp.DataConnectorGroupedFields defines fields in named groups. The groups are passed as a list of pairs. The first element of each pair is the name of the group, and the second is a dictionary with the fields. This dictionary has the same format as one used directly as the value for jmp.DataConnectorType.fields.
...
**Beschreibung:** Subclasses must define a class-level variable named "fields" that specifies the configuration options for this type. It should be a dict or a jmp.DataConnectorGroupedFields object.
...
**Beschreibung:** Subclasses can override this function to provide a list of the schemas in the data source. If this function is not overridden or it returns None, the data source is assumed to not support schemas.
...
**Beschreibung:** Subclasses should override this function to provide a list of the tables in the data source. If schemas are supported, this list should include only the tables under the schema that is passed. If schemas are not supported, the schema parameter should be ignored.
...
**Beschreibung:** Subclasses should override this function to get the table data for the named table. If schemas are not supported, the value of the schema argument can be ignored. The function should return a jmp.DataTable or a string containing a path to a file in a data format supported by JMP.
...
**Beschreibung:** Ruft eine globale JSL-Variable als Python-Objekt ab. Gibt None zurück, wenn das Objekt nicht gefunden werden kann. Gibt einen undurchsichtigen Typ für Datentypen zurück, die nicht verarbeitet werden können. Kann die gleichen Objekttypen übertragen wie Python Send() und Python Get().
...
**Beschreibung:** Provides the means to set a value into the JSL script&apos;s &apos;here&apos; namespace. This allows sending back a value that could not be reached by Python Get(). Only variables in the Python global scope can be seen by Python Get(). Supports same object types as Python Get(). Note on JSL namespace scoping, the &apos;here&apos; namespace is local to the script submission.  Code submitted from different script windows will have separate &apos;here&apos; namespaces. Code including another script will have a single &apos;here&apos; namespace. This holds true for a JSL script running Python via Submit() or Execute(), Python scripts calling run_jsl().
...
**Beschreibung:** The jmpex package&apos;s R support version.
...
**Beschreibung:** The jmpex package contains extension interfaces supported by JMP. Currently it contains only an R support module.
...
**Beschreibung:** Diese Funktion kann von JMP aus aufgerufen werden und umhüllt den Python-Befehl pip. Übergeben Sie diesen Funktionsbefehl pip als Zeichenkette und optional eine durch Leerstellen getrennte Zeichenkette der zu installierenden Pakete. Es werden auch Listen mit einzelnen Argumenten akzeptiert. Das Argument packages ist standardmäßig eine leere Zeichenkette. Das Paket „jmputils“ befindet sich am selben Speicherort wie die eingebetteten Python-Standardbibliotheksdateien. Alle Funktionen in jmputils verwenden nur Funktionen der Python-Standardbibliothek oder in JMP integrierte Funktionen.
