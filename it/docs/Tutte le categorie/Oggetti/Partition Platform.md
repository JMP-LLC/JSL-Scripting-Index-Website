...
**Sintassi:** Partition( Y( column ), X( columns ) )
...
**Sintassi:** Partition(Y( column ), X( columns ), Method( "Decision Tree" ))

**Descrizione:** Esegue partizioni ricorsive dei dati per prevedere una risposta; dette anche strutture ad albero di classificazione e regressione.
...
**Sintassi:** obj = Partition(...Initial Splits( condition, {left condition}, {right condition} )...)

**Descrizione:** Descrive le partizioni che vengono effettuate. L&apos;argomento condition specifica il lato sinistro della prima partizione. Gli argomenti {condizione a sinistra} e {condizione a destra} specificano una partizione sul rispettivo lato e questo formato continua ricorsivamente per il numero di partizioni desiderato. Per specificare una partizione a destra e non a sinistra, assegnare l&apos;argomento di sinistra come elenco vuoto. Per specificare una partizione a sinistra e non a destra, omettere l&apos;argomento a destra.
...
**Sintassi:** Missing Value Order( Low(list of numeric columns),High(list of numeric columns))

**Descrizione:** Specifica se i valori mancanti sono trattati come bassi o alti.
...
**Sintassi:** obj = Decision Tree(...Use Excluded Rows for Validation( state=0|1 )...)
