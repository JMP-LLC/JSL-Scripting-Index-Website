...
**Descrizione:** Use DisplayBoxPtr::jssClasses and DisplayBoxPtr::jssID and then target those using selectors. OutlineBoxes and some others automatically set classes from their Scriptable, etc.
...
**Descrizione:** The main unit of a JSS file. Composed of a Selector and multiple Declarations/Messages.

**JMP Versione aggiunta:** 19

### <Selector>

**Sintassi:** obj << <Selector>

**Descrizione:** The first part of a Rule; Used to select boxes. You can compose selectors together to make more complex selectors.

**JMP Versione aggiunta:** 19

### <Specificity>

**Sintassi:** obj << <Specificity>

**Descrizione:** This helps us be more declarative with our Rules. See CSS documentation for details.
...
**Sintassi:** And( selector, ..., selector )

**Descrizione:** Combine two or more selectors, making a new selector that matches only when all of those match. Use the functional form And(...) or wrap in parentheses before sending messages.
...
**Sintassi:** Attr( <<Boolean Box Message == 0|1 )

**Descrizione:** Matches boxes with the given attribute. Where the box responds to the given message with that value. Currently only supports boolean attributes.
...
**Sintassi:** Child( parentSelector, childSelector )

**Descrizione:** Combines two selectors into a direct parent/child path. A box matches when it and its direct parent both match their individual selectors.
...
**Sintassi:** Class( className )

**Descrizione:** Matches boxes that have the given JSS class (loose comparison).
...
**Sintassi:** Constants( var1 = expr1; var2 = expr2; ... )

**Descrizione:** Defines constants that can be used within rule declarations. Basic operations are allowed in the expressions and the names can reference each other. Names are global within a Theme and the last declaration of a name is used.
...
**Sintassi:** Descend( selector, ..., selector )

**Descrizione:** Combines two or more selectors into a path. A box matches when the path to the box matches. Note that the path can skip levels in the display tree.
...
**Descrizione:** Matches when a box is the first child of its parent.
...
**Descrizione:** Matches boxes that have the given JSS id (loose comparison).
...
**Sintassi:** If( condA, rulesA, ..., rulesElse )

**Descrizione:** The allowable condition expressions are limited and resolved when the JSS is parsed. No branch testing is done during the JSS application passes. In other words, these are for testing the state of the system and JMP, not individual boxes or box trees.
...
**Sintassi:** Include( path, <<NoWarnIfMissing )

**Descrizione:** Includes another JSS file from a path relative to this one. Errors within that file will be ignored. Errors accessing that file will not, unless you use the <<NoWarnIfMissing flag.
...
**Sintassi:** JMP Style Sheets

**Descrizione:** JMP Style Sheets (JSS) helps us separate DisplayBox content from DisplayBox presentation. The system is modeled on CSS. CSS is to HTML as JSS is to DisplayBox trees in JMP.
...
**Descrizione:** Matches when a box is the last child of its parent.
...
**Sintassi:** Next Sibling( precedingSelector, selector )

**Descrizione:** Box must match the second selector and be immediately preceded by a box matching the first selector (sharing the same parent).
...
**Sintassi:** Or( selector, ..., selector )

**Descrizione:** Combines two or more selectors, making a new selector that matches when any of the individual selectors match. Use the functional form Or(...) or wrap in parentheses before sending messages.
...
**Sintassi:** Type( boxTypeName )

**Descrizione:** Matches boxes of the given type (loose comparison).
...
**Descrizione:** Matches all boxes. Useful mainly for clearly expressing intent.
