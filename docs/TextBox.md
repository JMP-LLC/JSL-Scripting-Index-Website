...
**Description:** Indicates that the text might contain HTML markup and should be interpreted. The HTML tags supported are bold (<b></b>), italic (<i></i>), underlined(<u></u>), font face(<font face=&apos;&apos;></font>), font color(<font color=&apos;&apos;></font>), font size(<font size=&apos;&apos;></font>, and background color(<background color=&apos;&apos;></background>). For font color and background color, you can specify the color by name (blue), by hexadecimal triplet, specified in HTML syntax (#0000ff), or hexadecimal format (0x0000ff). If a name specifies a color, the name uses the JMP definition of the color.
