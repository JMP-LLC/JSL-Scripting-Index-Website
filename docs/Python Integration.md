...
### Copyability with copy.copy

**Description:** DataConnector objects support shallow copying with the standard copy.copy function.



Note: Changing the copying behavior in subclasses is not supported, and as such subclasses should not add additional state as it will not be copied.
...
**Description:** Subclasses should override this function to create a connection. The returned connection, in the form of a (subclass of) jmp.DataSource, is used to provide Query Builder integration.



Note: Avoid storing and using a reference to the connector instance (self) in the returned data source. Other code may also hold a reference and change configuration values out from under the data source. If referencing the connector is desired, copy.copy can be used to make an independent copy, like "self_copy = copy.copy(self)".
...
**Description:** Subclasses can override this function, which opens a table directly. It is used to implement the Open message in JSL (New Data Connector(...) << Open()).



If both this and _do_as_data_source are implemented, it should be possible to open the same set of tables with either.
...
**Description:** Associates a jmp.DataConnectorType subclass and a jmp.DataConnector subclass, "tying" them together. This way, the jmp.DataConnector subclass knows about the field definitions among other things, and when asked to create a data connector instance, the jmp.DataConnectorType subclass uses the specified subclass of jmp.DataConnector.



This works by setting attributes on the subclasses.
...
**Syntax:** field = jmp.DataConnectorField(type, <default=DEFAULT_VALUE,> <tooltip="TOOLTIP" | None,> <ui_name="UI NAME" | None>

**Description:** DataConnectorField specifies a field.
...
default: The default value for the field, such as 42. If not provided, the default value depends on the type. It is None if that is supported, e.g. if the type is "int | None", and otherwise it is the "empty" value for the type â "" for str, 0 for int, and False for bool.



tooltip: If provided and not None, it will be used as a tooltip for the field in the data connector editor.



ui_name: If provided and not None, it will be used as the name of the field in the data connector editor instead of the actual name of the field.
...
# To see the tooltips and UI name you'll need to launch the Data Connector Editor
...
**Description:** jmp.DataConnectorGroupedFields divides fields into named groups. Each field subset is specified in a dictionary like usual.



Note: This division into groups affects the fields&apos; presentation in the data connector editor but does not affect their programmatic access or anything else. Also, the group name is only used in the UI, so it is fine for it to include characters that shouldn&apos;t be used in JSL names and therefore in field names.
...
**Description:** Subclasses must define a class-level variable "fields" that specifies the fields, a.k.a. the configuration options, for this type. It should be a dict or a jmp.DataConnectorGroupedFields object.
...
The following types are supported: bool, int, str, and optional versions of these, such as typing.Optional[bool] or bool | None. A missing value is represented by None.
...
**Description:** Subclasses can override this function to provide a list of the schemas in the data source. If not overridden, the data source is considered not to support schemas; returning None from an implementation is also equivalent.
...
**Description:** Subclasses should override this function to provide a list of the tables in the data source. If schemas are supported, this should be the tables under the schema whose name is passed. If they are not supported, the schema parameter should be ignored.
...
**Description:** Subclasses should override this function to get the table data for the named table. If schemas are not supported, the value of the schema argument can be ignored. It should return a jmp.DataTable or a string containing a path to a file in a data format supported by JMP.



Note: If returning a data table directly, the table should be created privately. If returning a path string, JMP will open the table itself and capture the settings it used, preserving them in Query Builder scripts.
...
**Syntax:** dt.new_column(  name=&apos;column_name&apos;,
...
**Syntax:** jmp.RowState(selected?=<boolean>, hidden?=<booleane>, labeled?=<boolean>, excluded?=<boolean>, color?=<int>, marker?=<int>)
...
**Description:** Set or create the corresponding JSL global variable from Python. Supports same object types as Python Send(), Python Get().
...
**Description:** Gets a JSL variable from the &apos;here&apos; namespace as a Python object. Will return None if the object cannot be found, and an opaque type for data types that cannot yet be handled. Capable of transferring the same types of objects as Python Send() and Python Get().
...
**Description:** Set or create the corresponding JSL variable in the &apos;here&apos; namespace. Supports same object types as Python Send(), Python Get().
...
**Description:** The jmpex package contains JMP supported extension interfaces.  Currently this contains only an R support module.
